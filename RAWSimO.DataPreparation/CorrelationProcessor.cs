...
public class CorrelationProcessor
{
    ...
    public static void WriteCorrelationMatrix(string footprintFile, string outputFile, IEnumerable<FootprintDatapoint.FootPrintEntry> measures)
    {
        ...
        var footprintLines = File.ReadAllLines(footprintFile).Skip(1).ToList(); // Skip the header
        ...
        var footprints = footprintLines.Select(l => new FootprintDatapoint(l)).ToList();
        Console.WriteLine("Done!");
        // Calculate correlations
        Console.Write("Calculating correlations for " + measures.Count().ToString(IOConstants.FORMATTER) + " measures ... ");
        var correlations = Cor(footprints, measures);
        var means = Mean(footprints, measures);
        var stddev = StdDev(footprints, measures);
        Console.WriteLine("Done!");
        // Write to file
    ...
    {
        // Init
        var results = new MultiKeyDictionary<FootprintDatapoint.FootPrintEntry, FootprintDatapoint.FootPrintEntry, double>();
        // Prepare means
        var means = measures.ToDictionary(m => m, m => data.Average(i => (double)i[m])); // Expecting double measures or at least int ones
    ...
    {
        // Init and get means
        var stddev = new Dictionary<FootprintDatapoint.FootPrintEntry, double>();
        var means = Mean(data, measures);
        // Calculate std dev
