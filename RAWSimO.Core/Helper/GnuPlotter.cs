...
public class GnuPlotter
{
    ...
    public static void Plot2DPoints(string filename, IEnumerable<Tuple<string, IEnumerable<Tuple<double, double>>>> points, string title)
    {
        ...
        {
            using (var sw = new StreamWriter(filename + datasetCounter++ + ".dat"))
            {
        ...
        // Write plot script
        using (var sw = new StreamWriter(filename + ".gp"))
        {
            sw.WriteLine("reset");
            ...
            foreach (var pointset in points)
            {
                var dataFilename = filename + datasetCounter++ + ".dat";
        ...
        using (var sw = new StreamWriter(filename + ".cmd"))
        {
