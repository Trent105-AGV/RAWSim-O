...
public abstract partial class BotManager
{
    ...
    internal bool AnyRelevantRequests(Pod pod, OutputStation station, PodSelectionExtractRequestFilteringMode filterForConsideration)
    {
        ...
        return filterForConsideration switch
        {
            PodSelectionExtractRequestFilteringMode.AssignedOnly =>
                // Check only assigned requests
                Instance.ResourceManager.GetExtractRequestsOfStation(station).Any(t => pod.IsAvailable(t.Item)),
            PodSelectionExtractRequestFilteringMode.AssignedAndQueuedEqually =>
                // Check assigned and queued requests equally
                Instance.ResourceManager.GetExtractRequestsOfStation(station)
                    .Concat(Instance.ResourceManager.GetQueuedExtractRequestsOfStation(station))
                    .Any(t => pod.IsAvailable(t.Item)),
            PodSelectionExtractRequestFilteringMode.AssignedAndCompleteQueued => Instance.ResourceManager
                    .GetExtractRequestsOfStation(station)
                    .Any(t => pod.IsAvailable(t.Item)) ||
                // Check completely assignable order's requests
                station.QueuedOrders.Any(o =>
                    Instance.ResourceManager.GetExtractRequestsOfOrder(o).Any(r => pod.IsAvailable(r.Item)) && Instance
                        .ResourceManager.GetExtractRequestsOfOrder(o)
                        .GroupBy(r => r.Item)
                        .All(i => pod.CountAvailable(i.Key) >= i.Count())),
            _ => throw new ArgumentException("Unknown mode: " + filterForConsideration)
        };
    }
    ...
    internal List<ExtractRequest> GetPossibleRequests(Pod pod, OutputStation station, PodSelectionExtractRequestFilteringMode filterForReservation)
    {
        ...
        var requestsToHandle = new List<ExtractRequest>();
        ...
        foreach (var itemRequestGroup in Instance.ResourceManager.GetExtractRequestsOfStation(station).GroupBy(r => r.Item))
        {
            ...
            var possibleRequests = itemRequestGroup.Take(_availableCounts[itemRequestGroup.Key]);
    ...
    private Func<double> GenerateScorerIStationForBotWithPod(PCScorerIStationForBotWithPod scorerConfig)
    {
        switch (scorerConfig.Type())
        {
            ...
            { var tempcfg = scorerConfig as PCScorerIStationForBotWithPodRandom; return () => { return Score(tempcfg, _currentBot, _currentIStation); }; }
            case PrefIStationForBotWithPod.Nearest:
            { var tempcfg = scorerConfig as PCScorerIStationForBotWithPodNearest; return () => { return Score(tempcfg, _currentBot, _currentIStation); }; }
            case PrefIStationForBotWithPod.WorkAmount:
            { var tempcfg = scorerConfig as PCScorerIStationForBotWithPodWorkAmount; return () => { return Score(tempcfg, _currentBot, _currentIStation); }; }
    ...
    private Func<double> GenerateScorerOStationForBotWithPod(PCScorerOStationForBotWithPod scorerConfig)
    {
        switch (scorerConfig.Type())
        {
            ...
            { var tempcfg = scorerConfig as PCScorerOStationForBotWithPodRandom; return () => { return Score(tempcfg, _currentBot, _currentOStation); }; }
            case PrefOStationForBotWithPod.Nearest:
            { var tempcfg = scorerConfig as PCScorerOStationForBotWithPodNearest; return () => { return Score(tempcfg, _currentBot, _currentOStation); }; }
            case PrefOStationForBotWithPod.WorkAmount:
            { var tempcfg = scorerConfig as PCScorerOStationForBotWithPodWorkAmount; return () => { return Score(tempcfg, _currentBot, _currentOStation); }; }
            default: throw new ArgumentException("Unknown score type: " + scorerConfig.Type());
        }
    }
    /// <summary>
    ...
    private Func<double> GenerateScorerPodForIStationBot(PCScorerPodForIStationBot scorerConfig)
    {
        switch (scorerConfig.Type())
        {
            ...
            { var tempcfg = scorerConfig as PCScorerPodForIStationBotRandom; return () => { return Score(tempcfg, _currentBot, _currentPod, _currentIStation); }; }
            case PrefPodForIStationBot.Nearest:
            { var tempcfg = scorerConfig as PCScorerPodForIStationBotNearest; return () => { return Score(tempcfg, _currentBot, _currentPod, _currentIStation); }; }
            case PrefPodForIStationBot.Demand:
            { var tempcfg = scorerConfig as PCScorerPodForIStationBotDemand; return () => { return Score(tempcfg, _currentBot, _currentPod, _currentIStation); }; }
            case PrefPodForIStationBot.WorkAmount:
            { var tempcfg = scorerConfig as PCScorerPodForIStationBotWorkAmount; return () => { return Score(tempcfg, _currentBot, _currentPod, _currentIStation); }; }
            default: throw new ArgumentException("Unknown score type: " + scorerConfig.Type());
        }
    }
    /// <summary>
    ...
    private Func<double> GenerateScorerPodForOStationBot(PCScorerPodForOStationBot scorerConfig)
    {
        switch (scorerConfig.Type())
        {
            ...
            { var tempcfg = scorerConfig as PCScorerPodForOStationBotRandom; return () => { return Score(tempcfg, _currentBot, _currentPod, _currentOStation); }; }
            case PrefPodForOStationBot.Fill:
            { var tempcfg = scorerConfig as PCScorerPodForOStationBotFill; return () => { return Score(tempcfg, _currentBot, _currentPod, _currentOStation); }; }
            case PrefPodForOStationBot.Nearest:
            { var tempcfg = scorerConfig as PCScorerPodForOStationBotNearest; return () => { return Score(tempcfg, _currentBot, _currentPod, _currentOStation); }; }
            case PrefPodForOStationBot.Demand:
            { var tempcfg = scorerConfig as PCScorerPodForOStationBotDemand; return () => { return Score(tempcfg, _currentBot, _currentPod, _currentOStation); }; }
            case PrefPodForOStationBot.Completeable:
            { var tempcfg = scorerConfig as PCScorerPodForOStationBotCompleteable; return () => { return Score(tempcfg, _currentBot, _currentPod, _currentOStation); }; }
            case PrefPodForOStationBot.WorkAmount:
            { var tempcfg = scorerConfig as PCScorerPodForOStationBotWorkAmount; return () => { return Score(tempcfg, _currentBot, _currentPod, _currentOStation); }; }
            default: throw new ArgumentException("Unknown score type: " + scorerConfig.Type());
        }
    }
    ...
    {
        return config.DistanceMetric switch
        {
            DistanceMetricType.Euclidean => Distances.CalculateEuclid(bot, station, Instance.WrongTierPenaltyDistance),
            DistanceMetricType.Manhattan => Distances.CalculateManhattan(bot, station,
                Instance.WrongTierPenaltyDistance),
            DistanceMetricType.ShortestPath => bot.CurrentWaypoint == null
                ?
            ...
            DistanceMetricType.ShortestTime => bot.CurrentWaypoint == null
                ?
            ...
            _ => throw new ArgumentException("Unknown distance metric: " + config.DistanceMetric)
        };
    }
    /// <summary>
    /// Determines a score that can be used to decide about an assignment.
    ...
    public double Score(PCScorerOStationForBotWithPodWorkAmount config, Bot bot, OutputStation station)
    {
        ...
        if (bot.Pod == null)
            return double.MaxValue;
        var value = config.ValueMetric switch
        {
            PCScorerWorkAmountValueMetric.OrderAge =>
                // Measure work amount by also including the time the order was already present at the station
                -Instance.ResourceManager.GetExtractRequestsOfStation(station)
                    .GroupBy(r => r.Item)
                    .Sum(g => g.OrderBy(o => o.Order.TimeStampSubmit)
                        .Take(bot.Pod.CountAvailable(g.Key))
                        .Sum(r => (Instance.Controller.CurrentTime - r.Order.TimeStampSubmit) /
                                  r.Order.Positions.Count())),
            PCScorerWorkAmountValueMetric.OrderDueTime =>
                // Measure work amount by also including the time the order was already present at the station
                -Instance.ResourceManager.GetExtractRequestsOfStation(station)
                    .GroupBy(r => r.Item)
                    .Sum(g => g.OrderBy(o => o.Order.DueTime)
                        .Take(bot.Pod.CountAvailable(g.Key))
                        .Sum(r => (config.OnlyPositiveLateness
                            ? Math.Max(Instance.Controller.CurrentTime - r.Order.DueTime, 0)
                            : Instance.Controller.CurrentTime - r.Order.DueTime) / r.Order.Positions.Count())),
            PCScorerWorkAmountValueMetric.Picks =>
            ...
            _ => throw new ArgumentException("Unknown value metric: " + config.ValueMetric)
        };
        // Penalize stations that are on a different tier
        if (config.PreferSameTier && bot.Tier != station.Tier)
    ...
    public double Score(PCScorerPodForOStationBotCompleteable config, Bot bot, Pod pod, OutputStation station)
    {
        ...
        var completeableAssignedOrders = 0;
        var completeableQueuedOrders = 0;
        ...
        foreach (var order in station.AssignedOrders)
        {
            ...
            var itemDemands = Instance.ResourceManager.GetExtractRequestsOfOrder(order).GroupBy(r => r.Item).ToList();
        ...
        var value = config.ValueMetric switch
        {
            PCScorerWorkAmountValueMetric.OrderAge =>
                ...
                -Instance.ResourceManager.GetExtractRequestsOfStation(station)
                    ...
                    .Sum(g => g.OrderBy(r => r.Station != null ? 0 : 1)
                        .ThenBy(r => r.Order.TimeStampSubmit)
                        .Take(pod.CountAvailable(g.Key))
                        // Sum the age we can get rid off (use a different weight for requests belonging to queued orders)
                        .Sum(r => (r.Station != null
                                      ?
                                      // Order is submitted
                                      (Instance.Controller.CurrentTime - r.Order.TimeStampSubmit) /
                                      r.Order.GetOpenDemandCount()
                                      :
                                      // Order is queued
                                      config.BacklogWeight *
                                      (Instance.Controller.CurrentTime - r.Order.TimeStampQueued) /
                                      r.Order.GetOpenDemandCount()) *
            ...
            PCScorerWorkAmountValueMetric.OrderDueTime =>
                ...
                -Instance.ResourceManager.GetExtractRequestsOfStation(station)
                    ...
                    // Per SKU get the already assigned requests first, but also consider the queued ones - take as many as the pod allows
                    .Sum(g => g.OrderBy(r => r.Station != null ? 0 : 1)
                        .ThenBy(r => r.Order.DueTime)
                        .Take(pod.CountAvailable(g.Key))
                        // Sum the age we can get rid off (use a different weight for requests belonging to queued orders)
                        .Sum(r => (r.Station != null
                                      ?
                                      // Order is submitted
                                      (config.OnlyPositiveLateness
                                          ? Math.Max(Instance.Controller.CurrentTime - r.Order.DueTime, 0)
                                          : Instance.Controller.CurrentTime - r.Order.DueTime) /
                                      r.Order.GetOpenDemandCount()
                                      :
                                      // Order is queued
                                      config.BacklogWeight *
    ...
    /// </summary>
    private Pod _currentPod;
    /// <summary>
    /// The current input station to assess
    /// </summary>
    private InputStation _currentIStation;
    /// <summary>
    /// The current output station to assess
    /// </summary>
    private OutputStation _currentOStation;
    // --->>> BEST CANDIDATE SELECTORS FOR INPUT TASKS
    /// <summary>
    /// A helper used to determine the best candidate.
    /// </summary>
    private BestCandidateSelector _bestPodIStationCandidateSelector;
    /// <summary>
    /// A helper used to determine the best candidate.
    /// </summary>
    private BestCandidateSelector _bestIStationCandidateSelector;
    // --->>> BEST CANDIDATE SELECTORS FOR OUTPUT TASKS
    /// <summary>
    /// A helper used to determine the best candidate.
    /// </summary>
    private BestCandidateSelector _bestPodOStationCandidateSelector;
    /// <summary>
    /// A helper used to determine the best candidate.
    /// </summary>
    private BestCandidateSelector _bestOStationCandidateSelector;
    ...
    protected bool DoStoreTaskForStation(Bot bot, InputStation iStation, bool extendSearch, double extendSearchRadius, DefaultPodSelectionConfiguration config)
    {
        ...
        if (bot.Pod != null)
        {
            ...
            if (AnyRelevantRequests(bot.Pod, iStation))
            {
                ...
                var fittingRequests = GetPossibleRequests(bot.Pod, iStation);
                // Log
            ...
            if (extendSearch)
            {
                ...
                {
                    // Get all fitting requests
                    var fittingRequests = GetPossibleRequests(bot.Pod, bestStation);
                    // Log
                    ...
                    if (_statIStationForPodScorerValues == null)
                        ...
                        for (var i = 0; i < _bestIStationCandidateSelector.BestScores.Length; i++)
        ...
        {
            // Get all fitting requests
            var fittingRequests = GetPossibleRequests(bestPod, iStation);
            // Log
            ...
            if (_statPodForIStationScorerValues == null)
                ...
                for (var i = 0; i < _bestPodIStationCandidateSelector.BestScores.Length; i++)
    ...
    protected bool DoExtractTaskForStation(Bot bot, OutputStation oStation, bool extendSearch, double extendedSearchRadius, DefaultPodSelectionConfiguration config)
    {
        ...
        if (bot.Pod != null)
        {
            ...
            {
                // Get all fitting requests
                var fittingRequests = GetPossibleRequests(bot.Pod, oStation, config.FilterForReservation);
                // Log
            ...
            if (extendSearch)
            {
                ...
                {
                    // Get all fitting requests
                    var fittingRequests = GetPossibleRequests(bot.Pod, bestStation, config.FilterForReservation);
                    // Log
                    ...
                    if (_statOStationForPodScorerValues == null)
                        ...
                        for (var i = 0; i < _bestOStationCandidateSelector.BestScores.Length; i++)
        ...
        {
            // Get all fitting requests
            var fittingRequests = GetPossibleRequests(bestPod, oStation, config.FilterForReservation);
            // Log
            ...
            if (_statPodForOStationScorerValues == null)
                ...
                for (var i = 0; i < _bestPodOStationCandidateSelector.BestScores.Length; i++)
    ...
    /// </summary>
    private bool _onTheFlyExtractSituationInvestigated;
    /// <summary>
    /// Indicates whether the current store request situation was already investigated.
    /// </summary>
    private bool _onTheFlyStoreSituationInvestigated;
    /// <summary>
        ...
        if (!_onTheFlyExtractSituationInvestigated || !_onTheFlyStoreSituationInvestigated)
        {
            ...
            foreach (var bot in Instance.Bots
                         ...
                         .Where(b =>
                         {
                             ...
                             // Ensure that there is any work to do
                             if (b.CurrentTask is InsertTask)
                                 return
                         ...
                         .OrderBy(b =>
                         {
                             ...
                             var stationWP =
                                 ...
                                 (b.CurrentTask as InsertTask)?.InputStation.Waypoint;
                         ...
                         }))
            {
                ...
                {
                    // Fetch the task
                    var extractTask = bot.CurrentTask as ExtractTask;
                    // Match more items to orders of the station, if possible
                    var itemsToHandle = GetPossibleRequests(bot.Pod, extractTask.OutputStation, config.FilterForReservation);
                ...
                {
                    // Fetch the task
                    var storeTask = bot.CurrentTask as InsertTask;
                    // Match more bundles to orders of the station, if possible
                    var bundlesToHandle = GetPossibleRequests(bot.Pod, storeTask.InputStation);
                    // Add new matches to current task
    ...
    /// </summary>
    private double[] _statPodForOStationScorerValues;
    /// <summary>
    /// Contains the number of assignments done.
    /// </summary>
    private double _statPodForOStationAssignments;
    /// <summary>
    /// Contains the aggregated scorer values.
    /// </summary>
    private double[] _statPodForIStationScorerValues;
    /// <summary>
    /// Contains the number of assignments done.
    /// </summary>
    private double _statPodForIStationAssignments;
    /// <summary>
    /// Contains the aggregated scorer values.
    /// </summary>
    private double[] _statOStationForPodScorerValues;
    /// <summary>
    /// Contains the number of assignments done.
    /// </summary>
    private double _statOStationForPodAssignments;
    /// <summary>
    /// Contains the aggregated scorer values.
    /// </summary>
    private double[] _statIStationForPodScorerValues;
    /// <summary>
    /// Contains the number of assignments done.
    /// </summary>
    private double _statIStationForPodAssignments;
    /// <summary>
    ...
    {
        var scoreInfos = new List<string>();
        if (_statPodForOStationScorerValues != null)
