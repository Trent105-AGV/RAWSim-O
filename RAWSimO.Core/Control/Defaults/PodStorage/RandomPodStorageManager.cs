...
public class RandomPodStorageManager : PodStorageManager
{
    ...
    private readonly RandomPodStorageConfiguration _config;
    ...
    protected override Waypoint GetStorageLocationForPod(Pod pod)
    {
        ...
        if (!_config.PreferSameTier || Instance.Compound.Tiers.Count <= 1)
            return !Instance.ResourceManager.UnusedPodStorageLocations.Any()
                ...
                Instance.ResourceManager.UnusedPodStorageLocations.ElementAt(
                    Instance.Randomizer.NextInt(Instance.ResourceManager.UnusedPodStorageLocations.Count()));
        // Try to get a location on the same tier
        var locationsOfThisTier = Instance.ResourceManager.UnusedPodStorageLocations.Where(l => l.Tier == pod.Tier).ToArray();
        // Return a location on this tier, if there is one
        if (locationsOfThisTier.Length != 0)
            return locationsOfThisTier[Instance.Randomizer.NextInt(locationsOfThisTier.Length)];

        // Check success
        return !Instance.ResourceManager.UnusedPodStorageLocations.Any() ? throw new InvalidOperationException("There was no suitable storage location for the pod: " + pod) :
            // Default behavior: return a random location of all available ones
            Instance.ResourceManager.UnusedPodStorageLocations.ElementAt(Instance.Randomizer.NextInt(Instance.ResourceManager.UnusedPodStorageLocations.Count()));
    }
