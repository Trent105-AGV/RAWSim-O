...
public class StationBasedPodStorageManager : PodStorageManager
{
    ...
    /// </summary>
    private readonly StationBasedPodStorageConfiguration _config;
    ...
    {
        var minDistance = double.PositiveInfinity; Waypoint bestStorageLocation = null;
        foreach (var storageLocation in Instance.ResourceManager.UnusedPodStorageLocations)
        {
            // Calculate the distance
            var distance = _config.PodDisposeRule switch
            {
                StationBasedPodStorageLocationDisposeRule.Euclid => (_config.OutputStationMode
                    ? Instance.OutputStations.Cast<Circle>()
                    : Instance.InputStations.Cast<Circle>()).Min(station =>
                    Distances.CalculateEuclid(station, storageLocation, Instance.WrongTierPenaltyDistance)),
                StationBasedPodStorageLocationDisposeRule.Manhattan => (_config.OutputStationMode
                    ? Instance.OutputStations.Cast<Circle>()
                    : Instance.InputStations.Cast<Circle>()).Min(station =>
                    Distances.CalculateManhattan(station, storageLocation, Instance.WrongTierPenaltyDistance)),
                StationBasedPodStorageLocationDisposeRule.ShortestPath => (_config.OutputStationMode
                    ? Instance.OutputStations.Select(s => s.Waypoint)
                    : Instance.InputStations.Select(s => s.Waypoint)).Min(station =>
                    Distances.CalculateShortestPathPodSafe(storageLocation, station, Instance)),
                StationBasedPodStorageLocationDisposeRule.ShortestTime => (_config.OutputStationMode
                    ? Instance.OutputStations.Select(s => s.Waypoint)
                    : Instance.InputStations.Select(s => s.Waypoint)).Min(station =>
                    Distances.CalculateShortestTimePathPodSafe(storageLocation, station, Instance)),
