...
public class RandomMethodManager : MethodManager
{
    ...
    /// </summary>
    private readonly RandomMethodManagementConfiguration _config;
    /// <summary>
    ...
    /// </summary>
    private readonly List<PodStorageMethodType> _podManagers =
    [
    ...
    public override void Update(double lastTime, double currentTime)
    {
        ...
        if (currentTime > _nextChange)
        {
            ...
            {
                // Select next manager
                var nextPodManagerType = _podManagers[Instance.Randomizer.NextInt(_podManagers.Count)];
