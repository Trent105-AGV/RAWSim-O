...
public class WorkShiftStationActivationManager : StationManager
{
    ...
    /// </summary>
    private readonly WorkShiftStationActivationConfiguration _config;
    /// <summary>
    /// All given shifts.
    /// </summary>
    private readonly List<Tuple<double, bool>> _shifts;
    /// <summary>
    ...
    {
        var nextIndex = (_currentShiftIndex + 1) % _shifts.Count;
        var fullLoops = (int)Math.Floor(currentTime / _config.LoopTime);
        if (nextIndex == 0)
            fullLoops++;
