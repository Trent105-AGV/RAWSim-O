...
public class ConstantRatioStationManager : StationManager
{
    ...
    public ConstantRatioStationManager(Instance instance) : base(instance)
    {
        ...
        var activeStationCount = (int)((instance.OutputStations.Count + instance.InputStations.Count) * _config.ActiveRatio);
        var activeOutputStationCount = (int)(activeStationCount * _config.PickReplenishRatio);
        var activeInputStationCount = activeStationCount - activeOutputStationCount;
        var activeIStations = instance.InputStations.OrderBy(s => s.ActivationOrderID).Take(activeInputStationCount).ToList();
        var inactiveIStations = instance.InputStations.Except(activeIStations).ToList();
        var activeOStations = instance.OutputStations.OrderBy(s => s.ActivationOrderID).Take(activeOutputStationCount).ToList();
        var inactiveOStations = instance.OutputStations.Except(activeOStations).ToList();
    ...
    /// </summary>
    private readonly ConstantRatioStationActivationConfiguration _config;
