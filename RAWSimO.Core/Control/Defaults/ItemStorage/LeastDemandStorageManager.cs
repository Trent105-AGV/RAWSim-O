...
public class LeastDemandStorageManager : ItemStorageManager
{
    ...
    /// </summary>
    private readonly LeastDemandItemStorageConfiguration _config;
    /// <summary>
    /// The last pod.
    /// </summary>
    private Pod _lastChosenPod;
    ...
    protected override void DecideAboutPendingBundles()
    {
        foreach (var bundle in _pendingBundles.ToArray())
        {
            ...
            {
                // Choose the next pod to use by selecting the one with the least demand for its content
