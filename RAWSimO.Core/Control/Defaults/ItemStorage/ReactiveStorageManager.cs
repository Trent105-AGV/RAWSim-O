...
public class ReactiveStorageManager : ItemStorageManager
{
    ...
    /// </summary>
    private readonly ReactiveItemStorageConfiguration _config;
    /// <summary>
    /// Tracks the assignments of bundles to input-stations.
    /// </summary>
    private readonly Dictionary<ItemBundle, InputStation> _bundleToStation = new();
    ...
    protected override void DecideAboutPendingBundles()
    {
        ...
        {
            // Find a pod
            var chosenPod = _config.AllocationRule switch
            {
                ReactiveItemStorageAllocationRule.RandomSameTier => Instance.Pods
                    .Where(b => b.FitsForReservation(bundle))
                    .OrderBy(b =>
                    {
                        // Smalles == best == random
                        var orderValue = Instance.Randomizer.NextDouble();
                        // Punish pod in use
                        if (b.InUse) orderValue += 1;
                        // Punish wrong tier
                        if (b.Tier != _bundleToStation[bundle].Tier) orderValue += 1;
                        return orderValue;
                    })
                    .FirstOrDefault(),
                ReactiveItemStorageAllocationRule.EmptiestSameTier => Instance.Pods
                    .Where(b => b.FitsForReservation(bundle))
                    .OrderBy(b =>
                    {
                        // Smalles == best == emptiest
                        var orderValue = (b.CapacityInUse + b.CapacityReserved) / b.Capacity;
                        // Punish pod in use
                        if (b.InUse) orderValue += 1;
                        // Punish wrong tier
                        if (b.Tier != _bundleToStation[bundle].Tier) orderValue += 1;
                    ...
                    .FirstOrDefault(),
                _ => throw new ArgumentException("Unknown allocation rule: " + _config.AllocationRule)
            };
