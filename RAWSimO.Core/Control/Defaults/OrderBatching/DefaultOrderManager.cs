...
public class DefaultOrderManager : OrderManager
{
    ...
    /// </summary>
    private readonly DefaultOrderBatchingConfiguration _config;
    /// <summary>
    /// The station that was chosen last time.
    /// </summary>
    private OutputStation _lastChosenStation;
    ...
    {
        // Setup scorers
        _bestCandidateSelectOrder = _config.OrderSelectionRule switch
        {
            DefaultOrderSelection.Random => new BestCandidateSelector(false,
                () => { return Instance.Randomizer.NextDouble(); }),
            DefaultOrderSelection.FCFS => new BestCandidateSelector(false, () => { return _currentOrder.TimeStamp; }),
            DefaultOrderSelection.DueTime => new BestCandidateSelector(false, () => { return _currentOrder.DueTime; }),
            DefaultOrderSelection.FrequencyAge => new BestCandidateSelector(false, () =>
            {
                var orderBirth = 1 - (_currentOrder.TimeStamp - _oldestOrderTimestamp) /
                    (_newestOrderTimestamp - _oldestOrderTimestamp);
                var frequency =
                    _currentOrder.Positions.Average(p => Instance.FrequencyTracker.GetMeasuredFrequency(p.Key));
                return -(orderBirth * frequency);
            }),
            _ => throw new ArgumentException("Unknown selection rule: " + _config.OrderSelectionRule)
        };
        _bestCandidateSelectStation = _config.StationSelectionRule switch
        {
            DefaultOutputStationSelection.Random => new BestCandidateSelector(false,
                () => { return Instance.Randomizer.NextDouble(); }),
            DefaultOutputStationSelection.LeastBusy => new BestCandidateSelector(false,
                () =>
                {
                    return (_currentStation.CapacityInUse + _currentStation.CapacityReserved) /
                           (double)_currentStation.Capacity;
                }),
            DefaultOutputStationSelection.MostBusy => new BestCandidateSelector(false,
                () =>
                {
                    return -((_currentStation.CapacityInUse + _currentStation.CapacityReserved) /
                             (double)_currentStation.Capacity);
        ...
        _bestCandidateSelectFastLane = new BestCandidateSelector(true,
            ...
            () =>
            {
                return _config.FastLaneTieBreaker switch
                {
                    Shared.FastLaneTieBreaker.Random => Instance.Randomizer.NextDouble(),
                    Shared.FastLaneTieBreaker.EarliestDueTime => -_currentOrder.DueTime,
                    Shared.FastLaneTieBreaker.FCFS => -_currentOrder.TimeStamp,
                    _ => throw new ArgumentException("Unknown tie breaker: " + _config.FastLaneTieBreaker)
                };
            });
        if (_config.FastLane)
    ...
    protected override void DecideAboutPendingOrders()
    {
        ...
        var validStationNormalAssignment = _config.FastLane ? (Func<OutputStation, bool>)IsAssignableKeepFastLaneSlot : IsAssignable;
        var validStationFastLaneAssignment = IsAssignable;
        // Get some meta info
        PrepareAssessment();
        // Assign fast lane orders while possible
        var furtherOptions = true;
            ...
            if (bestOrder != null)
            {
                ...
                if (bestStation != null)
                {
                    ...
                    if (_statScorerValuesOrder == null)
                        ...
                        for (var i = 0; i < _bestCandidateSelectOrder.BestScores.Length; i++)
                    ...
                    if (!recycling)
                    {
                        if (_statScorerValuesStation == null)
                            ...
                            for (var i = 0; i < _bestCandidateSelectStation.BestScores.Length; i++)
    ...
    /// </summary>
    private double[] _statScorerValuesOrder;
    /// <summary>
    /// Contains the aggregated scorer values.
    /// </summary>
    private double[] _statScorerValuesStation;
    /// <summary>
    /// Contains the number of selections done.
    /// </summary>
    private double _statOrderSelections;
    /// <summary>
    /// Contains the number of selections done.
    /// </summary>
    private double _statStationSelections;
    /// <summary>
    ...
    public override void StatFinish()
    {
        Instance.StatCustomControllerInfo.CustomLogOBString =
            _statScorerValuesOrder == null || _statScorerValuesStation == null ? "" :
                ...
                IOConstants.DELIMITER_CUSTOM_CONTROLLER_FOOTPRINT +
