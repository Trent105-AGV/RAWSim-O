...
public class PodMatchingOrderManager : OrderManager
{
    ...
    /// </summary>
    private readonly PodMatchingOrderBatchingConfiguration _config;
    ...
    private void Initialize()
    {
        ...
        var normalScorers = new List<Func<double>>();
        ...
        normalScorers.Add(() =>
        {
            return _config.TieBreaker switch
            {
                Shared.OrderSelectionTieBreaker.Random => Instance.Randomizer.NextDouble(),
                Shared.OrderSelectionTieBreaker.EarliestDueTime => -_currentOrder.DueTime,
                Shared.OrderSelectionTieBreaker.FCFS => -_currentOrder.TimeStamp,
                _ => throw new ArgumentException("Unknown tie breaker: " + _config.FastLaneTieBreaker)
            };
        });
        // --> Setup fast lane scorers
        var fastLaneScorers = new List<Func<double>>
        {
            // If we run into ties use the oldest order
            () =>
            {
        ...
        while (furtherOptions)
        {
            ...
            if (chosenOrder != null)
            {
                ...
                if (_statScorerValues == null)
                    ...
                    for (var i = 0; i < _bestCandidateSelectNormal.BestScores.Length; i++)
    ...
    /// </summary>
    private double[] _statScorerValues;
    /// <summary>
    /// Contains the number of assignments done.
    /// </summary>
    private double _statAssignments;
    /// <summary>
