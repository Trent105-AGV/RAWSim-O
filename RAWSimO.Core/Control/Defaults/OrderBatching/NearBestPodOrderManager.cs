...
public class NearBestPodOrderManager : OrderManager
{
    ...
    /// </summary>
    private readonly NearBestPodOrderBatchingConfiguration _config;
    ...
    protected override void DecideAboutPendingOrders()
    {
        foreach (var order in _pendingOrders.Where(o => o.Positions.All(p => Instance.StockInfo.GetActualStock(p.Key) >= p.Value)).ToArray())
        {
            ...
            foreach (var station in Instance.OutputStations.Where(s => s.Active && s.FitsForReservation(order)))
            {
                ...
                {
                    var distance = _config.DistanceRule switch
                    {
                        NearBestPodOrderBatchingDistanceRule.Euclid => Distances.CalculateEuclid(pod, station,
                            Instance.WrongTierPenaltyDistance),
                        NearBestPodOrderBatchingDistanceRule.Manhattan => Distances.CalculateManhattan(pod, station,
                            Instance.WrongTierPenaltyDistance),
                        NearBestPodOrderBatchingDistanceRule.ShortestPath => Distances.CalculateShortestPathPodSafe(
                            pod.Waypoint, station.Waypoint, Instance),
                        _ => throw new ArgumentException("Unknown distance rule: " + _config.DistanceRule)
                    };
