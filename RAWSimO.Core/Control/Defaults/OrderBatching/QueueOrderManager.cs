...
public class QueueOrderManager : OrderManager
{
    ...
    public QueueOrderManager(Instance instance) : base(instance)
    {
        ...
        _stationQueues = new VolatileIDDictionary<OutputStation, HashSet<Order>>(instance.OutputStations.Select(s => new VolatileKeyValuePair<OutputStation, HashSet<Order>>(s,
            [])).ToList());
        _inboundPodsByDistance = new VolatileIDDictionary<OutputStation, List<Pod>>(instance.OutputStations.Select(s => new VolatileKeyValuePair<OutputStation, List<Pod>>(s,
            [])).ToList());
    ...
    /// </summary>
    private readonly QueueOrderBatchingConfiguration _config;

    /// <summary>
    /// The queues of orders per station.
    /// </summary>
    private readonly VolatileIDDictionary<OutputStation, HashSet<Order>> _stationQueues;
    ...
    private readonly VolatileIDDictionary<OutputStation, List<Pod>> _inboundPodsByDistance;
    private readonly VolatileIDDictionary<OutputStation, Pod> _nearestInboundPod;
    ...
    private Func<double> GenerateScorer(QueueOrderSelectionRuleConfig scorerConfig, bool stationAssignment)
    {
        switch (scorerConfig.Type())
        {
            ...
            { var tempcfg = scorerConfig as QueueOrderSelectionRandom; return () => { return Score(tempcfg, stationAssignment); }; }
            case QueueOrderSelectionRuleType.FCFS:
            { var tempcfg = scorerConfig as QueueOrderSelectionFCFS; return () => { return Score(tempcfg, stationAssignment); }; }
            case QueueOrderSelectionRuleType.EarliestDeadline:
            { var tempcfg = scorerConfig as QueueOrderSelectionEarliestDeadline; return () => { return Score(tempcfg, stationAssignment); }; }
            case QueueOrderSelectionRuleType.VacantDeadline:
            { var tempcfg = scorerConfig as QueueOrderSelectionDeadlineVacant; return () => { return Score(tempcfg, stationAssignment); }; }
            case QueueOrderSelectionRuleType.InboundMatches:
            { var tempcfg = scorerConfig as QueueOrderSelectionInboundMatches; return () => { return Score(tempcfg, stationAssignment); }; }
            case QueueOrderSelectionRuleType.Completable:
            { var tempcfg = scorerConfig as QueueOrderSelectionCompleteable; return () => { return Score(tempcfg, stationAssignment); }; }
            case QueueOrderSelectionRuleType.Lines:
            { var tempcfg = scorerConfig as QueueOrderSelectionMostLines; return () => { return Score(tempcfg, stationAssignment); }; }
            case QueueOrderSelectionRuleType.Related:
            { var tempcfg = scorerConfig as QueueOrderSelectionRelated; return () => { return Score(tempcfg, stationAssignment); }; }
            default: throw new ArgumentException("Unknown scorer type: " + scorerConfig.Type());
        }
    }
    ...
    private bool ValidFastLaneAssignment(OutputStation s, Order o)
    {
        return _nearestInboundPod[s] == null || _nearestInboundPod[s].GetDistance(s) > Instance.SettingConfig.Tolerance ? false :
            o.Requests.Any(r => r.State != RequestState.Finished && r.Pod != null && r.Pod != _nearestInboundPod[s]) ? false :
            o.Requests.Where(r => r.State != RequestState.Finished && r.Pod == null).GroupBy(r => r.Item)
    ...
    {
        // Init
        var podCount = _inboundPodsByDistance[_currentStation].Count;
        var podNumber = 0;
        double score = 0;
        // Get demands given by the current order
        var demands = Instance.ResourceManager.GetExtractRequestsOfOrder(_currentOrder).GroupBy(r => r.Item).ToList();
        ...
        foreach (var pod in _inboundPodsByDistance[_currentStation])
        {
            ...
            foreach (var item in demands.Select(d => d.Key))
            {
                ...
                if (_orderItemDemand[item] <= 0)
                    continue;
                // Get available count
                var availableCount = Math.Min(_orderItemDemand[item], pod.CountAvailable(item));
    ...
    private double Score(QueueOrderSelectionCompleteable config, bool queueAssignment)
    {
        ...
        if (config.OnlyNearestPod)
            ...
            return _nearestInboundPod[_currentStation] == null ? 0 :
                _currentOrder.Requests.Any(r => r.State != RequestState.Finished && r.Pod != null && r.Pod != _nearestInboundPod[_currentStation]) ? 0 :
                _currentOrder.Requests.Where(r => r.State != RequestState.Finished && r.Pod == null).GroupBy(r => r.Item)
                    .All(i => _nearestInboundPod[_currentStation].CountAvailable(i.Key) >= i.Count()) ? 1 : 0;
        // Order is completable with all inbound pods
        return _currentOrder.Requests.Where(r => r.State != RequestState.Finished && r.Pod == null).GroupBy(r => r.Item)
    ...
    protected override void DecideAboutPendingOrders()
    {
        ...
        var validStation = _config.FastLane ? (Func<OutputStation, bool>)ValidStationKeepFastLane : ValidStationIgnoreFastLane;
        // Only assign as many orders to the queue as specified
        var validQueue = (HashSet<Order> q) => { return q.Count < _config.QueueLength; };
