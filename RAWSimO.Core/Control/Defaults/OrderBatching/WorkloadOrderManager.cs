...
public class WorkloadOrderManager : OrderManager
{
    ...
    /// </summary>
    private readonly WorkloadOrderBatchingConfiguration _config;
    ...
    protected override void DecideAboutPendingOrders()
    {
        ...
        {
            var chosenStation = _config.OrderingRule switch
            {
                WorkloadOrderingRule.LowestOrderCount => Instance.OutputStations
                    .Where(s => s.Active &&
                                s.FitsForReservation(order)) // There has to be sufficient capacity left at the station
                    ...
                    .FirstOrDefault(),
                WorkloadOrderingRule.HighestOrderCount => Instance.OutputStations
                    .Where(s => s.Active &&
                                s.FitsForReservation(order)) // There has to be sufficient capacity left at the station
                    .OrderByDescending(s => s.CapacityInUse + s.CapacityReserved) // Choose the one with the most orders
                    .ThenBy(s => Instance.Randomizer.NextDouble()) // Use a random tie-breaker
                    .FirstOrDefault(),
                _ => throw new ArgumentException("Unknown ordering rule: " + _config.OrderingRule)
            };
