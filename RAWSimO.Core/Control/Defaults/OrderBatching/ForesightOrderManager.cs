...
public class ForesightOrderManager : OrderManager
{
    ...
    /// </summary>
    private readonly ForesightOrderBatchingConfiguration _config;
    ...
    private double ScoreWeightedDepletePod(OutputStation station, Order order)
    {
        ...
        {
            var podMatchesPosition = false;
            // Determine the inventory available for this position from all inbound pods
            var inboundInventoryAvailable = 0;
            foreach (var pod in station.InboundPods)
            {
                var podInventoryAvailable = pod.CountAvailable(position.Key);
    ...
    private double ScoreWeightedOTW(OutputStation station, Order order)
    {
        ...
        foreach (var position in order.Positions)
        {
            var podDoesNotMatchPosition = false;
            // Determine the inventory available for this position from all inbound pods
            var inboundInventoryAvailable = 0;
            foreach (var pod in station.InboundPods)
            {
    ...
    {
        var score = scoreFunction switch
        {
            ScoreFunctionStationOrder.InboundPodsAvailablePicks => ScoreNormal(station, order),
            ScoreFunctionStationOrder.InboundPodsAvailablePicksDepletePod => ScoreWeightedDepletePod(station, order),
            ScoreFunctionStationOrder.Deadline => ScoreDeadline(station, order),
            ScoreFunctionStationOrder.InboundPodsAvailablePicksNotDepletePod => ScoreWeightedOTW(station, order),
            _ => throw new ArgumentException("Unknown score function: " + _config.ScoreFunctionStationOrder)
        };
        return score;
    }
    ...
    protected override void DecideAboutPendingOrders()
    {
        ...
        var validStation = (OutputStation s) => { return s.Active && s.CapacityReserved + s.CapacityInUse < s.Capacity; };
           
        var validOrders = _pendingOrders.Where(o => o.Positions.All(p => Instance.StockInfo.GetActualStock(p.Key) >= p.Value)).ToList();
            ...
            Instance.OutputStations.Any(s => validStation(s)))
        {
            ...
            var bestOrderIndex = -1;
            var bestScore = double.NegativeInfinity;
            var bestScoreSecondLevel = double.NegativeInfinity;
                         ...
                         .OrderByDescending(s => s.Capacity - s.CapacityInUse - s.CapacityReserved))
            {
                ...
                for (var orderIndex = 0; orderIndex < validOrders.Count; orderIndex++)
                {
                    // Look at next order
                    var order = validOrders[orderIndex];
                    // Determine score
                    var score = Score(_config.ScoreFunctionStationOrder, station, order);
                    ...
                    {
                        var scoreSecondLevel = Score(_config.ScoreFunctionStationOrderSecondLevel, station, order);
