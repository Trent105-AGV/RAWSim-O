...
public class BalancedBotManager : BotManager
{
    ...
    /// The configuration.
    /// </summary>
    private readonly BalancedTaskAllocationConfiguration _config;
    /// <summary>
    /// All stations.
    /// </summary>
    private readonly List<Circle> _stations;
    /// <summary>
    ...
    /// </summary>
    private readonly Dictionary<Circle, List<Circle>> _allStationsList;
    /// <summary>
    /// Contains a list per station with the station in it as the only element.
    /// </summary>
    private readonly Dictionary<Circle, List<Circle>> _singleStationLists;
    /// <summary>
    /// The stations the bots are assigned to.
    /// </summary>
    private readonly Dictionary<Bot, Circle> _botStations = new();
    /// <summary>
    /// The bots per station.
    /// </summary>
    private readonly Dictionary<Circle, HashSet<Bot>> _stationBots = new();
    /// <summary>
    ...
    /// </summary>
    private readonly Dictionary<Circle, int> _stationWorkerCount = new();
    /// <summary>
    /// The stations the bots are currently working for (respects the real work - not only the assignment).
    /// </summary>
    private readonly Dictionary<Bot, Circle> _workerStations = new();
    /// <summary>
    /// Bots currently assigned to do repositioning jobs exclusively.
    /// </summary>
    private readonly HashSet<Bot> _repositioningBots;
    /// <summary>
    /// All currently unemployed bots
    /// </summary>
    private readonly HashSet<Bot> _unassignedBots = [];
    /// <summary>
    ...
    private void ReallocateBots(bool overrideActivity)
    {
        ...
        var activeInputStations = Instance.InputStations.Where(s => overrideActivity || s.ItemBundles.Any()).ToList();
        var activeOutputStations = Instance.OutputStations.Where(s => overrideActivity || s.AssignedOrders.Any()).ToList();
        // Check whether something changed since last time
        var currentlyActiveStations = activeInputStations.Cast<Circle>().Concat(activeOutputStations);
        ...
        var activeWeightInputStations = _config.WeightInputStations * ((double)activeInputStations.Count / Instance.InputStations.Count);
        var activeWeightOutputStations = _config.WeightOutputStations * ((double)activeOutputStations.Count / Instance.OutputStations.Count);
        ...
        var inputStationBots = Convert.ToInt32(Math.Floor(_bots.Count * (activeWeightInputStations / (activeWeightInputStations + activeWeightOutputStations))));
        var outputStationBots = Convert.ToInt32(Math.Floor(_bots.Count * (activeWeightOutputStations / (activeWeightInputStations + activeWeightOutputStations))));
        ...
        var stationBotGoals = _stations.ToDictionary(k => k, v => 0);
        for (var i = 0; inputStationBots > 0; i = (i + 1) % activeInputStations.Count)
        {
        ...
        for (var i = 0; outputStationBots > 0; i = (i + 1) % activeOutputStations.Count)
        {
        ...
        while (overflowBots > 0 && stationBotGoals.Any(s => s.Value < _config.BotsPerStationLimit))
        {
            ...
            var receivingStation = _stations.Where(s => stationBotGoals[s] < _config.BotsPerStationLimit).ArgMax(s => s.GetDistance(s.Tier.Length / 2.0, s.Tier.Width / 2.0));
        ...
        while (_stations.Any(s => stationBotGoals[s] != _stationBots[s].Count))
        {
            ...
            Bot bestBot = null; Circle receivingStation = null; var bestValue = double.PositiveInfinity; var unassignedBot = true;
            ...
            foreach (var station in _stations.Where(s => _stationBots[s].Count < stationBotGoals[s]))
            {
                ...
                foreach (var bot in _unassignedBots.Concat(_stationBots.Where(s => _stationBots[s.Key].Count > stationBotGoals[s.Key]).SelectMany(s => s.Value)))
                {
                    ...
                    var distance = bot.TargetWaypoint?.GetDistance(station) ?? Distances.CalculateEuclid(bot, station, Instance.WrongTierPenaltyDistance);
            ...
            if (reassignments != null)
                ...
                reassignments = [new(bestBot, unassignedBot ? null : _botStations[bestBot], receivingStation)];
            // Perform the switch
            if (unassignedBot)
