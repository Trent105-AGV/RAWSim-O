...
public class SwarmBotManager : BotManager
{
    ...
    private readonly SwarmTaskAllocationConfiguration _config;

    private readonly BotToStationMatch _botToStationMatch;

    private readonly Dictionary<object, double> _multiplikatorPerStation = new();

    private readonly Dictionary<object, double> _stimulusPerStation = new();
    ...
    /// </summary>
    public int numberOfFullStations;
        ...
        : base(instance)
    {
        ...
        var tmp = new Dictionary<object, int>();
    ...
    {
        var probabilityPerStation = new Dictionary<object, double>();
        RefreshStimuliPerStation();
        foreach (var pair in _stimulusPerStation)
        {
            //                      s_j ^2
            //--------------------------------------------------------
            //s_j^2 + alpha * Thresh_ij ^2 + beta * Distance_ij ^2
            var probabilty = Math.Pow(pair.Value, 2) / (Math.Pow(pair.Value, 2) + _config.alpha * Math.Pow(_threshold, 2) + _config.beta * Math.Pow(GetDistance(bot, pair.Key), 2));
        ...
        var thresholdForRestTask = Convert.ToInt32(_stimulusPerStation.Count() * _config.restPercentage);
    ...
    private object CalculateStation(Dictionary<Object, double> probabilityPerStation)
    {
        ...
        double sum = 0;
        foreach (var value in probabilityPerStation.Values)
        {
            sum += value;
        }

        //create a Random value and choose a Station by its probability
        var random = Instance.Randomizer.NextDouble() * sum;
        double marker = 0;
        foreach (var entry in probabilityPerStation)
        {
            marker += entry.Value;
            if (random <= marker)//then choose the station
            {
                //increase the multiplikator of all stations and reset it from the chosen one
                var keys = _multiplikatorPerStation.Keys.ToList();
                foreach (var multiplikatorKey in keys)
                {
    ...
    private void RefreshStimuliPerStation()
    {
        ...
        //Inputstation
        foreach (var inStation in Instance.InputStations)
        {
        ...
        //Outputstation
        foreach (var outStation in Instance.OutputStations)
        {
            double stimulus = 0;
            //Number of Bundles - Bots currently at this station
    ...
    private double GetDistance(Bot bot, Object station)
    {
        ...
        if (station is InputStation)
        {
            var inStation = station as InputStation;
            //Manhatten Distance
        ...
        {
            var outStation = station as OutputStation;
            //Manhatten Distance
...
{
    private readonly Dictionary<Bot, Object> _lastStation = new();
    private readonly Dictionary<Object, int> _botPerStationCount = new();
