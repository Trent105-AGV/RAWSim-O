...
public class ConstantRatioBotManager : BotManager
{
    ...
    /// The configuration.
    /// </summary>
    private readonly ConstantRatioTaskAllocationConfiguration _config;
    /// <summary>
    ...
    {
        // --> Allocate bots
        var pickBots = (int)(Instance.Bots.Count * _config.PickBotRatio);
        var replenishmentBots = Instance.Bots.Count - pickBots;
        // Check active stations
        var activeInputStations = Instance.InputStations.Where(s => firstAllocation || s.Active).ToList();
        var activeOutputStations = Instance.OutputStations.Where(s => firstAllocation || s.Active).ToList();
        ...
        for (var i = 0; replenishmentBots > 0; i = (i + 1) % activeInputStations.Count)
        {
        ...
        for (var i = 0; pickBots > 0; i = (i + 1) % activeOutputStations.Count)
        {
