...
/// </summary>
public class RandomBotManager : BotManager
{
    /// <summary>
    /// Creates a new instance of this manager.
    /// </summary>
    /// <param name="instance">The instance this manager belongs to.</param>
    public RandomBotManager(Instance instance)
        : base(instance)
    {
        ...
        _config = instance.ControllerConfig.TaskAllocationConfig as RandomTaskAllocationConfiguration;
    }

    private readonly RandomTaskAllocationConfiguration _config;
    private readonly Func<bool> _decideExtractMode;
    private readonly Dictionary<Bot, bool> _extractMode;
    ...
    private bool DoExtractTaskWithPod(Bot bot, Pod pod)
    {
        ...
        foreach (var oStation in Instance.OutputStations.OrderBy(s => GetOrderValue(s, bot)))
        {
            ...
            var fittingRequests = GetPossibleRequests(pod, oStation, PodSelectionExtractRequestFilteringMode.AssignedOnly);
    ...
    private bool DoStoreTaskWithPod(Bot bot, Pod pod)
    {
        ...
        foreach (var iStation in Instance.InputStations.OrderBy(s => GetOrderValue(s, bot)))
        {
            ...
            var fittingRequests = GetPossibleRequests(pod, iStation);
            if (fittingRequests.Any())
            {
                // Simply execute the next task with the pod
    ...
    protected override void GetNextTask(Bot bot)
    {
        ...
        var keepPod = bot.Pod != null && Instance.Randomizer.NextDouble() < _config.StickToPodProbability;
