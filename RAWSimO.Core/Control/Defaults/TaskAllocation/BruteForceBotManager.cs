...
public class BruteForceBotManager : BotManager
{
    ...
    private bool _deliverMode = true;
    ...
    public bool DoExtractTask(Bot bot)
    {
        ...
        EnqueueExtract(bot, bestStation, bestPod, [bestRequest]);
        return true;
    }
        ...
        out Waypoint bestStorageLocationOldPod)
    {
        ...
        var bestTaskTime = double.PositiveInfinity;
        bestPod = null;
        ...
        var baseTime = bot.PodTransferTime;
        ...
        foreach (var pod in Instance.ResourceManager.UnusedPods)
        {
            ...
            if (bot.Pod != null)
            {
                ...
                var targetPodWaypoint = pod.Waypoint;

                var minStorageTime = double.PositiveInfinity;
                foreach (var storageLocation in Instance.ResourceManager.UnusedPodStorageLocations)
                {
                    ...
                    var storeTime = 2 * bot.PodTransferTime;
    ...
    private void GetBestTaskForPod(Bot bot, Pod pod, Waypoint podLocation,
        out ExtractRequest bestExtractTask, out double bestTimeForExtract)
    {
        ...
        foreach (var delivery in Instance.ResourceManager.AvailableAndAssignedExtractRequests)
        {
            ...
            if (pod.IsContained(delivery.Item))
            {
                ...
                var sw = delivery.Station.Waypoint;
                var time = Math.Max(Estimators.EstimateTravelTimeEuclid(bot, podLocation, sw), Estimators.EstimateOutputStationWaitTime(bot, sw));
    ...
    public bool DoStoreTask(Bot bot)
    {
        ...
        EnqueueInsert(bot, bestStation, bestPod, [bestRequest]);
        return true;
    }
    ...
    public void ReserveBestItemToPickUp(Bot bot, out InsertRequest bestRequest, out Pod bestPod, out InputStation bestStation)
    {
        ...
        if (bot.Pod != null)
        {
            ...
            var closestInputStationTime = double.PositiveInfinity;
            ...
            foreach (var storeTask in Instance.ResourceManager.AvailableStoreRequests)
            {
                ...
                // Choose the worst of delivering or waiting
                var sw = storeTask.Station.Waypoint;
                var time = Math.Max(Estimators.EstimateTravelTimeEuclid(bot, bot.CurrentWaypoint, sw), Estimators.EstimateInputStationWaitTime(bot, sw));
        ...
        // Don't have a pod
        var bestPickupTime = double.PositiveInfinity;
        bestRequest = null;
        bestPod = null;
        ...
        foreach (var pod in Instance.ResourceManager.UnusedPods)
        {
            ...
            var pickupTime = Estimators.EstimateTravelTimeEuclid(bot, pod.Waypoint);
            ...
            foreach (var storeTask in Instance.ResourceManager.AvailableStoreRequests)
            {
                ...
                if (pod.CapacityInUse + storeTask.Bundle.BundleWeight <= pod.Capacity)
                {
                    ...
                    // Choose the worst of delivering or waiting
                    var sw = storeTask.Station.Waypoint;
                    var deliverTime = Math.Max(Estimators.EstimateTravelTimeEuclid(bot, pod.Waypoint, sw), Estimators.EstimateInputStationWaitTime(bot, sw));
    ...
    public void StorePodAtClosestStorageLocation(Bot bot)
    {
        ...
        {
            var distance = bot.GetDistance(w);
    ...
    /// </summary>
    /// <param name="bot">The bot to consider.</param>
    private void GetOutOfTheWay(Bot bot)
    {
