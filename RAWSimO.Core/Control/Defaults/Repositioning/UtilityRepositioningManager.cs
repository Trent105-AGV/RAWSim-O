...
public class UtilityRepositioningManager : RepositioningManager
{
    ...
    /// The configuration.
    /// </summary>
    private readonly UtilityRepositioningConfiguration _config;
    /// <summary>
    ...
    protected override RepositioningMove GetRepositioningMove(Bot robot)
    {
        ...
        var move = new RepositioningMove();
        var bestRankOffset = 0;
        foreach (var pod in Instance.ResourceManager.UnusedPods)
        {
            // Determine rank the pod should be located at
            var desiredRank = Instance.SharedControlElements.PodUtilityManager.DetermineRank(pod);
            // Determine rank offset
            var currentRank = Instance.ElementMetaInfoTracker.GetStorageLocationRank(pod.Waypoint);
            var rankOffset = Math.Abs(currentRank - desiredRank);
