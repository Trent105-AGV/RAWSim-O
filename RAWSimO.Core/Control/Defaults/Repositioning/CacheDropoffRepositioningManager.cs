...
public class CacheDropoffRepositioningManager : RepositioningManager
{
    ...
    /// The configuration.
    /// </summary>
    private readonly CacheDropoffRepositioningConfiguration _config;
    /// <summary>
    ...
    /// </summary>
    private readonly Bot _currentBot = null;
    /// <summary>
    ...
    private void Init()
    {
        ...
        scorers.Add(() =>
        {
            if (_currentPod.Waypoint.InfoTagCache == ZoneType.Dropoff && _currentStorageLocation.InfoTagCache == ZoneType.None)
                // Drop-off -> Normal
                return 2;
            if (_currentPod.Waypoint.InfoTagCache == ZoneType.Dropoff && _currentStorageLocation.InfoTagCache == ZoneType.Cache)
                // Drop-off -> Cache
                return 2;
            if (_currentPod.Waypoint.InfoTagCache == ZoneType.Cache && _currentStorageLocation.InfoTagCache == ZoneType.None)
                // Cache -> Normal
                return 1;
            if (_currentPod.Waypoint.InfoTagCache == ZoneType.None && _currentStorageLocation.InfoTagCache == ZoneType.Cache)
                // Normal -> Cache
                return 1;
        ...
        scorers.Add(() =>
        {
            ...
            if (_currentStorageLocation.InfoTagCache == ZoneType.Cache) // Cache filling move
        ...
        scorers.Add(() =>
        {
            ...
            {
                CacheStorageLocationSelectionRule.Euclid => -Distances.CalculateEuclid(_currentPod.Waypoint,
                    _currentStorageLocation, Instance.WrongTierPenaltyDistance),
                CacheStorageLocationSelectionRule.Manhattan => -Distances.CalculateManhattan(_currentPod.Waypoint,
                    _currentStorageLocation, Instance.WrongTierPenaltyDistance),
                CacheStorageLocationSelectionRule.ShortestPath => -Distances.CalculateShortestPathPodSafe(
                    _currentPod.Waypoint, _currentStorageLocation, Instance),
