...
internal class TurnoverClassBuilder : IUpdateable
{
    ...
    public TurnoverClassBuilder(Instance instance)
    {
        Instance = instance;
    ...
    public void ParseConfigAndEnsureCompatibility(double[] classBorders, double reallocationDelay, int reallocationOrderCount)
    {
        ...
        else
        {
            ...
            for (var i = 0; i < _classBorders.Length; i++)
    ...
    /// </summary>
    private Dictionary<Pod, int> _podClasses = new();
    ...
    public int DetermineStorageClass(ItemBundle bundle)
    {
        EnsureInit();
        return _itemClasses[bundle.ItemDescription];
    }
    ...
    public int DetermineStorageClass(Pod pod)
    {
        EnsureInit();
        return _podClasses[pod];
    }
    ...
    public List<Pod> GetClassPods(int storageClass)
    {
        EnsureInit();
        return _classStoragePods[storageClass];
    }
    ...
    public List<Waypoint> GetClassStorageLocations(int storageClass)
    {
        EnsureInit();
        return _classStorageLocations[storageClass];
    }
    ...
    private void NewOrderCallback(Order order)
    {
        _orderCountOfPeriod++;
    }
    ...
    private void Init()
    {
        ...
        _storageLocationsOrdered = Instance.Waypoints
            ...
            {
                var value = _storageLocationClassRule switch
                {
                    TurnoverPodStorageLocationClassRule.OutputStationDistanceEuclidean => Distances.CalculateEuclid(wp,
                        o, Instance.WrongTierPenaltyDistance),
                    TurnoverPodStorageLocationClassRule.OutputStationDistanceManhattan => Distances.CalculateManhattan(
                        wp, o, Instance.WrongTierPenaltyDistance),
                    TurnoverPodStorageLocationClassRule.OutputStationDistanceShortestPath => Distances
                        .CalculateShortestPathPodSafe(wp, o.Waypoint, Instance),
                    TurnoverPodStorageLocationClassRule.OutputStationDistanceShortestTime => Distances
                        .CalculateShortestTimePathPodSafe(wp, o.Waypoint, Instance),
                    _ => throw new ArgumentException("Unknown storage location rule: " + _storageLocationClassRule)
                };
        ...
        var tempPodsOrdered = _storageLocationsOrdered.ToList();
        var classFractions = _classBorders.ToList();
        for (var i = _classCount - 1; i > 0; i--)
    ...
    private void ReallocatePods()
    {
        ...
        _podsOrdered = Instance.Pods
            ...
            .ThenBy(p => Instance.OutputStations.Min(o =>
            {
                var value = _storageLocationClassRule switch
                {
                    TurnoverPodStorageLocationClassRule.OutputStationDistanceEuclidean => Distances.CalculateEuclid(p,
                        o, Instance.WrongTierPenaltyDistance),
                    TurnoverPodStorageLocationClassRule.OutputStationDistanceManhattan => Distances.CalculateManhattan(
                        p, o, Instance.WrongTierPenaltyDistance),
                    TurnoverPodStorageLocationClassRule.OutputStationDistanceShortestPath =>
                        Distances.CalculateShortestPathPodSafe(
                            Instance.WaypointGraph.GetClosestWaypoint(p.Tier, p.X, p.Y), o.Waypoint, Instance),
                    TurnoverPodStorageLocationClassRule.OutputStationDistanceShortestTime =>
        ...
        var classStorageCapacityShares = new Dictionary<int, double>();
        for (var i = 0; i < _classCount; i++)
        ...
        var currentClass = 0;
        var podCount = 0;
        var aggregatedRelativeCapacity = classStorageCapacityShares[0];
    ...
    private void ReallocateItems()
    {
        ...
        var overallCapacity = _classStoragePods.Sum(l => l.Sum(p => p.Capacity));
        var classStorageCapacityShares = new Dictionary<int, double>();
        for (var i = 0; i < _classCount; i++)
        ...
        var overallWeight = _itemDescriptionsOrdered.Sum(i => i.Weight);
        var weightedItemDemand = _itemDescriptionsOrdered.ToDictionary(
            k => k,
        ...
        var aggregatedWeightedCapacity = classStorageCapacityShares[0];
    ...
    {
        EnsureInit();
        ReallocateCallback(currentTime);
    }
}