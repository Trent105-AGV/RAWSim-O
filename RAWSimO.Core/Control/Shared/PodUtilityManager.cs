...
public class PodUtilityManager
{
    ...
    public void InitOrEnsureInit(PodUtilityConfiguration config)
    {
        ...
        if (_config != null)
        {
            ...
            if (!_config.Match(config))
                throw new ArgumentException("Incompatible pod utility configurations: " + _config + " vs. " + config);
        }
        else
        {
            // Store config
    ...
    public int DetermineRank(Pod pod)
    {
        ...
        var fractionalScoreIndex = (double)podScoreIndex / _instance.Pods.Count;
    ...
    public void PrepareAllPodRanks()
    {
        ...
        var currentRank = -1; var currentScore = double.PositiveInfinity;
    ...
    public Waypoint GetStorageLocation(Pod pod, Waypoint tripStart)
    {
        ...
        var storageLocationRank = DetermineRank(pod);
        var ranksAssessed = 0; Waypoint bestStorageLocation = null; var bestTripTime = double.PositiveInfinity; var goBetter = false;
        var betterRank = storageLocationRank;
        var worseRank = storageLocationRank;
        while (
            ...
            bestStorageLocation == null)
        {
            ...
            goBetter = !goBetter;
            // Get current rank
            var currentRank = goBetter ? betterRank : worseRank;
            ...
            foreach (var storageLocation in _instance.ElementMetaInfoTracker.GetStorageLocationsOfRank(currentRank).Where(sl => !_instance.ResourceManager.IsStorageLocationClaimed(sl)))
            {
                ...
                var tripTime = Distances.CalculateShortestTimePathPodSafe(tripStart, storageLocation, _instance);
