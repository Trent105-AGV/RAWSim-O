...
public class StoragePartitioner
{
    ...
    internal double GetCacheValue(Pod pod, OutputStation station, double weightSpeed, double weightUtility)
    {
        ...
        var score = Instance.ElementMetaInfoTracker.GetPodCombinedScore(pod, weightSpeed, weightUtility);
    ...
    internal void CreateOrEnsureZones(CacheConfiguration config)
    {
        ...
        if (_config != null)
        {
            ...
            if (!_config.Match(config))
                throw new ArgumentException("Incompatible cache configurations: " + _config + " vs. " + config);
        }
        else
        {
            // Init
            _config = config;
            var zoneTypes = new List<ZoneType> { ZoneType.Cache, ZoneType.Dropoff };
            ...
            CachePartitions = new VolatileIDDictionary<OutputStation, HashSet<Waypoint>>(Instance.OutputStations.Select(s =>
                new VolatileKeyValuePair<OutputStation, HashSet<Waypoint>>(s, [])).ToList());
            var cacheWPCountPerStation = (int)Math.Ceiling(UnzonedStorageLocations.Count * _config.CacheFraction / Instance.OutputStations.Count);
            DropoffPartitions = new VolatileIDDictionary<OutputStation, HashSet<Waypoint>>(Instance.OutputStations.Select(s =>
                new VolatileKeyValuePair<OutputStation, HashSet<Waypoint>>(s, [])).ToList());
            var dropoffWPCountPerStation = _config.DropoffCount;
            // Init selector
            var scorer = new BestCandidateSelector(false,
                // First adhere to preference between zone types
                () =>
                {
                    return _config.ZonePriority switch
                    {
                        ZonePriority.CacheFirst => _currentZoneType == ZoneType.Cache ? 0 : 1,
                        ZonePriority.DropoffFirst => _currentZoneType == ZoneType.Dropoff ? 0 : 1,
                        ZonePriority.CacheDropoffEqual => 0,
                        _ => throw new ArgumentException("Unknown priority: " + _config.ZonePriority)
                    };
                },
                // Then assess the main distance metric
                () =>
                {
                    return _currentZoneType switch
                    {
                        ZoneType.Cache => Distances.CalculateShortestTimePathPodSafe(_currentStorageLocation,
                            _currentStation.Waypoint, Instance),
                        ZoneType.Dropoff => Distances.CalculateShortestTimePathPodSafe(_currentStation.Waypoint,
                            _currentStorageLocation, Instance),
                        _ => throw new ArgumentException("Unknown zone type for partitioning: " +
                                                         _currentZoneType)
                    };
                },
                ...
                () =>
                {
                    return _currentZoneType switch
                    {
                        ZoneType.Cache => -Distances.CalculateShortestTimePathPodSafe(_currentStation.Waypoint,
                            _currentStorageLocation, Instance),
                        ZoneType.Dropoff => -Distances.CalculateShortestTimePathPodSafe(_currentStorageLocation,
                            _currentStation.Waypoint, Instance),
                        _ => throw new ArgumentException("Unknown zone type for partitioning: " +
                                                         _currentZoneType)
                    };
                   ...
                   DropoffPartitions.Values.Any(p => p.Count < dropoffWPCountPerStation))
            {
                ...
                var bestZoneType = ZoneType.None;
                ...
                if (bestStation == null)
                    throw new InvalidOperationException("Ran out of available assignments while partitioning the caches - partitions so far: " +
                                                        "Cache: " + string.Join(",", CachePartitions.Select(p => p.Key + "(" + p.Value.Count + ")")) +
                                                        "Dropoff: " + string.Join(",", DropoffPartitions.Select(p => p.Key + "(" + p.Value.Count + ")")));
                // Set assignment
                switch (bestZoneType)
                {
                    case ZoneType.Cache:
