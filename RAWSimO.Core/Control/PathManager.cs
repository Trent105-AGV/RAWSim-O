...
public abstract class PathManager : IUpdateable, IStatTracker
{
    ...
    protected PathManager(Instance instance)
    {
        //instance
        Instance = instance;
        Log = true; //log => high memory consumption
    }
    ...
    protected Graph GenerateGraph()
    {
        ...
        foreach (var queueOwner in Instance.InputStations.Cast<IQueuesOwner>().Union(Instance.OutputStations.Cast<IQueuesOwner>().Union(Instance.Elevators.Cast<IQueuesOwner>())))
        ...
        foreach (var tier in waypointGraph.GetWayPoints())
        {
            foreach (var waypointFrom in tier.Value)
            {
                ...
                //create Array
                var outgoingEdges = new Edge[waypointFrom.Paths.Count(w => w.Tier == tier.Key)];
                var outgoingElevatorEdges = new ElevatorEdge[waypointFrom.Paths.Count(w => w.Tier != tier.Key)];

                //fill Array
                var edgeId = 0;
                var elevatorEdgeId = 0;
    ...
    private void UpdateLocksAndObstacles()
    {
        ...
        var queueBlockedNodes = _queueManagers.SelectMany(m => m.Value.LockedWaypoints.Keys);
        // Prepare waypoints blocked by idling robots
        var idlingAgentsBlockedNodes = Instance.Bots.Cast<BotNormal>().Where(b => b.IsResting()).Select(b => b.CurrentWaypoint);
    ...
    private void getBotAgentDictionary(out Dictionary<BotNormal, Agent> botAgentDictionary, double currentTime)
    {
        ...
        foreach (var abot in Instance.Bots)
        {
            ...
            // Get way points
            var nextWaypoint = bot.NextWaypoint ?? bot.CurrentWaypoint;
            var finalDestination = bot.DestinationWaypoint ?? bot.CurrentWaypoint;
    ...
    internal class QueueManager
    {
        ...
        /// The queue way point
        /// </summary>
        public readonly Waypoint QueueWaypoint;

        /// <summary>
        /// The queue
        /// </summary>
        public readonly List<Waypoint> Queue;

        /// <summary>
        /// The locked way points
        /// </summary>
        public readonly Dictionary<Waypoint, Bot> LockedWaypoints;

        /// <summary>
        /// The indices regarding this queue of the managed waypoints.
        /// </summary>
        private readonly Dictionary<Waypoint, int> _queueWaypointIndices;

        /// <summary>
        /// The bots that are want to reach the way point at the beginning of the queue.
        /// </summary>
        private readonly List<BotNormal> _managedBots;

        /// <summary>
        /// The bots that are want to reach the queue way point and already reached the queue.
        /// </summary>
        private readonly HashSet<BotNormal> _botsInQueue;

        /// <summary>
        /// The assigned place in queue
        /// </summary>
        private readonly Dictionary<BotNormal, Waypoint> _placeInQueue;
        ...
        public QueueManager(Waypoint queueWaypoint, List<Waypoint> queue, PathManager pathManager)
        {
            ...
            var minArcLength = Queue.Min(firstWP => Queue.Where(wp => wp != firstWP).Min(secondWP => firstWP.GetDistance(secondWP)));
        ...
        public void Update()
        {
            ...
            //check bot states
            for (var i = 0; i < _managedBots.Count; i++)
            {
                var bot = _managedBots[i];
            ...
            foreach (var bot in _queueCruisePaths.Keys)
            {
                ...
                var currentQueueIndex = _queueWaypointIndices[realWP];
            ...
            foreach (var bot in _botsInQueue.Where(bot => !_placeInQueue.ContainsKey(bot)))
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        var lockedPredecessorWaypoints = LockedWaypoints.Keys.Where(q => _queueWaypointIndices[q] < queueIndex);
                        var targetQueueIndex = lockedPredecessorWaypoints.Any() ? lockedPredecessorWaypoints.Max(w => _queueWaypointIndices[w]) + 1 : QueueWaypoint.Elevator != null ? 1 : 0;
                        var cruisePath = new List<Waypoint>();
                        var nextIndex = -1;
                        // Build cruise path through queue
                        for (var currentIndex = queueIndex; currentIndex >= targetQueueIndex;)
                        {
                            // Determine next waypoint in queue to go to after this one (consider shortcuts)
                            var shortCuts = Queue[currentIndex].Paths.Where(p => _queueWaypointIndices.ContainsKey(p) && _queueWaypointIndices[p] < currentIndex && targetQueueIndex <= _queueWaypointIndices[p]);
                            ...
                            var inboundOrientation = cruisePath.Count >= 1 && currentIndex > targetQueueIndex ? Circle.GetOrientation(cruisePath[cruisePath.Count - 1].X, cruisePath[cruisePath.Count - 1].Y, Queue[currentIndex].X, Queue[currentIndex].Y) : double.NaN;
                            var outboundOrientation = cruisePath.Count >= 1 && currentIndex > targetQueueIndex ? Circle.GetOrientation(Queue[currentIndex].X, Queue[currentIndex].Y, Queue[nextIndex].X, Queue[nextIndex].Y) : double.NaN;
                            var stopRequired = !double.IsNaN(inboundOrientation) && Math.Abs(Circle.GetOrientationDifference(inboundOrientation, outboundOrientation)) >= bot.Instance.StraightOrientationTolerance;
                            // Add connection to the overall path
                            LockedWaypoints[Queue[currentIndex]] = bot;
                            var stopAtNode = currentIndex == queueIndex || currentIndex == targetQueueIndex || stopRequired;
                            path.AddLast(
                        ...
                        // Check path
                        for (var i = 0; i < cruisePath.Count - 1; i++)
                        {
            ...
            var firstFreeNode = Queue.Count - 1;
            ...
            while (botsLeftToQueue.Count > 0)
            {
                ...
                for (var i = 1; i < botsLeftToQueue.Count; i++)
                {
