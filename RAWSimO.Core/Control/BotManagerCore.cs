...
public abstract partial class BotManager : IUpdateable, IOptimize, IStatTracker
{
    ...
    /// The waypoint graph.
    /// </summary>
    protected WaypointGraph waypointGraph;
    /// <summary>
    ...
    /// </summary>
    private readonly Dictionary<Bot, BotTask> _taskQueues = new();
    /// <summary>
    /// The last task that was assigned to the bot.
    /// </summary>
    private readonly Dictionary<Bot, BotTask> _lastTaskEnqueued = new();
    /// <summary>
    ...
    {
        var task = new ExtractTask(Instance, bot, pod, station, requests);
        task.Prepare();
    ...
    {
        var task = new InsertTask(Instance, bot, pod, station, requests);
        task.Prepare();
        if (_taskQueues[bot] != null)
            _taskQueues[bot].Cancel();
        _taskQueues[bot] = task;
    ...
    {
        var task = new ParkPodTask(Instance, bot, pod, storageLocation);
        task.Prepare();
        if (_taskQueues[bot] != null)
            _taskQueues[bot].Cancel();
        _taskQueues[bot] = task;
    ...
    {
        var task = new RepositionPodTask(Instance, bot, pod, storageLocation);
        task.Prepare();
        if (_taskQueues[bot] != null)
            _taskQueues[bot].Cancel();
        _taskQueues[bot] = task;
    ...
    {
        var task = new RestTask(Instance, bot, restLocation);
        task.Prepare();
        if (_taskQueues[bot] != null)
            _taskQueues[bot].Cancel();
        _taskQueues[bot] = task;
    ...
    protected void DoRestTask(Bot bot, PrefRestLocationForBot restLocationOrder)
    {
        ...
        var lastTask = GetLastEnqueuedTask(bot);
    ...
    protected bool DoRepositioningTask(Bot bot)
    {
        ...
        var move = Instance.Controller.RepositioningManager.GetNextMove(bot);
    ...
    public void TaskAborted(Bot r, BotTask t)
    {
        ...
        if (_taskQueues[r] == null)
            throw new InvalidOperationException("No task to abort - referenced task was: " + t);
        if (t.Type != BotTaskType.None && t != _taskQueues[r])
