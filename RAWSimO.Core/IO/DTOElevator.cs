...
public class DTOElevator : IDataTransferObject<Elevator, DTOElevator>
{
    ...
    public static implicit operator DTOElevator(Elevator value)
    {
        ...
        foreach (var queueStart in value.Queues.Keys)
            queues.AddRange(value.Queues[queueStart].Select(w => queueStart.ID.ToString() + IOConstants.DELIMITER_TUPLE + w.ID));

        // Init the DTO object with the values of the given elevator
        var elevator = new DTOElevator
        {
            ID = value.ID,
            ...
            Timings = value.ConnectedPoints.SelectMany(wp1 => value.ConnectedPoints.Where(wp => wp != wp1).Select(wp2 =>
            {
                return
                    ...
                    wp2.ID + IOConstants.DELIMITER_TUPLE +
    ...
    public static void SetConnections(Instance instance, List<DTOElevator> elevators)
    {
        foreach (var elevatorValue in elevators)
        {
            ...
            var elevator = instance.GetElevatorByID(elevatorValue.ID);
            // Parse connections
            var connectedPoints = elevatorValue.Waypoints.Select(id => instance.GetWaypointByID(id)).ToList();
            // Set connections
            elevator.RegisterPoints(0, connectedPoints);
            // Parse timings
            var timings = elevatorValue.Timings.Select(
                t =>
                {
                    var values = t.Split(IOConstants.DELIMITER_TUPLE);
    ...
    {
        //set queue
        var me = instance.Elevators.Find(elevator => elevator.ID == ID);
        me.Queues = new Dictionary<Waypoint, List<Waypoint>>();
    ...
    {
        // Init the elevator
        var elevator = instance.CreateElevator(ID);
        // Return it
        return elevator;
    }
