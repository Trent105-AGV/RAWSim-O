...
public class DTOWaypoint : IDataTransferObject<Waypoint, DTOWaypoint>
{
    ...
    public static implicit operator DTOWaypoint(Waypoint value)
    {
        ...
        var waypoint = new DTOWaypoint
        {
            ...
            OutputStation = value.OutputStation?.ID ?? INT_NULL,
            InputStation = value.InputStation?.ID ?? INT_NULL,
            Elevator = value.Elevator?.ID ?? INT_NULL,
            Pod = value.Pod?.ID ?? INT_NULL,
