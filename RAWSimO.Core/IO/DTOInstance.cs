...
public class DTOInstance : IDataTransferObject<Instance, DTOInstance>
{
    ...
    public static implicit operator DTOInstance(Instance value)
    {
        ...
        var dtoInstance = new DTOInstance { Name = value.Name };
        dtoInstance.Bots = [];
    ...
    public Instance Submit(Instance instance)
    {
        ...
        if (instance.SettingConfig.OverrideConfig != null && instance.SettingConfig.OverrideConfig.OverrideInputStationCount)
        {
            ...
            var stationsToKeep = Math.Max(1, (int)Math.Floor(instance.SettingConfig.OverrideConfig.OverrideInputStationCountValue * InputStations.Count));
        ...
        {
            // Determine stations to remove, but at least keep one
            var stationsToKeep = Math.Max(1, (int)Math.Floor(instance.SettingConfig.OverrideConfig.OverrideOutputStationCountValue * OutputStations.Count));
        ...
        if (instance.SettingConfig.OverrideConfig != null && instance.SettingConfig.OverrideConfig.OverrideBotCountPerOStation)
        {
            ...
            var referenceBot = Bots.First();
            Bots.Clear();
            var _usedBotPositions = new HashSet<DTOWaypoint>();
            for (var i = 1; i <= instance.SettingConfig.OverrideConfig.OverrideBotCountPerOStationValue * OutputStations.Count; i++)
            {
                // Choose random waypoint to set bot position to
                var botPosition = Waypoints.Where(w => !w.IsQueueWaypoint && !w.PodStorageLocation && !_usedBotPositions.Contains(w)).OrderBy(w => rand.NextDouble()).First();
