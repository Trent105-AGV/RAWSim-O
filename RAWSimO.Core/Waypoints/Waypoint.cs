...
public class Waypoint : Circle, IWaypointInfo, IExposeVolatileID
{
    ...
    /// </summary>
    private readonly Dictionary<Waypoint, double> _paths = new();

    /// <summary>
    /// Returns all connected waypoints as an enumeration.
    /// </summary>
    public IEnumerable<Waypoint> Paths => _paths.Keys;
    ...
    public double this[Waypoint other] => GetPathDistance(other);

    /// <summary>
    /// Contains all guards used to block certain paths dynamically.
    /// </summary>
    private readonly Dictionary<Waypoint, List<QueueGuard>> _guards = new();

    /// <summary>
    /// The bots registered at this waypoint (approaching).
    /// </summary>
    private readonly HashSet<Bot> _botsApproaching = [];

    /// <summary>
    /// The bots registered at this waypoint (leaving).
    /// </summary>
    private readonly HashSet<Bot> _botsLeaving = [];
    ...
    /// </summary>
    public int BotCountOverall => _botsApproaching.Count + _botsLeaving.Count;

    /// <summary>
    /// Indicates whether any bots are registered at this waypoint.
    /// </summary>
    public bool AnyBotsOverall => _botsApproaching.Any() || _botsLeaving.Any();

    /// <summary>
    /// Supplies the number of bots currently approaching the waypoint.
    /// </summary>
    public int BotCountApproaching => _botsApproaching.Count;

    /// <summary>
    /// Indicates whether any bots are approaching the waypoint.
    /// </summary>
    public bool AnyBotsApproaching => _botsApproaching.Any();

    /// <summary>
    /// Supplies the number of bots currently leaving the waypoint.
    /// </summary>
    public int BotCountLeaving => _botsLeaving.Count;

    /// <summary>
    /// Indicates whether any bots are leaving the waypoint.
    /// </summary>
    public bool AnyBotsLeaving => _botsLeaving.Any();
    ...
    public double ShortestPodPathDistanceToNextOutputStation
    {
        get
        {
            if (double.IsNaN(field))
                field = Instance.OutputStations.Min(s => Distances.CalculateShortestPathPodSafe(this, s.Waypoint, Instance));
            return field;
        }
    } = double.NaN;
    ...
    public double ShortestPodPathDistanceToNextInputStation
    {
        ...
        {
            if (double.IsNaN(field))
                field = Instance.InputStations.Min(s => Distances.CalculateShortestPathPodSafe(this, s.Waypoint, Instance));
            return field;
        }
    } = double.NaN;
    ...
    /// </summary>
    public int StatOutgoingTrips { get { return _statForwardStar?.Sum(c => c.Value.Count) ?? 0; } }
    /// <summary>
    /// The overall time spent for trips outgoing from this waypoint.
    /// </summary>
    public double StatOutgoingTripTime { get { return _statForwardStar?.Sum(c => c.Value.TravelTimeSum) ?? 0; } }
    /// <summary>
