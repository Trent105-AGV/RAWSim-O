...
public class QueueGuard : IGuardInfo
{
    ...
    /// </summary>
    public QueueSemaphore Semaphore { get; }

    /// <summary>
    /// Indicates whether the protected area is accessible (not at its limit) or not.
    /// </summary>
    public bool IsAccessible => !Barrier || Semaphore.IsAccessible;
    ...
    public override string ToString() { return "Guard-" + From + "->" + To; }
    ...
    public double GetInfoCenterX()
    {
        ...
        var third = Math.Abs(From.X - To.X) / 3.0;
        var xEntry = From.X < To.X ? To.X - third : To.X + third;
        var xExit = From.X < To.X ? From.X + third : From.X - third;
    ...
    {
        // Calculate intermediate values
        var third = Math.Abs(From.Y - To.Y) / 3.0;
        var yEntry = From.Y < To.Y ? To.Y - third : To.Y + third;
        var yExit = From.Y < To.Y ? From.Y + third : From.Y - third;
        // If entry gate mark the center near to the to side
        if (Entry)
            return yEntry;
        // If exit gate mark the center near to the from side
