...
{
    private readonly Dictionary<Tier, HashSet<Waypoint>> _waypoints = new();
    private readonly Dictionary<InputStation, Waypoint> _inputStations = new();
    private readonly Dictionary<OutputStation, Waypoint> _outputStations = new();
    private readonly Dictionary<Pod, Waypoint> _pods = new();
    private readonly Dictionary<Tier, QuadTree<Waypoint>> _waypointQuadtree = new();
    private readonly Dictionary<Tier, double> _largestEdgeBetweenTwoWaypoints = new();

    /// <summary>
    /// All input-station waypoints.
    /// </summary>
    public IEnumerable<Waypoint> InputStationWaypoints => _inputStations.Values;

    /// <summary>
    /// All output-station waypoints.
    /// </summary>
    public IEnumerable<Waypoint> OutputStationWaypoints => _outputStations.Values;
    ...
    public Waypoint GetClosestWaypoint(Tier tier, double x, double y)
    {
        ...
        var closest = GetWaypointsWithinDistance(tier, x, y, _largestEdgeBetweenTwoWaypoints[tier]).OrderBy(wp => wp.GetDistance(x, y)).FirstOrDefault();
        if (closest != null)
            return closest;
