...
public class DefaultOrderBatchingConfiguration : OrderBatchingConfiguration
{
    ...
    public override string GetMethodName()
    {
        ...
        var name = "obD";
        name += OrderSelectionRule switch
        {
            DefaultOrderSelection.Random => "r",
            DefaultOrderSelection.FCFS => "f",
            DefaultOrderSelection.DueTime => "d",
            DefaultOrderSelection.FrequencyAge => "a",
            _ => throw new ArgumentException("Unexpected argument!")
        };
        name += StationSelectionRule switch
        {
            DefaultOutputStationSelection.Random => "r",
            DefaultOutputStationSelection.LeastBusy => "l",
            DefaultOutputStationSelection.MostBusy => "m",
            _ => throw new ArgumentException("Unexpected argument!")
        };
...
public class WorkloadOrderBatchingConfiguration : OrderBatchingConfiguration
{
    ...
    public override string GetMethodName()
    {
        if (!string.IsNullOrWhiteSpace(Name)) return Name;
        var name = "obWL";
        name += OrderingRule switch
        {
            WorkloadOrderingRule.LowestOrderCount => "l",
            WorkloadOrderingRule.HighestOrderCount => "h",
            _ => throw new ArgumentException("Unexpected argument!")
        };
        return name;
    }
    ...
    /// </summary>
    public readonly WorkloadOrderingRule OrderingRule = WorkloadOrderingRule.LowestOrderCount;
}
...
public class RelatedOrderBatchingConfiguration : OrderBatchingConfiguration
{
    ...
    public override string GetMethodName()
    {
        if (!string.IsNullOrWhiteSpace(Name)) return Name;
        var name = "obRL";
        name += TieBreaker switch
        {
            RelatedOrderBatchingTieBreaker.Random => "r",
            RelatedOrderBatchingTieBreaker.LeastBusy => "l",
            RelatedOrderBatchingTieBreaker.MostBusy => "m",
            _ => throw new ArgumentException("Unexpected argument!")
        };
        return name;
    }
    ...
    /// </summary>
    public readonly RelatedOrderBatchingTieBreaker TieBreaker = RelatedOrderBatchingTieBreaker.LeastBusy;
}
...
public class NearBestPodOrderBatchingConfiguration : OrderBatchingConfiguration
{
    ...
    public override string GetMethodName()
    {
        if (!string.IsNullOrWhiteSpace(Name)) return Name;
        var name = "obNBP";
        name += DistanceRule switch
        {
            NearBestPodOrderBatchingDistanceRule.Euclid => "e",
            NearBestPodOrderBatchingDistanceRule.Manhattan => "m",
            NearBestPodOrderBatchingDistanceRule.ShortestPath => "s",
            _ => throw new ArgumentException("Unexpected argument!")
        };
        return name;
    }
    ...
    /// </summary>
    public readonly NearBestPodOrderBatchingDistanceRule DistanceRule = NearBestPodOrderBatchingDistanceRule.ShortestPath;
}
...
public class ForesightOrderBatchingConfiguration : OrderBatchingConfiguration
{
    ...
    /// </summary>
    public readonly ScoreFunctionStationOrder ScoreFunctionStationOrder = ScoreFunctionStationOrder.InboundPodsAvailablePicks;
    /// <summary>
    /// The tie breaker function to determine the best combination of order and station.
    /// </summary>
    public readonly ScoreFunctionStationOrder ScoreFunctionStationOrderSecondLevel = ScoreFunctionStationOrder.Deadline;
}
...
public class QueueOrderBatchingConfiguration : OrderBatchingConfiguration
{
    ...
    /// </summary>
    public readonly int QueueLength = 20;
    /// <summary>
    /// Indicates whether there always is one capacity slot of a station reserved for an immediately fulfillable order.
    /// </summary>
    public readonly bool FastLane = true;

    /// <summary>
    /// Rule settings for selecting an order to be assigned to a station (main rule).
    /// </summary>
    public readonly QueueOrderSelectionRuleConfig StationOrderSelectionRule1 = new QueueOrderSelectionDeadlineVacant();
    /// <summary>
    /// Rule settings for selecting an order to be assigned to a station (first tie breaker).
    /// </summary>
    public readonly QueueOrderSelectionRuleConfig StationOrderSelectionRule2 = new QueueOrderSelectionCompleteable { OnlyNearestPod = true };
    /// <summary>
    /// Rule settings for selecting an order to be assigned to a station (second tie breaker).
    /// </summary>
    public readonly QueueOrderSelectionRuleConfig StationOrderSelectionRule3 = new QueueOrderSelectionCompleteable { OnlyNearestPod = false };
    /// <summary>
    /// Rule settings for selecting an order for an open fast lane slot of a station (main rule).
    /// </summary>
    public readonly QueueOrderSelectionRuleConfig FastLaneOrderSelectionRule1 = new QueueOrderSelectionMostLines();
    /// <summary>
    /// Rule settings for selecting an order for an open fast lane slot of a station (first tie breaker).
    /// </summary>
    public readonly QueueOrderSelectionRuleConfig FastLaneOrderSelectionRule2 = new QueueOrderSelectionEarliestDeadline();
    /// <summary>
    /// Rule settings for selecting an order for an open fast lane slot of a station (second tie breaker).
    /// </summary>
    public readonly QueueOrderSelectionRuleConfig FastLaneOrderSelectionRule3 = new QueueOrderSelectionFCFS();
    /// <summary>
    /// Rule settings for selecting an order for the queue of a station (main rule).
    /// </summary>
    public readonly QueueOrderSelectionRuleConfig QueueOrderSelectionRule1 = new QueueOrderSelectionInboundMatches();
    /// <summary>
    /// Rule settings for selecting an order for the queue of a station (first tie breaker).
    /// </summary>
    public readonly QueueOrderSelectionRuleConfig QueueOrderSelectionRule2 = new QueueOrderSelectionRelated();
    /// <summary>
    /// Rule settings for selecting an order for the queue of a station (second tie breaker).
    /// </summary>
    public readonly QueueOrderSelectionRuleConfig QueueOrderSelectionRule3 = new QueueOrderSelectionEarliestDeadline();
    ...
    public class QueueOrderSelectionDeadlineVacant : QueueOrderSelectionRuleConfig
    {
        ...
        /// </summary>
        public readonly double VacantOrderCutoff = 600;
    }
    /// <summary>
    ...
    public class QueueOrderSelectionInboundMatches : QueueOrderSelectionRuleConfig
    {
        ...
        /// </summary>
        public readonly double DistanceForWeighting = 10;
    }
    /// <summary>
    ...
    public class QueueOrderSelectionMostLines : QueueOrderSelectionRuleConfig
    {
        ...
        /// </summary>
        public readonly bool UnitsInsteadOfLines = true;
    }
    /// <summary>
