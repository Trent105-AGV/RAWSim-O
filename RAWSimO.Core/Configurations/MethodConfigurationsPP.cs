...
public class WHCAnStarPathPlanningConfiguration : PathPlanningConfiguration
{
    ...
    /// Indicates whether the method uses a deadlock handler.
    /// </summary>
    public readonly bool UseDeadlockHandler = true;
    /// <summary>
    /// Parses the specified arguments.
    /// </summary>
    /// <param name="args">The arguments.</param>
...
public class FARPathPlanningConfiguration : PathPlanningConfiguration
{
    ...
    public override string GetMethodName()
    {
        ...
        return "ppFAR" +
               ...
               MaximumNumberOfBreakingManeuverTries +
               (NoBackEvading ? "t" : "f") +
...
public class CBSPathPlanningConfiguration : PathPlanningConfiguration
{
    ...
    public override string GetMethodName()
    {
        if (!string.IsNullOrWhiteSpace(Name)) return Name;
        var name = "ppCBS";
        name += SearchMethod switch
        {
            CBSMethod.CBSSearchMethod.BestFirst => "O",
            CBSMethod.CBSSearchMethod.DepthFirst => "D",
            CBSMethod.CBSSearchMethod.BreathFirst => "B",
...
public class PASPathPlanningConfiguration : PathPlanningConfiguration
{
    ...
    /// </summary>
    /// <returns>The name of the method.</returns>
    public override string GetMethodName() { if (!string.IsNullOrWhiteSpace(Name)) return Name; return "ppPAS" + MaxPriorities + LengthOfAWindow.ToString(IOConstants.EXPORT_FORMAT_SHORTER, IOConstants.FORMATTER); }
    /// <summary>
...
public class ODIDPathPlanningConfiguration : PathPlanningConfiguration
{
    ...
    /// </summary>
    /// <returns>The name of the method.</returns>
    public override string GetMethodName() { if (!string.IsNullOrWhiteSpace(Name)) return Name; return "ppODID" + LengthOfAWindow.ToString(IOConstants.EXPORT_FORMAT_SHORTER, IOConstants.FORMATTER) + (UseFinalReservations ? "t" : "f") + MaxNodeCountPerAgent; }
    /// <summary>
    /// The length of a time space search window
    /// </summary>
    public double LengthOfAWindow = 30.0;
    ...
    /// </summary>
    public bool UseFinalReservations;
    /// <summary>
    /// Use Final Reservations
    /// </summary>
