...
public class InventoryConfiguration
{
    ...
    /// </summary>
    public readonly bool IgnoreCapacityForBundleGeneration = false;
    ...
    /// </summary>
    public readonly double BufferBundlesUntilInventoryLoad = 1.1;
    ...
    /// </summary>
    public readonly bool DueTimePriorityMode = true;
    /// <summary>
    /// The probability for generating a priority order.
    /// </summary>
    public readonly double DueTimePriorityOrderProbability = 0.2;
    /// <summary>
    /// The due time of a priority order.
    /// </summary>
    public readonly double DueTimePriorityOrder = TimeSpan.FromMinutes(30).TotalSeconds;
    /// <summary>
    /// The due time of an ordinary order.
    /// </summary>
    public readonly double DueTimeOrdinaryOrder = TimeSpan.FromHours(2).TotalSeconds;
    /// <summary>
    ...
    /// </summary>
    public readonly double DueTimeOffsetMean = TimeSpan.FromHours(3).TotalSeconds;
    /// <summary>
    /// The standard deviation of the due time.
    /// </summary>
    public readonly double DueTimeOffsetStdDev = TimeSpan.FromMinutes(15).TotalSeconds;
    /// <summary>
    /// The minimal time the due time is set seen from the generation timestamp of the order
    /// </summary>
    public readonly double DueTimeOffsetMin = TimeSpan.FromHours(2).TotalSeconds;
    /// <summary>
    /// The maximal time the due time is set seen from the generation timestamp of the order
    /// </summary>
    public readonly double DueTimeOffsetMax = TimeSpan.FromHours(4).TotalSeconds;

    /// <summary>
    /// The configuration for generating orders and inventory in a demand based mode.
    /// </summary>
    public DemandInventoryConfiguration DemandInventoryConfiguration = new();
    ...
    /// </summary>
    public SimpleItemConfiguration SimpleItemConfiguration = new();
    ...
    public void autogenerate()
    {
        if (ItemType == ItemType.Letter && ColoredWordConfiguration == null)
        {
            var p = new DefaultConstructorIdentificationClass();
...
public class PoissonInventoryConfiguration
{
    ...
    {
        TimeDependentOrderWeights =
        [
            new()
            {
                Key = TimeSpan.FromHours(0).TotalSeconds,
                Value = PoissonGenerator.TranslateIntoRateParameter(TimeSpan.FromMinutes(60), 20)
            },
            new()
            {
                Key = TimeSpan.FromHours(1).TotalSeconds,
                Value = PoissonGenerator.TranslateIntoRateParameter(TimeSpan.FromMinutes(60), 10)
            },
            new()
            {
                Key = TimeSpan.FromHours(2).TotalSeconds,
                Value = PoissonGenerator.TranslateIntoRateParameter(TimeSpan.FromMinutes(60), 5)
            },
            new()
            {
                Key = TimeSpan.FromHours(3).TotalSeconds,
                Value = PoissonGenerator.TranslateIntoRateParameter(TimeSpan.FromMinutes(60), 5)
            },
            new()
            {
                Key = TimeSpan.FromHours(4).TotalSeconds,
                Value = PoissonGenerator.TranslateIntoRateParameter(TimeSpan.FromMinutes(60), 10)
            },
            new()
            {
                Key = TimeSpan.FromHours(5).TotalSeconds,
                Value = PoissonGenerator.TranslateIntoRateParameter(TimeSpan.FromMinutes(60), 10)
            },
            new()
            {
                Key = TimeSpan.FromHours(6).TotalSeconds,
                Value = PoissonGenerator.TranslateIntoRateParameter(TimeSpan.FromMinutes(60), 20)
            },
            new()
            {
                Key = TimeSpan.FromHours(7).TotalSeconds,
                Value = PoissonGenerator.TranslateIntoRateParameter(TimeSpan.FromMinutes(60), 20)
            },
            new()
            {
                Key = TimeSpan.FromHours(8).TotalSeconds,
                Value = PoissonGenerator.TranslateIntoRateParameter(TimeSpan.FromMinutes(60), 40)
            },
            new()
            {
                Key = TimeSpan.FromHours(9).TotalSeconds,
                Value = PoissonGenerator.TranslateIntoRateParameter(TimeSpan.FromMinutes(60), 80)
            },
            new()
            {
                Key = TimeSpan.FromHours(10).TotalSeconds,
                Value = PoissonGenerator.TranslateIntoRateParameter(TimeSpan.FromMinutes(60), 80)
            },
            new()
            {
                Key = TimeSpan.FromHours(11).TotalSeconds,
                Value = PoissonGenerator.TranslateIntoRateParameter(TimeSpan.FromMinutes(60), 90)
            },
            new()
            {
                Key = TimeSpan.FromHours(12).TotalSeconds,
                Value = PoissonGenerator.TranslateIntoRateParameter(TimeSpan.FromMinutes(60), 110)
            },
            new()
            {
                Key = TimeSpan.FromHours(13).TotalSeconds,
                Value = PoissonGenerator.TranslateIntoRateParameter(TimeSpan.FromMinutes(60), 80)
            },
            new()
            {
                Key = TimeSpan.FromHours(14).TotalSeconds,
                Value = PoissonGenerator.TranslateIntoRateParameter(TimeSpan.FromMinutes(60), 90)
            },
            new()
            {
                Key = TimeSpan.FromHours(15).TotalSeconds,
                Value = PoissonGenerator.TranslateIntoRateParameter(TimeSpan.FromMinutes(60), 130)
            },
            new()
            {
                Key = TimeSpan.FromHours(16).TotalSeconds,
                Value = PoissonGenerator.TranslateIntoRateParameter(TimeSpan.FromMinutes(60), 180)
            },
            new()
            {
                Key = TimeSpan.FromHours(17).TotalSeconds,
                Value = PoissonGenerator.TranslateIntoRateParameter(TimeSpan.FromMinutes(60), 120)
            },
            new()
            {
                Key = TimeSpan.FromHours(18).TotalSeconds,
                Value = PoissonGenerator.TranslateIntoRateParameter(TimeSpan.FromMinutes(60), 190)
            },
            new()
            {
                Key = TimeSpan.FromHours(19).TotalSeconds,
                Value = PoissonGenerator.TranslateIntoRateParameter(TimeSpan.FromMinutes(60), 250)
            },
            new()
            {
                Key = TimeSpan.FromHours(20).TotalSeconds,
                Value = PoissonGenerator.TranslateIntoRateParameter(TimeSpan.FromMinutes(60), 220)
            },
            new()
            {
                Key = TimeSpan.FromHours(21).TotalSeconds,
                Value = PoissonGenerator.TranslateIntoRateParameter(TimeSpan.FromMinutes(60), 150)
            },
            new()
            {
                Key = TimeSpan.FromHours(22).TotalSeconds,
                Value = PoissonGenerator.TranslateIntoRateParameter(TimeSpan.FromMinutes(60), 110)
            },
            new()
            {
                Key = TimeSpan.FromHours(23).TotalSeconds,
                Value = PoissonGenerator.TranslateIntoRateParameter(TimeSpan.FromMinutes(60), 50)
            }
        ];
        TimeDependentBundleWeights =
        [
            new()
            {
                Key = TimeSpan.FromHours(0).TotalSeconds,
    ...
    /// </summary>
    public readonly PoissonMode PoissonMode = PoissonMode.TimeDependent;
    /// <summary>
    /// The initial number of orders ready to be allocated.
    /// </summary>
    public readonly int InitialOrderCount = 30;
    /// <summary>
    /// The initial number of bundles ready to be allocated.
    /// </summary>
    public readonly int InitialBundleCount = 30;
    /// <summary>
    /// Can be used to set the type of information used to distort the rate parameter during simulation execution.
    /// </summary>
    public readonly PoissonDistortionType DistortOrderRateParameter = PoissonDistortionType.None;
    /// <summary>
    /// Can be used to set the type of information used to distort the rate parameter during simulation execution.
    /// </summary>
    public readonly PoissonDistortionType DistortBundleRateParameter = PoissonDistortionType.None;
    /// <summary>
    ...
    /// </summary>
    public readonly double AverageOrdersPerHourAndStation = 160;
    /// <summary>
    ...
    /// </summary>
    public readonly double AverageBundlesPerHourAndStation = 45;
    /// <summary>
    ...
    /// </summary>
    public readonly double AverageOrdersPerHourAndStationHigh = 210;
    /// <summary>
    ...
    /// </summary>
    public readonly double AverageBundlesPerHourAndStationHigh = 75;
    /// <summary>
    ...
    /// </summary>
    public readonly double LowToHighSwitchesPerHour = 0.5;
    /// <summary>
    ...
    /// </summary>
    public readonly double HighToLowSwitchesPerHour = 2;
    /// <summary>
    ...
    /// </summary>
    public readonly bool BundleGenAffectedByHighPeriod = false;
    /// <summary>
    ...
    /// </summary>
    public readonly bool OrderGenAffectedByHighPeriod = true;
    /// <summary>
    /// The time after which the rate value of the poisson process for orders loops (in inhomogeneous poisson mode).
    /// </summary>
    public readonly double MaxTimeForTimeDependentOrderRates = TimeSpan.FromDays(1).TotalSeconds;
    /// <summary>
    /// The time after which the rate value of the poisson process for bundles loops (in inhomogeneous poisson mode).
    /// </summary>
    public readonly double MaxTimeForTimeDependentBundleRates = TimeSpan.FromDays(1).TotalSeconds;
    /// <summary>
    ...
    /// </summary>
    public readonly List<Skvp<double, double>> TimeDependentOrderWeights;
    /// <summary>
    /// The weights of the rates for the bundles depending on the current time. 
    /// One element specifies the weight of a rate as a value and the time in seconds (starting at 0). 
    /// The rate weight then is used starting from the time given as the key until another KVP specifies a new rate weight.
    /// </summary>
    public readonly List<Skvp<double, double>> TimeDependentBundleWeights;
}
...
public class DemandInventoryConfiguration
{
    ...
    /// </summary>
    public readonly DemandDownPeriodConfiguration DownPeriodConfiguration = null;
}
...
public class DemandDownPeriodConfiguration
{
    ...
    public DemandDownPeriodConfiguration(DefaultConstructorIdentificationClass param) : this()
    {
        ...
        OrderDownAndUpTimes =
        [
            new() { Key = TimeSpan.FromHours(0).TotalSeconds, Value = true },
            new() { Key = TimeSpan.FromHours(6).TotalSeconds, Value = false },
            new() { Key = TimeSpan.FromHours(22).TotalSeconds, Value = true }
        ];
        BundleDownAndUpTimes =
        [
            new() { Key = TimeSpan.FromHours(0).TotalSeconds, Value = true },
            new() { Key = TimeSpan.FromHours(6).TotalSeconds, Value = false },
            new() { Key = TimeSpan.FromHours(22).TotalSeconds, Value = true }
    ...
    /// </summary>
    public readonly double MaxTimeForOrderDownAndUpPeriods = TimeSpan.FromDays(1).TotalSeconds;
    /// <summary>
    /// The time after which the timepoints for bundle down periods loop.
    /// </summary>
    public readonly double MaxTimeForBundleDownAndUpPeriods = TimeSpan.FromDays(1).TotalSeconds;
    /// <summary>
    ...
    /// </summary>
    public readonly List<Skvp<double, bool>> OrderDownAndUpTimes;
    /// <summary>
    ...
    /// If no element with timepoint 0 is given, it is assumed that generation is activated at the begin of the period.
    /// </summary>
    public readonly List<Skvp<double, bool>> BundleDownAndUpTimes;
}
...
public class BatchInventoryConfiguration
{
    ...
    public BatchInventoryConfiguration(DefaultConstructorIdentificationClass param) : this()
    {
        ...
        BundleBatches = [new() { Key = TimeSpan.FromHours(7).TotalSeconds, Value = 0.9 }];
        // Once per day have a whole bunch of orders
        OrderBatches = [new() { Key = TimeSpan.FromHours(10).TotalSeconds, Value = 400 }];
    }
    /// <summary>
