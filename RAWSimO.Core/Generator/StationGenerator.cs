...
internal class StationGenerator
{
    ...
    public void buildElevator(int row, int column, directions d, List<Waypoint> bufferPaths)
    {
        ...
        var elevator = elevatorAtThisPositionAlreadyExists ? elevatorPositions[elevatorPosition] : instance.CreateElevator(instance.RegisterElevatorID());
    ...
    public void buildStationAndBuffer(int exitRow, int exitColumn, int entryRow, int entryColumn, int stationRow, int stationColumn, waypointTypes typeOfStation, HashSet<Coordinate> coordinatesStationEntrances, int entranceCounter, int activationOrderID)
    {
        ...
        for (var segment = 0; segment < nSegments; segment++)
        {
            ...
            {
                var d = determineDirection(place, segment, lengthSegment, nSegments, rowDiffToNextTile, columnDiffToNextTile, rowDiffToNextSegment, columnDiffToNextSegment);
    ...
    {
        var shortcutDirection = buildStationAndBuffer_shortcutDirection(rowDiffToNextTile, columnDiffToNextTile, rowDiffToNextSegment, columnDiffToNextSegment);
    ...
    public int distanceFirstExitOfStationFromSideOfWarehouse(bool otherHallPresentThatIncreasesDistance, int blockSize, bool eastOrNorthHall)
    {
        ...
        var includeDistanceBetweenFirstAndSecondAisle = lc.AislesTwoDirectional ? false : (eastOrNorthHall && !lc.CounterClockwiseRingwayDirection) || (!eastOrNorthHall && lc.CounterClockwiseRingwayDirection);
        var distanceDueToAisleWidth = !lc.SingleLane && !lc.AislesTwoDirectional && ((eastOrNorthHall && !lc.CounterClockwiseRingwayDirection) || (!eastOrNorthHall && lc.CounterClockwiseRingwayDirection)) ? 1 : 0;
        return distanceDueToOtherSideHall + distanceDueToRingway + blockSize + (includeDistanceBetweenFirstAndSecondAisle ? distanceBetweenFirstAndSecondAisle : 0) + distanceDueToAisleWidth;
    }
    ...
    {
        var rowDiffEntry = 0;
        var columnDiffEntry = lc.CounterClockwiseRingwayDirection ? lc.DistanceEntryExitStation : -lc.DistanceEntryExitStation;
        var rowDiffStation = -(lc.WidthBuffer - 1);
        var columnDiffStation = 0;
        var rowFirstExit = lc.WidthBuffer - 1;
        var columnFirstExit = distanceFirstExitOfStationFromSideOfWarehouse(lc.hasStationsWest(), lc.HorizontalLengthBlock, true);
        var rowDiffExits = 0;
        ...
        var possibleLocationsForExits = possibleLocationsForExitsStations(rowFirstExit, columnFirstExit, rowDiffExits, columnDiffExits, nPossibleStations);
    ...
    public void generateStationsSouth()
    {
        ...
        var columnFirstExit = distanceFirstExitOfStationFromSideOfWarehouse(lc.hasStationsWest(), lc.HorizontalLengthBlock, false);
        var rowDiffExits = 0;
        var columnDiffExits = lc.distanceBetweenPossibleExitLocationsAtNorthOrSouthHall();
        var nPossibleStations = calculateNrPossibleStations(lc.NrVerticalAisles);
        var possibleLocationsForExits = possibleLocationsForExitsStations(rowFirstExit, columnFirstExit, rowDiffExits, columnDiffExits, nPossibleStations);
    ...
    {
        var rowDiffEntry = lc.CounterClockwiseRingwayDirection ? -lc.DistanceEntryExitStation : lc.DistanceEntryExitStation;
        var columnDiffEntry = 0;
        var rowDiffStation = 0;
        var columnDiffStation = -(lc.WidthBuffer - 1);
        var rowFirstExit = distanceFirstExitOfStationFromSideOfWarehouse(lc.hasStationsNorth(), lc.VerticalLengthBlock, false);
    ...
    public void generateStationsEast()
    {
        ...
        var rowDiffStation = 0;
        var columnDiffStation = lc.WidthBuffer - 1;
        var rowFirstExit = distanceFirstExitOfStationFromSideOfWarehouse(lc.hasStationsNorth(), lc.VerticalLengthBlock, true);
    ...
    public void generateStationsOnOneSide(int[,] possibleLocationsForExits, int nPickStation, int nReplenishmentStations, int nElevators, int rowDiffEntry, int columnDiffEntry, int rowDiffStation, int columnDiffStation, HashSet<Coordinate> coordinatesStationEntrances)
    {
        ...
        for (var whichStation = 0; whichStation < exits.GetLength(0); whichStation++)
        {
            ...
            var typeOfStation = whichStation < nPickStation ? waypointTypes.PickStation : whichStation < nPickStation + nReplenishmentStations ? waypointTypes.ReplenishmentStation : waypointTypes.Elevator;
            var activationID = 0;
            switch (typeOfStation)
            {
