...
public partial class Instance
{
    ...
    {
        var tier = theTier as Tier;
        // Remove waypoints
        var remWaypoints = Waypoints.Where(w => w.Tier == tier && xMin <= w.X && w.X <= xMax && yMin <= w.Y && w.Y <= yMax).ToList();
        // Remove all of them
        ...
        var remBots = Bots.Where(b => b.Tier == tier && xMin <= b.X && b.X <= xMax && yMin <= b.Y && b.Y <= yMax).ToList();
        foreach (var bot in remBots)
        {
            Bots.Remove(bot);
            bot.Tier.RemoveBot(bot);
        }
        var remPods = Pods.Where(p => p.Tier == tier && xMin <= p.X && p.X <= xMax && yMin <= p.Y && p.Y <= yMax).ToList();
