...
public partial class Instance
{
    ...
    /// </summary>
    internal double StatTimeStart;
    /// <summary>
    /// The current time regarding all statistical measurements.
    /// </summary>
    internal double StatTime => Controller.CurrentTime - StatTimeStart;
    ...
    /// </summary>
    internal readonly List<double> _statOrderTurnoverTimes = [];
    /// <summary>
    /// The throughput times for all completed orders.
    /// </summary>
    internal readonly List<double> _statOrderThroughputTimes = [];
    /// <summary>
    /// The lateness for all completed orders.
    /// </summary>
    internal readonly List<double> _statOrderLatenessTimes = [];
    /// <summary>
    /// The turnover times for all completed bundles.
    /// </summary>
    internal readonly List<double> _statBundleTurnoverTimes = [];
    /// <summary>
    /// The throughput times for all completed bundles.
    /// </summary>
    internal readonly List<double> _statBundleThroughputTimes = [];
    /// <summary>
    ...
    /// </summary>
    internal readonly List<ItemHandledDatapoint> _statItemHandlingTimestamps = [];
    /// <summary>
    /// Stores all times at which an bundle was handled.
    /// </summary>
    internal readonly List<BundleHandledDatapoint> _statBundleHandlingTimestamps = [];
    /// <summary>
    /// Stores all times at which an order was handled.
    /// </summary>
    internal readonly List<OrderHandledDatapoint> _statOrderHandlingTimestamps = [];
    /// <summary>
    /// Stores all times at which an incoming bundle was placed.
    /// </summary>
    internal readonly List<BundlePlacedDatapoint> _statBundlePlacementTimestamps = [];
    /// <summary>
    /// Stores all times at which a new order was placed.
    /// </summary>
    internal readonly List<OrderPlacedDatapoint> _statOrderPlacementTimestamps = [];
    /// <summary>
    /// Stores all times at which a collision happened.
    /// </summary>
    internal readonly List<CollisionDatapoint> _statCollisionTimestamps = [];
    /// <summary>
    /// Stores all completed trips to stations queueing areas.
    /// </summary>
    internal readonly List<StationTripDatapoint> _statStationTripTimestamps = [];
    /// <summary>
    ...
    protected void StatAddTrip(StationTripType tripType, double tripTime)
    {
        switch (tripType)
        {
            case StationTripType.O: _oStationTripCount++; _oStationTripTimeAvg += (tripTime - _oStationTripTimeAvg) / _oStationTripCount; break;
            case StationTripType.I: _iStationTripCount++; _iStationTripTimeAvg += (tripTime - _iStationTripTimeAvg) / _iStationTripCount; break;
    ...
    /// The number of bots that reached their targeted output station queueing area.
    /// </summary>
    public double OStationTripCount => _oStationTripCount;

    /// <summary>
    /// The average time it took a bot for their last trip towards an output station queueing area.
    /// </summary>
    public double OStationTripTimeAvg => _oStationTripTimeAvg;

    /// <summary>
    /// The number of bots that reached their targeted input station queueing area.
    /// </summary>
    public double IStationTripCount => _iStationTripCount;

    /// <summary>
    /// The number of bots that reached their targeted output station queueing area.
    /// </summary>
    public double IStationTripTimeAvg => _iStationTripTimeAvg;
    ...
    /// </summary>
    public double StatStorageFillLevel => StorageUsage / StorageCapacity;

    /// <summary>
    /// The overall storage fill level including the reservations present for the pods.
    /// </summary>
    public double StatStorageFillAndReservedLevel => (StorageUsage + StorageReserved) / StorageCapacity;
    ...
    /// </summary>
    public double StatStorageFillAndReservedAndBacklogLevel => (StorageUsage + StorageReserved + StorageBacklog) / StorageCapacity;
    ...
    /// </summary>
    public CustomControllerDatapoint StatCustomControllerInfo { get; private set; } = new();
    ...
    public void StatReset()
    {
        ...
        if (Directory.Exists(SettingConfig.StatisticsDirectory))
        {
            ...
            {
                var statFilePath = Path.Combine(SettingConfig.StatisticsDirectory, statFileName);
    ...
    public void StatFlushBundlesHandled()
    {
        ...
        switch (SettingConfig.LogFileLevel)
        {
            case Configurations.LogFileLevel.All:
                var alreadyExists = File.Exists(Path.Combine(SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.BundleProgressionRaw]));
                using (var sw = new StreamWriter(Path.Combine(SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.BundleProgressionRaw]), true))
                {
                    if (!alreadyExists)
    ...
    public void StatFlushItemsHandled()
    {
        ...
        {
            case Configurations.LogFileLevel.All:
                var alreadyExists = File.Exists(Path.Combine(SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.ItemProgressionRaw]));
                using (var sw = new StreamWriter(Path.Combine(SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.ItemProgressionRaw]), true))
                {
                    if (!alreadyExists)
    ...
    public void StatFlushOrdersHandled()
    {
        ...
        {
            case Configurations.LogFileLevel.All:
                var alreadyExists = File.Exists(Path.Combine(SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.OrderProgressionRaw]));
                using (var sw = new StreamWriter(Path.Combine(SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.OrderProgressionRaw]), true))
                {
                    if (!alreadyExists)
    ...
    public void StatFlushBundlesPlaced()
    {
        ...
        {
            case Configurations.LogFileLevel.All:
                var alreadyExists = File.Exists(Path.Combine(SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.BundlePlacementProgressionRaw]));
                using (var sw = new StreamWriter(Path.Combine(SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.BundlePlacementProgressionRaw]), true))
                {
                    if (!alreadyExists)
    ...
    public void StatFlushOrdersPlaced()
    {
        ...
        {
            case Configurations.LogFileLevel.All:
                var alreadyExists = File.Exists(Path.Combine(SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.OrderPlacementProgressionRaw]));
                using (var sw = new StreamWriter(Path.Combine(SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.OrderPlacementProgressionRaw]), true))
                {
                    if (!alreadyExists)
    ...
    private void StatFlushPathFinding()
    {
        ...
        {
            case Configurations.LogFileLevel.All:
                var alreadyExists = File.Exists(Path.Combine(SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.PathFinding]));
                using (var sw = new StreamWriter(Path.Combine(SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.PathFinding]), true))
                {
                    if (!alreadyExists)
    ...
    public void StatFlushCollisions()
    {
        ...
        {
            case Configurations.LogFileLevel.All:
                var alreadyExists = File.Exists(Path.Combine(SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.CollisionProgressionRaw]));
                using (var sw = new StreamWriter(Path.Combine(SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.CollisionProgressionRaw]), true))
                {
                    if (!alreadyExists)
    ...
    public void StatFlushTripsCompleted()
    {
        ...
        {
            case Configurations.LogFileLevel.All:
                var alreadyExists = File.Exists(Path.Combine(SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.TripsCompletedProgressionRaw]));
                using (var sw = new StreamWriter(Path.Combine(SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.TripsCompletedProgressionRaw]), true))
                {
                    if (!alreadyExists)
                        sw.WriteLine(IOConstants.COMMENT_LINE + GetHeader());
    ...
    public void StatFlushTripStatistics()
    {
        ...
        switch (SettingConfig.LogFileLevel)
        {
            case Configurations.LogFileLevel.All:
                ...
                var outputstationWPs = Waypoints.Where(wp => wp.OutputStation != null).ToList();
                var inputstationWPs = Waypoints.Where(wp => wp.InputStation != null).ToList();
                // Create files and flush data
                using (var sw = new StreamWriter(Path.Combine(SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.HeatTrips]), false))
                {
                    // Write header
                    ...
                    {
                        var datapoint = new TimeIndependentTripDataPoint
                        {
    ...
    public void StatFlushConnectionStatistics()
    {
        ...
        {
            case Configurations.LogFileLevel.All:
                using (var sw = new StreamWriter(Path.Combine(SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.ConnectionStatistics]), false))
                {
    ...
    public void WriteStatistics()
    {
        ...
        // Write footprint
        using (var sw = new StreamWriter(Path.Combine(SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.Footprint])))
            // Write stat line
        ...
        switch (SettingConfig.LogFileLevel)
        {
            case Configurations.LogFileLevel.All:
                ...
                using (var sw = new StreamWriter(Path.Combine(SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.ReadableStatistics])))
                    PrintStatistics(sw.WriteLine, detailedAll: true);
                // Write station statistics
                using (var sw = new StreamWriter(Path.Combine(SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.StationStatistics])))
                    WriteStationStatistics(sw.Write);
                // Write item descriptions statistics
                using (var sw = new StreamWriter(Path.Combine(SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.ItemDescriptionStatistics])))
        ...
        // Write instance name
        using (var sw = new StreamWriter(Path.Combine(SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.InstanceName])))
            sw.Write(string.IsNullOrWhiteSpace(Name) ? GetMetaInfoBasedInstanceName() : Name);

        // Write setting name
        using (var sw = new StreamWriter(Path.Combine(SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.SettingName])))
            sw.Write(SettingConfig.Name);

        // Write controller name
        using (var sw = new StreamWriter(Path.Combine(SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.ControllerName])))
    ...
    public void WriteStationStatistics(Action<string> writer)
    {
        ...
        foreach (var station in InputStations)
        {
            sb.AppendLine(
                ...
                station.StatNumBundlesStored + IOConstants.DELIMITER_VALUE +
                station.StatNumInjectedBundlesStored + IOConstants.DELIMITER_VALUE +
                station.StatPodsHandled + IOConstants.DELIMITER_VALUE +
        ...
        foreach (var station in OutputStations)
        {
            sb.AppendLine(
                ...
                station.X.ToString(IOConstants.EXPORT_FORMAT_SHORT, IOConstants.FORMATTER) + IOConstants.DELIMITER_VALUE +
                station.Y.ToString(IOConstants.EXPORT_FORMAT_SHORT, IOConstants.FORMATTER) + IOConstants.DELIMITER_VALUE +
                station.StatNumItemsPicked + IOConstants.DELIMITER_VALUE +
                station.StatNumInjectedItemsPicked + IOConstants.DELIMITER_VALUE +
                station.StatPodsHandled + IOConstants.DELIMITER_VALUE +
                station.StatPodHandlingTimeAvg.ToString(IOConstants.FORMATTER) + IOConstants.DELIMITER_VALUE +
                station.StatPodHandlingTimeVar.ToString(IOConstants.FORMATTER) + IOConstants.DELIMITER_VALUE +
                station.StatPodHandlingTimeMin.ToString(IOConstants.FORMATTER) + IOConstants.DELIMITER_VALUE +
    ...
    public void PrintStatistics(Action<string> writer, bool detailedAll = false, bool detailedBots = false, bool detailedPods = false, bool detailedStations = false)
    {
        ...
        if (detailedAll || detailedStations)
        {
            ...
            foreach (var iStation in InputStations)
            {
                ...
                sb.AppendLine("PodsHandled: " + iStation.StatPodsHandled);
            ...
            foreach (var oStation in OutputStations)
            {
                ...
                sb.AppendLine("PodsHandled: " + oStation.StatPodsHandled);
