...
public class RandomizerSimple : IRandomizer
{
    ...
    private bool _secondNormallyDistributedNumberIsCached;
    ...
    {
        var number = NextExponentialDouble(lambda);
    ...
    public double NextGammaDouble(double k, double theta)
    {
        ...
        if (k < 1)
            k++;
        var d = k - 1.0 / 3.0;
        var cc = 1 - Math.Sqrt(9.0 * d);
        var gammaVariate = -1.0;
        double u;
        double x = 0;
        double v = 0;
        bool generateAnotherV;
        var gammaVariateStillToBeGenerated = true;
    ...
    {
        var number = NextGammaDouble(k, theta);
        while (number < lowerBound || upperBound < number)
