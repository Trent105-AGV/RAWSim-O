...
public class Estimators
{
    ...
    {
        var travelTime = (bot.GetDistance(w) + (bot.Tier == w.Tier ? 0 : bot.Instance.WrongTierPenaltyDistance)) / bot.MaxVelocity;
        return travelTime;
    }
    ...
    {
        var travelTime = (start.GetDistance(end) + (start.Tier == end.Tier ? 0 : bot.Instance.WrongTierPenaltyDistance)) / bot.MaxVelocity;
        return travelTime;
    }
    ...
    {
        var ws = w.OutputStation;
        // Assume the wait time is 2 * length of the bot there plus the time for all items
        var waitTime = (ws.ItemTransferTime + 5 * 2 * bot.Radius) / bot.MaxVelocity * w.BotCountOverall * w.BotCountOverall; // TODO this time estimate cannot hold when transferring multiple items at once
        return waitTime;
    }
    ...
    {
        var ls = w.InputStation;
        // Assume the wait time is 2 * length of the podbot there plus the time for all bundles 
        var waitTime = (ls.ItemBundleTransferTime + 5 * 2 * bot.Radius) / bot.MaxVelocity * w.BotCountOverall * w.BotCountOverall; // TODO this estimate cannot hold when transferring many items
        return waitTime;
    }
}