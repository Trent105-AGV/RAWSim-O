...
public partial class Instance
{
    ...
    public static Instance CreateInstance(SettingConfiguration settingConfig, ControlConfiguration controlConfig)
    {
        ...
        {
            SettingConfig = settingConfig ?? new SettingConfiguration(),
            ControllerConfig = controlConfig ?? new ControlConfiguration(),
        };
        return instance;
    }
    ...
    /// </summary>
    private readonly HashSet<int> _volatileItemDescriptionIDs = [];
    /// <summary>
    ...
    {
        ItemDescription item = itemType switch
        {
            ItemType.Letter => new ColoredLetterDescription(this),
            ItemType.SimpleItem => new SimpleItemDescription(this),
    ...
    {
        // Create bundle
        ItemBundle bundle = itemDescription.Type switch
        {
            ItemType.Letter => new ColoredLetterBundle(this),
            ItemType.SimpleItem => new SimpleItemBundle(this),
            _ => throw new ArgumentException("Unknown item type: " + itemDescription.Type)
        };
    ...
    public Tier CreateTier(int id, double length, double width, double relativePositionX, double relativePositionY, double relativePositionZ)
    {
        ...
        var tier = new Tier(this, length, width)
        {
            ID = id,
            VolatileID = _tierID,
    ...
    /// </summary>
    private readonly HashSet<int> _volatileBotIDs = [];
    /// <summary>
    ...
    public Bot CreateBot(int id, Tier tier, double x, double y, double radius, double orientation, double podTransferTime, double maxAcceleration, double maxDeceleration, double maxVelocity, double turnSpeed, double collisionPenaltyTime)
    {
        ...
        // Init
        Bot bot = ControllerConfig.PathPlanningConfig.GetMethodType() switch
        {
            PathPlanningMethodType.Simple => new BotHazard(this,
                ControllerConfig.PathPlanningConfig as SimplePathPlanningConfiguration),
            PathPlanningMethodType.Dummy or PathPlanningMethodType.WHCAvStar or PathPlanningMethodType.WHCAnStar
                or PathPlanningMethodType.FAR or PathPlanningMethodType.BCP or PathPlanningMethodType.OD_ID
                or PathPlanningMethodType.CBS or PathPlanningMethodType.PAS => new BotNormal(id, this, radius,
                    podTransferTime, maxAcceleration, maxDeceleration, maxVelocity, turnSpeed, collisionPenaltyTime, x,
                    y),
            _ => throw new ArgumentException("Unknown path planning engine: " +
                                             ControllerConfig.PathPlanningConfig.GetMethodType())
        };
        // Set values
        bot.ID = id;
        bot.Tier = tier;
    ...
    /// </summary>
    private readonly HashSet<int> _volatilePodIDs = [];
    /// <summary>
    ...
    public Pod CreatePod(int id, Tier tier, double x, double y, double radius, double orientation, double capacity)
    {
        ...
        // Create the pod
        var pod = new Pod(this) { ID = id, Tier = tier, Radius = radius, X = x, Y = y, Orientation = orientation, Capacity = capacity };
        Pods.Add(pod);
        tier.AddPod(pod);
    ...
    public Pod CreatePod(int id, Tier tier, Waypoint waypoint, double radius, double orientation, double capacity)
    {
        ...
        if (SettingConfig.OverrideConfig != null && SettingConfig.OverrideConfig.OverridePodCapacity)
            capacity = SettingConfig.OverrideConfig.OverridePodCapacityValue;
        // Create the pod
        var pod = new Pod(this) { ID = id, Tier = tier, Radius = radius, X = waypoint.X, Y = waypoint.Y, Orientation = orientation, Capacity = capacity, Waypoint = waypoint };
        Pods.Add(pod);
        tier.AddPod(pod);
        _idToPods[pod.ID] = pod;
        // Set volatile ID
    ...
    {
        var elevator = new Elevator(this) { ID = id };
    ...
    /// </summary>
    private readonly HashSet<int> _volatileInputStationIDs = [];
    /// <summary>
    ...
    public InputStation CreateInputStation(int id, Tier tier, double x, double y, double radius, double capacity, double itemBundleTransfertime, int activationOrderID)
    {
        ...
        // Init
        var inputStation = new InputStation(this)
    ...
    /// </summary>
    private readonly HashSet<int> _volatileOutputStationIDs = [];
    /// <summary>
    ...
    public OutputStation CreateOutputStation(int id, Tier tier, double x, double y, double radius, int capacity, double itemTransferTime, double itemPickTime, int activationOrderID)
    {
        ...
        // Init
        var outputStation = new OutputStation(this)
    ...
    /// </summary>
    private readonly HashSet<int> _volatileWaypointIDs = [];
    /// <summary>
    ...
    {
        var wp = new Waypoint(this) { ID = id, X = station.X, Y = station.Y, Radius = station.Radius, InputStation = station, IsQueueWaypoint = isQueueWaypoint };
        station.Waypoint = wp;
        tier.AddWaypoint(wp);
        Waypoints.Add(wp);
    ...
    {
        var wp = new Waypoint(this) { ID = id, X = station.X, Y = station.Y, Radius = station.Radius, OutputStation = station, IsQueueWaypoint = isQueueWaypoint };
        station.Waypoint = wp;
        tier.AddWaypoint(wp);
        Waypoints.Add(wp);
        WaypointGraph.Add(wp);
    ...
    {
        var wp = new Waypoint(this) { ID = id, X = x, Y = y, Elevator = elevator, IsQueueWaypoint = isQueueWaypoint };
        tier.AddWaypoint(wp);
        Waypoints.Add(wp);
        WaypointGraph.Add(wp);
        _idToWaypoint[wp.ID] = wp;
    ...
    {
        var wp = new Waypoint(this) { ID = id, X = pod.X, Y = pod.Y, Radius = pod.Radius, PodStorageLocation = true, Pod = pod };
        pod.Waypoint = wp;
        tier.AddWaypoint(wp);
        Waypoints.Add(wp);
        WaypointGraph.Add(wp);
    ...
    {
        var wp = new Waypoint(this) { ID = id, Tier = tier, X = x, Y = y, PodStorageLocation = podStorageLocation, IsQueueWaypoint = isQueueWaypoint };
        tier.AddWaypoint(wp);
        Waypoints.Add(wp);
        WaypointGraph.Add(wp);
        _idToWaypoint[wp.ID] = wp;
    ...
    {
        var semaphore = new QueueSemaphore(this, maximalCount) { ID = id };
