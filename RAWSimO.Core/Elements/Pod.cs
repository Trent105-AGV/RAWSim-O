...
public class Pod : Circle, IPodInfo, IExposeVolatileID
{
    ...
    /// </summary>
    public IEnumerable<ItemBundle> RegisteredBundles => _registeredBundles;

    /// <summary>
    /// All items that are physically contained in this pod.
    /// </summary>
    private readonly HashSet<ItemDescription> _itemDescriptionsContained = [];
    /// <summary>
    /// All items that are physically contained in this pod.
    /// </summary>
    internal IEnumerable<ItemDescription> ItemDescriptionsContained => _itemDescriptionsContained;
    ...
    /// </summary>
    private readonly HashSet<ExtractRequest> _extractRequestsRegistered = [];
    /// <summary>
    ...
    public override string GetIdentfierString() { return "Pod" + ID; }
    /// <summary>
    ...
    internal double InfoTagPodSpeed
    {
        get
        {
            var value = Instance.ElementMetaInfoTracker != null && Instance.ElementMetaInfoTracker.PodSpeedMax > 0 ?
    ...
    internal double InfoTagPodUtility
    {
        get
        {
            var value = Instance.ElementMetaInfoTracker != null && Instance.ElementMetaInfoTracker.PodUtilityMax > 0 ?
    ...
    internal double InfoTagPodCombined
    {
        get
        {
            var value = Instance.ElementMetaInfoTracker?.GetPodCombinedScore(this, 1, 1) ?? 0;
            return Math.Min(value, 1);
        }
    }
        ...
        switch (Instance.SettingConfig.HeatMode)
        {
            ...
            case HeatMode.AverageFrequency:
            {
                if (_itemDescriptionCountContained == null || !Instance.ItemDescriptions.Any(item => _itemDescriptionCountContained[item] > 0))
                    return 0;
                var contained = 0; double value = 0;
            ...
            case HeatMode.CacheType:
            {
                ...
                return wp.InfoTagCache switch
                {
                    Control.Shared.ZoneType.None => 0.25,
                    Control.Shared.ZoneType.Cache => 0.75,
                    Control.Shared.ZoneType.Dropoff => 1,
                    _ => throw new ArgumentException("Unknown storage location type: " + wp.InfoTagCache)
                };
            }
