...
public class OutputStation : Circle, IUpdateable, IOutputStationInfo, IQueuesOwner, IExposeVolatileID
{
    ...
    public void Activate()
    {
        ...
        _lastActiveMeasurement = Instance.Controller?.CurrentTime ?? 0;
    }
    /// <summary>
    ...
    public void Deactivate()
    {
        ...
        if (_lastActiveMeasurement < (Instance.Controller?.CurrentTime ?? 0))
            StatActiveTime += (Instance.Controller?.CurrentTime ?? 0) - _lastActiveMeasurement;
    }
    ...
    /// </summary>
    public int CapacityInUse => _assignedOrders.Count;

    /// <summary>
    /// The amount of capacity reserved by a controller.
    /// </summary>
    internal int CapacityReserved => _registeredOrders.Count;

    /// <summary>
    /// The orders currently assigned to this station.
    /// </summary>
    private readonly HashSet<Order> _assignedOrders = [];
    /// <summary>
    /// The set of orders not yet allocated but already registered with this station.
    /// </summary>
    private readonly HashSet<Order> _registeredOrders = [];
    /// <summary>
    /// The set of orders queued for this station.
    /// </summary>
    private readonly HashSet<Order> _queuedOrders = [];

    /// <summary>
    /// The orders currently assigned to this station.
    /// </summary>
    public IEnumerable<Order> AssignedOrders => _assignedOrders;

    /// <summary>
    /// The orders currently queued to this station.
    /// </summary>
    public IEnumerable<Order> QueuedOrders => _queuedOrders;
    ...
    /// </summary>
    private readonly Queue<ExtractRequest> _requestsExtract = new();
    /// <summary>
    /// The queue of bots per item extraction request.
    /// </summary>
    private readonly Queue<Bot> _requestsBot = new();
    ...
    protected bool TakeItemFromPod(double currentTime)
    {
        ...
        while (_requestsExtract.Count > 0)
        {
            ...
            var bot = _requestsBot.Dequeue();
            var pod = bot.Pod;
            var request = _requestsExtract.Dequeue();
    ...
    /// </summary>
    private readonly HashSet<Pod> _inboundPods = [];
    /// <summary>
    ...
    /// </summary>
    internal IEnumerable<Pod> InboundPods => _inboundPods;

    /// <summary>
    /// All extract tasks that are currently carried out by robots for this station.
    /// </summary>
    private readonly HashSet<ExtractTask> _activeExtractTasks = [];
    /// <summary>
    ...
    /// </summary>
    private IEnumerable<ExtractTask> ActiveTasks => _activeExtractTasks;
    ...
    /// </summary>
    public int StatPodsHandled => _statPodHandling?.PodsHandled ?? 0;

    /// <summary>
    /// The time it took to handle one pod in average.
    /// </summary>
    public double StatPodHandlingTimeAvg => _statPodHandling?.PodHandlingTimeAvg ?? 0;

    /// <summary>
    /// The variance in the handling times of the pods.
    /// </summary>
    public double StatPodHandlingTimeVar => _statPodHandling?.PodHandlingTimeVariance ?? 0;

    /// <summary>
    /// The minimal handling time of a pod.
    /// </summary>
    public double StatPodHandlingTimeMin => _statPodHandling?.PodHandlingTimeMin ?? 0;

    /// <summary>
    /// The maximal handling time of a pod.
    /// </summary>
    public double StatPodHandlingTimeMax => _statPodHandling?.PodHandlingTimeMax ?? 0;
    ...
    /// </summary>
    public double StatItemPileOn => _statPodHandling == null ? 0 : StatNumItemsPicked / (double)_statPodHandling.PodsHandled;
    ...
    /// </summary>
    public double StatInjectedItemPileOn => _statPodHandling == null ? 0 : StatNumInjectedItemsPicked / (double)_statPodHandling.PodsHandled;

    /// <summary>
    /// The order pile-on of this station, i.e. the relative number of orders finished from the same pod in one 'transaction'.
    /// </summary>
    public double StatOrderPileOn => _statPodHandling == null ? 0 : StatNumOrdersFinished / (double)_statPodHandling.PodsHandled;
    ...
    /// </summary>
    private double _lastActiveMeasurement;
    /// <summary>
    ...
    public override string GetIdentfierString() { return "OutputStation" + ID; }
    /// <summary>
    /// Returns a simple string giving information about the object.
    /// </summary>
    /// <returns>A simple string.</returns>
    public override string ToString() { return "OutputStation" + ID; }
    ...
    private readonly List<IOrderInfo> _completedOrders = [];
    private readonly List<IOrderInfo> _openOrders = [];
    /// <summary>
    ...
    public double GetInfoBlockedLeft() { var currentTime = Instance.Controller.CurrentTime; var blockedUntil = BlockedUntil; return currentTime < blockedUntil ? blockedUntil - currentTime : double.NaN; }
    /// <summary>
    ...
    /// </summary>
    int IExposeVolatileID.VolatileID => VolatileID;
