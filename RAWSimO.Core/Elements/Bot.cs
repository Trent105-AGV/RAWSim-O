...
public abstract class Bot : Circle, IBotInfo, IUpdateable, IBotEventListener, IExposeVolatileID
{
    ...
    public double GetSpeed() { return Math.Sqrt(XVelocity * XVelocity + YVelocity * YVelocity); }
    ...
    public bool PickupPod(Pod pod, double currentTime)
    {
        ...
        if ((pod.X - X) * (pod.X - X) + (pod.Y - Y) * (pod.Y - Y) > Instance.SettingConfig.Tolerance * Instance.SettingConfig.Tolerance)
            return false;
        ...
        pod.InUse = true;
        Pod = pod;
        BlockedUntil = currentTime + PodTransferTime;
        StatNumberOfPickups++;
    ...
    public bool SetdownPod(double currentTime)
    {
        ...
        Pod.Orientation = Pod.DEFAULT_BUCKET_ORIENTATION;
    ...
    /// </summary>
    public Waypoint StatLastWaypoint;
    /// <summary>
    ...
    /// </summary>
    public Dictionary<BotTaskType, double> StatTotalTaskTimes = new(Enum.GetValues(typeof(BotTaskType)).Cast<BotTaskType>().ToDictionary(k => k, v => 0.0));
    /// <summary>
    /// The number of tasks done per type.
    /// </summary>
    public Dictionary<BotTaskType, int> StatTotalTaskCounts = new(Enum.GetValues(typeof(BotTaskType)).Cast<BotTaskType>().ToDictionary(k => k, v => 0));
    /// <summary>
    ...
    /// </summary>
    public Dictionary<BotStateType, double> StatTotalStateTimes = new(Enum.GetValues(typeof(BotStateType)).Cast<BotStateType>().ToDictionary(k => k, v => 0.0));
    /// <summary>
    /// The number of times the bot was in each state type.
    /// </summary>
    public Dictionary<BotStateType, int> StatTotalStateCounts = new(Enum.GetValues(typeof(BotStateType)).Cast<BotStateType>().ToDictionary(k => k, v => 0));
    ...
    public override string GetIdentfierString() { return "Bot" + ID; }
    /// <summary>
    /// Returns a string that can be used to identify the object.
    /// </summary>
    /// <returns>A string representing the object.</returns>
    public override string ToString() { return "Bot" + ID; }
    ...
    public bool GetInfoChanged() { var changed = _changed; _changed = false; return changed; }
    /// <summary>
