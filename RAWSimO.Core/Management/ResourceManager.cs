...
public class ResourceManager : IUpdateable
{
    ...
    public ResourceManager(Instance instance)
    {
        ...
        var usedStorageLocations = instance.Waypoints.Where(w => w.Pod != null).ToList();
        var unusedStorageLocations = instance.Waypoints.Where(w => w.PodStorageLocation).Except(usedStorageLocations).ToList();
        ...
        {
            _availableExtractRequestsPerStation[oStation] = [];
            _availableExtractRequestsPerStationQueue[oStation] = [];
        }
        foreach (var iStation in instance.InputStations)
            _availableStoreRequestsPerStation[iStation] = [];
        foreach (var pod in instance.Pods)
            _availableStoreRequestsPerPod[pod] = [];
    ...
    /// </summary>
    private readonly Dictionary<Pod, Bot> _usedPods;

    /// <summary>
    /// All pods currently not used.
    /// </summary>
    private readonly HashSet<Pod> _unusedPods;

    /// <summary>
    /// All pods currently reserved by bots.
    /// </summary>
    public IEnumerable<Pod> UsedPods => _usedPods.Keys;

    /// <summary>
    /// All pods currently not used.
    /// </summary>
    public IEnumerable<Pod> UnusedPods => _unusedPods;
    ...
    {
        // Sanity check
        if (_usedPods.TryGetValue(pod, out var value) && value != bot)
    ...
    /// </summary>
    private readonly HashSet<Waypoint> _unusedPodStorageLocations = [];

    /// <summary>
    /// All pod storage locations currently in use.
    /// </summary>
    private readonly HashSet<Waypoint> _usedPodStorageLocations = [];

    /// <summary>
    /// All pod storage locations currently not in use.
    /// </summary>
    public IEnumerable<Waypoint> UnusedPodStorageLocations => _unusedPodStorageLocations;

    /// <summary>
    /// All pod storage locations currently in use.
    /// </summary>
    public IEnumerable<Waypoint> UsedPodStorageLocations => _usedPodStorageLocations;
    ...
    {
        var successfulOperation = _usedPodStorageLocations.Add(storageLocation);
    ...
    {
        var successfulOperation = _unusedPodStorageLocations.Add(storageLocation);
    ...
    public IEnumerable<Waypoint> UnusedRestingLocations =>
        ...
        _unusedPodStorageLocations.Except(_forbiddenRestingLocations);

    /// <summary>
    /// Locations that cannot be used for resting.
    /// </summary>
    private readonly HashSet<Waypoint> _forbiddenRestingLocations = [];
    /// <summary>
    ...
    {
        var successfulOperation = _usedPodStorageLocations.Add(waypoint);
    ...
    {
        var successfulOperation = _unusedPodStorageLocations.Add(waypoint);
    ...
    /// </summary>
    private readonly HashSet<InsertRequest> _availableStoreRequests = [];

    /// <summary>
    /// All requests to store a bundle per input station they are allocated to.
    /// </summary>
    private readonly Dictionary<InputStation, HashSet<InsertRequest>> _availableStoreRequestsPerStation = new();

    /// <summary>
    /// All requests to store a bundle per pod they are allocated to.
    /// </summary>
    private readonly Dictionary<Pod, HashSet<InsertRequest>> _availableStoreRequestsPerPod = new();

    /// <summary>
    /// All available requests for storing an item bundle.
    /// </summary>
    public IEnumerable<InsertRequest> AvailableStoreRequests => _availableStoreRequests;
    ...
    {
        var request = new InsertRequest(item, inputStation, pod);
        _availableStoreRequests.Add(request);
        if (request.Station != null)
        {
            _availableStoreRequestsPerStation[request.Station].Add(request);
    ...
    /// </summary>
    private readonly HashSet<ExtractRequest> _availableExtractRequests = [];
    /// <summary>
    /// All available requests to extract an item per order they belong to.
    /// </summary>
    private readonly Dictionary<Order, HashSet<ExtractRequest>> _availableExtractRequestsPerOrder = new();
    /// <summary>
    /// All requests to extract an item per output station they are allocated to.
    /// </summary>
    private readonly Dictionary<OutputStation, HashSet<ExtractRequest>> _availableExtractRequestsPerStation = new();
    /// <summary>
    /// All requests to extract an item that have been queued for the respective stations.
    /// </summary>
    private readonly Dictionary<OutputStation, HashSet<ExtractRequest>> _availableExtractRequestsPerStationQueue = new();
    /// <summary>
    /// The respective stations an extract request was queued for, if it was queued.
    /// </summary>
    private readonly Dictionary<ExtractRequest, OutputStation> _stationQueuedPerExtractRequest = new();
    ...
    public void CreateExtractRequests(Order order)
    {
        ...
        foreach (var l in order.Positions)
            ...
            {
                var request = new ExtractRequest(l.Key, order, null);
...
public class ExtractRequest
{
    ...
    public override string ToString() { return Station.ToString() + IOConstants.DELIMITER_TUPLE + Order + IOConstants.DELIMITER_TUPLE + Item; }
}
...
public class InsertRequest
{
    ...
    /// </summary>
    public ItemBundle Bundle { get; }
    /// <summary>
    /// The station the bundle is located.
    /// </summary>
    public InputStation Station { get; }
    /// <summary>
    ...
    /// </summary>
    /// <returns>A simple string.</returns>
    public override string ToString() { return Station.ToString() + IOConstants.DELIMITER_TUPLE + Bundle; }
}