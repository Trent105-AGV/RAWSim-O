...
internal class MetaInformationManager
{
    ...
    /// </summary>
    public readonly ShortestPathManager ShortestPathManager;
    /// <summary>
    /// Keeps track of shortest paths from one waypoint to another.
    /// </summary>
    public readonly TimeEfficientPathManager TimeEfficientPathManager;
}
...
internal class ShortestPathManager
{
    ...
    /// </summary>
    private readonly MultiKeyDictionary<Waypoint, Waypoint, bool, double> _shortestPaths = new();
    ...
    private double CalculateShortestPath(Waypoint startNode, Waypoint destinationNode, double wrongTierPenalty, bool emulatePodCarrying)
    {
        ...
        var openLocations = new Dictionary<Waypoint, WaypointSearchDatapoint>();
        var closedLocations = new Dictionary<Waypoint, WaypointSearchDatapoint>();
        ...
        while (true)
        {
            ...
            var currentNodeKVP = openLocations.ArgMin(w => w.Value.DistanceTraveled + w.Value.DistanceToGoal);
            ...
            if (currentNodeKVP.Equals(default(KeyValuePair<Waypoint, WaypointSearchDatapoint>)))
                return double.PositiveInfinity;
            var currentNode = currentNodeKVP.Key;

            // Grab the details about the current waypoint
            var currentNodeData = currentNodeKVP.Value;
...
internal class TimeEfficientPathManager
{
    ...
    public class TimeGraph
    {
        ...
        /// </summary>
        public readonly double TurnSpeed;
        /// <summary>
        /// All generated time-waypoints.
        /// </summary>
        public readonly HashSet<TimeWaypoint> TimeWaypoints = [];
        /// <summary>
        /// The time-waypoints of all original waypoints.
        /// </summary>
        public readonly Dictionary<Waypoint, HashSet<TimeWaypoint>> TimeWaypointsOfOriginalWaypoints = new();
        /// <summary>
        /// The time-waypoints reflecting specific connections.
        /// </summary>
        public readonly MultiKeyDictionary<Waypoint, Waypoint, TimeWaypoint> TimeWaypointsOfConnections = new();
        /// <summary>
        ...
        public TimeGraph(Instance instance)
        {
            ...
            foreach (var to in from.Paths)
            {
                ...
                if (!TimeWaypointsOfOriginalWaypoints.ContainsKey(to))
                    TimeWaypointsOfOriginalWaypoints[to] = [];
            ...
            foreach (var timeWPFrom in TimeWaypoints)
            {
                ...
                {
                    var timeWPTo = TimeWaypointsOfConnections[timeWPFrom.OriginalWaypoint, connectedWP];
    ...
    public class TimeWaypoint
    {
        ...
        /// </summary>
        public readonly List<Tuple<TimeWaypoint, double>> Edges = [];
        /// <summary>
    ...
    private double EstimateTravelTime(TimeWaypoint from, Waypoint to)
    {
        ...
        // --> Calculate time needed for driving the given distance
        var travelTime = Distances.CalculateEuclid(from.OriginalWaypoint, to, _instance.WrongTierPenaltyDistance) / _timeGraph.Speed;
    ...
    private double CalculateShortestPath(Waypoint start, Waypoint destination, double wrongTierPenalty, bool emulatePodCarrying)
    {
        ...
        {
            var openLocations = new Dictionary<TimeWaypoint, TimeWaypointSearchDatapoint>();
            var closedLocations = new Dictionary<TimeWaypoint, TimeWaypointSearchDatapoint>();
            openLocations[startNode] = new TimeWaypointSearchDatapoint(0.0, EstimateTravelTime(startNode, destination), startNode, null, 0);

            // Loop until end is found
            while (true)
            {
                // Find the lowest cost waypoint in openLocations
                var currentNodeKVP = openLocations.ArgMin(w => w.Value.TimeTraveled + w.Value.TimeToGoal);
                // Something wrong happened -can't find the end
                ...
                foreach (var edge in currentNode.Edges)
                {
                    ...
                    {
                        // It's already in the open list, but see if this new path is better
                        var oldPath = openLocations[edge.Item1];
