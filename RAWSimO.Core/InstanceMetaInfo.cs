...
public partial class Instance : IInstanceInfo
{
    ...
    /// </summary>
    public readonly WaypointGraph WaypointGraph = new();
    ...
    /// </summary>
    internal SharedControlElementsContainer SharedControlElements { get; }
        ...
        get
        {
            if (_updateables == null)
            {
                ...
                [
                    ...
                    Controller.BundleManager,
                    Controller.Allocator,
                    Controller.BotManager,
                    Controller.StationManager,
                    Controller.PodStorageManager,
                    Controller.RepositioningManager,
    ...
    /// </summary>
    private readonly string _loggingColumnSep = "|";
    /// <summary>
    ...
    {
        return
            (Controller.BotManager as BalancedBotManager)?.GetAssignedBotCount(station) ?? ((Controller.BotManager as ConstantRatioBotManager)?.GetAssignedBotCount(station) ?? 0);
    }
    /// <summary>
    /// Gets the number of robots currently assigned to the given station. Does only work for the balanced bot manager. For others it will only return 0.
    /// </summary>
    ...
    public double WrongTierPenaltyDistance
    {
        get
        {
            if (double.IsNaN(field)) { field = Compound.Tiers.Max(t => t.Length + t.Width) + 1; }
            return field;
        }
    } = double.NaN;
    ...
    public bool GetInfoChanged() { var was = Changed; Changed = false; return was; }
    /// <summary>
    ...
    /// </summary>
    private readonly Dictionary<int, Bot> _idToBots = new();
    ...
    /// Stores the corresponding elements by their ID.
    /// </summary>
    private readonly Dictionary<int, Pod> _idToPods = new();
    ...
    /// Stores the corresponding elements by their ID.
    /// </summary>
    private readonly Dictionary<int, InputStation> _idToInputStations = new();
    ...
    /// Stores the corresponding elements by their ID.
    /// </summary>
    private readonly Dictionary<int, OutputStation> _idToOutputStations = new();
    ...
    /// Stores the corresponding elements by their ID.
    /// </summary>
    private readonly Dictionary<int, Elevator> _idToElevators = new();
    ...
    /// Stores the corresponding elements by their ID.
    /// </summary>
    private readonly Dictionary<int, Tier> _idToTier = new();
    ...
    /// Stores the corresponding elements by their ID.
    /// </summary>
    private readonly Dictionary<int, Waypoint> _idToWaypoint = new();
    ...
    /// Stores the corresponding elements by their ID.
    /// </summary>
    private readonly Dictionary<int, QueueSemaphore> _idToSemaphore = new();
    ...
    /// Stores the corresponding elements by their ID.
    /// </summary>
    private readonly Dictionary<int, ItemDescription> _idToItemDescription = new();
