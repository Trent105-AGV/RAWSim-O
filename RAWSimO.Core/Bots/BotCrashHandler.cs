...
internal class BotCrashHandler : IUpdateable
{
    ...
    /// </summary>
    private readonly HashSet<Bot> _potentialCrashBots = [];
    /// <summary>
    ...
    {
        if (_potentialCrashBots.Count == 0) return;
        ...
        foreach (var crashBot in _potentialCrashBots)
        {
            ...
            if (!crashBot.Tier.BotQuadTree.IsCollision(crashBot) &&
                // Check for collisions with the tier boundaries
                !(crashBot.X - crashBot.Radius < 0) && !(crashBot.X + crashBot.Radius > crashBot.Tier.Length) &&
                !(crashBot.Y - crashBot.Radius < 0) && !(crashBot.Y + crashBot.Radius > crashBot.Tier.Width) &&
                // Check for collisions with other pods
                (crashBot.Pod == null || !crashBot.Pod.Tier.PodQuadTree.IsCollision(crashBot.Pod))) continue;
            // Remember that we have a collision
            anyCollision = true;
            // ---> Debug: Log info about the crash pilot
            _instance.LogSevere("Bot" + crashBot.ID + " crashed! Information:");
            _instance.LogSevere(
            ...
            for (var i = 2; i < 5; i++)
            {
                _instance.LogSevere("Potential crash partners (bots) within " + i + " * radius:");
                ...
                _instance.LogSevere("Potential crash partners (pods) within " + i + " * radius:");
        ...
        {
            // Save instance
            var debugInstanceFile = System.IO.Path.Combine(_instance.SettingConfig.StatisticsDirectory, "debug.xinst");
