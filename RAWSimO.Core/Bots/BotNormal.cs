...
public class BotNormal : Bot
{
    ...
    /// </summary>
    internal override Waypoint TargetWaypoint => _destinationWaypoint ?? _currentWaypoint;
    ...
    /// The state queue
    /// </summary>
    private readonly Queue<IBotState> _stateQueue = new();
    /// <summary>
    ...
    private IBotState StateQueueDequeue() { var state = _stateQueue.Dequeue(); _currentInfoStateName = _stateQueue.Any() ? _stateQueue.Peek().ToString() : ""; return state; }
    /// <summary>
    ...
    /// </summary>
    private int StateQueueCount => _stateQueue.Count;
    ...
    /// rotate until
    /// </summary>
    private double _startOrientation;

    /// <summary>
    /// rotate until
    /// </summary>
    private double _endOrientation;

    /// <summary>
    /// The agent reached the next way point
    /// </summary>
    private bool _eventReachedNextWaypoint;

    /// <summary>
    /// Indicates whether the first position info was received from the remote server.
    /// </summary>
    private bool _initialEventReceived;
    ...
    public Path Path
    {
        ...
        internal set
        {
            ...
            if (Instance.SettingConfig.VisualizationAttached && field != null)
                _currentPath = field.Actions.Select(a => Instance.Controller.PathManager.GetWaypointByNodeId(a.Node))
                    .Cast<IWaypointInfo>().ToList();
        }
    }
    ...
    public override void AssignTask(BotTask t)
    {
        ...
        switch (t.Type)
        {
            ...
            case BotTaskType.RepositionPod:
                ...
                var repositionPodTask = t as RepositionPodTask;
    ...
    {
        var dequeuedState = StateQueueDequeue();
    ...
    private bool IsInStationQueueZone(OutputStation station)
    {
        if (_queueZonesOStations == null)
            ...
            {
                var lowX = s.Queues != null && s.Queues.Any() && s.Queues.First().Value.Any() ? s.Queues.Min(q => q.Value.Min(w => w.X)) : s.X - 0.5;
                var highX = s.Queues != null && s.Queues.Any() && s.Queues.First().Value.Any() ? s.Queues.Max(q => q.Value.Max(w => w.X)) : s.X + 0.5;
                var lowY = s.Queues != null && s.Queues.Any() && s.Queues.First().Value.Any() ? s.Queues.Min(q => q.Value.Min(w => w.Y)) : s.Y - 0.5;
                var highY = s.Queues != null && s.Queues.Any() && s.Queues.First().Value.Any() ? s.Queues.Max(q => q.Value.Max(w => w.Y)) : s.Y + 0.5;
        ...
        else
        {
            ...
            if (_endOrientation != Orientation)
            {
                ...
                if (Pod != null && Instance.SettingConfig.RotatePods)
                    Pod.Orientation = _endOrientation;
            }
    ...
    {
        var destinationWP = DestinationWaypoint;
    ...
    {
        var currentTime = Instance.Controller.CurrentTime; var waitUntil = _waitUntil; var blockedUntil = BlockedUntil;
    ...
    internal class BotMove : IBotState
    {
        ...
        public void Act(Bot self, double lastTime, double currentTime)
        {
            ...
            {
                // --> Not reached yet
                var successfulRegistration = bot.setNextWaypoint(bot.Instance.Controller.PathManager.GetWaypointByNodeId(bot.Path.NextAction.Node), currentTime);
            ...
            else
            {
                ...
                if (bot.Path.NextAction == bot.Path.LastAction && bot.Path.NextNodeToPrepareFor >= 0)
                {
                    ...
                    var turnTowards = bot.Instance.Controller.PathManager.GetWaypointByNodeId(bot.Path.NextNodeToPrepareFor);
                    bot._endOrientation = GetOrientation(bot.X, bot.Y, turnTowards.X, turnTowards.Y);
                    var rotateDuration = bot.Physics.getTimeNeededToTurn(bot._startOrientation, bot._endOrientation);
    ...
    internal class BotPutItems : IBotState
    {
        ...
        { _extractTask = extractTask; DestinationWaypoint = extractTask.OutputStation.Waypoint; }
    ...
    internal class UseElevator : IBotState
    {
        ...
        private readonly Waypoint _waypointFrom;
        private readonly Waypoint _waypointTo;
        private bool _initialized;
        private bool inUse;
        private double travelUntil;
        public Waypoint DestinationWaypoint => _waypointTo;
        ...
        /// State type.
        /// </summary>
        public BotStateType Type => BotStateType.UseElevator;
    }
    #endregion
    ...
    public override void OnReachedWaypoint(Waypoint waypoint)
    {
        ...
        // Logging info message
        Instance.LogInfo("Bot" + ID + " is at: " + waypoint.ID);
