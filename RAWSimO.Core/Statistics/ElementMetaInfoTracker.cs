...
public class ElementMetaInfoTracker
{
    ...
    private void EnsureInit()
    {
        if (_podsContainingItems == null)
        {
            ...
            for (var i = 0; i < StorageLocationsOrdered.Count; i++)
            ...
            var currentRank = 1; var currentProminenceValue = _storageLocationProminence[StorageLocationsOrdered.First()];
            ...
            _podUtility = new VolatileIDDictionary<Pod, double>(
                _instance.Pods.Select(p => new VolatileKeyValuePair<Pod, double>(p, p.ItemDescriptionsContained.Sum(i => Math.Min(p.CountContained(i), _instance.StockInfo.GetCurrentDemand(i)))))
                    .ToList());
            var bestUtility = _podUtility.ArgMax(pu => pu.Value);
            ...
            _podSpeed = new VolatileIDDictionary<Pod, double>(
                _instance.Pods.Select(p => new VolatileKeyValuePair<Pod, double>(p, p.ItemDescriptionsContained.Sum(i => p.CountContained(i) * _instance.FrequencyTracker.GetStaticFrequency(i))))
                    .ToList());
            var bestSpeed = _podSpeed.ArgMax(ps => ps.Value);
    ...
    public double GetPodCombinedScore(Pod pod, double weightSpeed, double weightUtility)
    {
        ...
        // Ensure init
        EnsureInit();
        var combinedWeight = weightSpeed + weightUtility;
        var normalizedPodSpeed = PodSpeedMax > 0 ? _podSpeed[pod] / PodSpeedMax : 0;
        var normalizedPodUtility = PodUtilityMax > 0 ? _podUtility[pod] / PodUtilityMax : 0;
        return
        ...
        out int invUtilityTotal, out int invUtilityRank, out double invUtilityAvgRank)
    {
        ...
        for (var index1 = 0; index1 < StorageLocationsOrdered.Count; index1++)
        {
            for (var index2 = index1 + 1; index2 < StorageLocationsOrdered.Count; index2++)
            {
    ...
    private void BundleStored(InputStation iStation, Bot bot, Pod pod, ItemBundle bundle)
    {
        ...
        // --> Update pod utility
        var itemDemand = _instance.StockInfo.GetCurrentDemand(bundle.ItemDescription);
        var beforeCount = pod.CountContained(bundle.ItemDescription) - bundle.ItemCount;
    ...
    private void ItemExtracted(Pod pod, ItemDescription item)
    {
        ...
        if (_podsContainingItems == null) return;
        // --> Update pod utility
        var itemDemand = _instance.StockInfo.GetCurrentDemand(item);
        var updateUtilityMax = false;
    ...
    private void NewOrder(Order order)
    {
        ...
        {
            var itemDemand = _instance.StockInfo.GetCurrentDemand(line.Key);
            foreach (var itemPod in _podsContainingItems[line.Key])
            {
                var beforeDemand = itemDemand - line.Value;
                var contained = itemPod.CountContained(line.Key);
