...
public class SimulationObserver : IUpdateable
{
    ...
    /// </summary>
    private readonly List<DistanceDatapoint> _logDistanceTraveled = [];
    ...
    private void FlushTraveledDistance()
    {
        ...
        switch (_instance.SettingConfig.LogFileLevel)
        {
            case Configurations.LogFileLevel.All:
                var alreadyExists = File.Exists(Path.Combine(_instance.SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.TraveledDistanceProgressionRaw]));
                using (var sw = new StreamWriter(Path.Combine(_instance.SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.TraveledDistanceProgressionRaw]), true))
                {
                    if (!alreadyExists)
    ...
    /// </summary>
    private readonly List<PerformanceDatapoint> _logPerformancePolling = [];
    ...
    private void FlushPerformancePolls()
    {
        ...
        {
            case Configurations.LogFileLevel.All:
                var alreadyExists = File.Exists(Path.Combine(_instance.SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.PerformancePollingRaw]));
                using (var sw = new StreamWriter(Path.Combine(_instance.SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.PerformancePollingRaw]), true))
                {
                    if (!alreadyExists)
    ...
    private void FlushLocationsPolled()
    {
        ...
        {
            case Configurations.LogFileLevel.All:
                var alreadyExists = File.Exists(Path.Combine(_instance.SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.HeatLocationPolling]));
                using (var sw = new StreamWriter(Path.Combine(_instance.SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.HeatLocationPolling]), true))
                {
                    if (!alreadyExists)
    ...
    /// </summary>
    private readonly List<StorageLocationInfoDatapoint> _logStorageLocationInfoPolling = [];
    ...
    private void FlushStorageLocationInfoPolled()
    {
        ...
        {
            case Configurations.LogFileLevel.All:
                var alreadyExists = File.Exists(Path.Combine(_instance.SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.HeatStorageLocationPolling]));
                using (var sw = new StreamWriter(Path.Combine(_instance.SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.HeatStorageLocationPolling]), true))
                {
                    if (!alreadyExists)
    ...
    /// </summary>
    private readonly List<BotDatapoint> _logBotInfoPolling = [];
    ...
    private void FlushBotInfoPolled()
    {
        ...
        {
            case Configurations.LogFileLevel.All:
                var alreadyExists = File.Exists(Path.Combine(_instance.SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.BotInfoPollingRaw]));
                using (var sw = new StreamWriter(Path.Combine(_instance.SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.BotInfoPollingRaw]), true))
                {
                    if (!alreadyExists)
    ...
    /// </summary>
    private readonly List<InventoryLevelDatapoint> _logInventoryLevelPolling = [];
    internal IEnumerable<InventoryLevelDatapoint> InventoryLevelLog => _logInventoryLevelPolling;
    ...
    private void FlushInventoryLevelPolled()
    {
        ...
        {
            case Configurations.LogFileLevel.All:
                var alreadyExists = File.Exists(Path.Combine(_instance.SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.InventoryLevelPollingRaw]));
                using (var sw = new StreamWriter(Path.Combine(_instance.SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.InventoryLevelPollingRaw]), true))
                {
                    if (!alreadyExists)
    ...
    /// </summary>
    private readonly List<BundleOrderSituationDatapoint> _logBundleOrderSituationPolling = [];

    /// <summary>
    /// Log of the polled backlog level over time.
    /// </summary>
    public IEnumerable<BundleOrderSituationDatapoint> BundleOrderSituationLog => _logBundleOrderSituationPolling;
    ...
    private void FlushBundleOrderSituationPolled()
    {
        ...
        {
            case Configurations.LogFileLevel.All:
                var alreadyExists = File.Exists(Path.Combine(_instance.SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.BundleOrderSituationPollingRaw]));
                using (var sw = new StreamWriter(Path.Combine(_instance.SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.BundleOrderSituationPollingRaw]), true))
                {
                    if (!alreadyExists)
    ...
    /// </summary>
    private readonly List<StationDataPoint> _logPodHandlingPolling = [];
    /// <summary>
    /// Log of the polled pod handling over time.
    /// </summary>
    public IEnumerable<StationDataPoint> PodHandlingLog => _logPodHandlingPolling;
    ...
    private void FlushPodHandlingPolled()
    {
        ...
        {
            case Configurations.LogFileLevel.All:
                var alreadyExists = File.Exists(Path.Combine(_instance.SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.StationPollingRaw]));
                using (var sw = new StreamWriter(Path.Combine(_instance.SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.StationPollingRaw]), true))
                {
                    if (!alreadyExists)
    ...
    /// </summary>
    private readonly Dictionary<Waypoint, int> _wellSortednessLastOStationTripCount = new();
    /// <summary>
    /// Log of the polled well sortedness over time.
    /// </summary>
    private readonly List<WellSortednessDatapoint> _logWellSortednessPolling = [];
    /// <summary>
    /// Log of the polled well sortedness over time.
    /// </summary>
    public IEnumerable<WellSortednessDatapoint> WellSortednessLog => _logWellSortednessPolling;
    ...
    private void FlushWellSortednessPolled()
    {
        ...
        {
            case Configurations.LogFileLevel.All:
                var alreadyExists = File.Exists(Path.Combine(_instance.SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.WellSortednessPollingRaw]));
                using (var sw = new StreamWriter(Path.Combine(_instance.SettingConfig.StatisticsDirectory, IOConstants.StatFileNames[IOConstants.StatFile.WellSortednessPollingRaw]), true))
                {
                    if (!alreadyExists)
    ...
    /// </summary>
    private double _timingDecisionsOverall;
    /// <summary>
    /// The overall time spent in computing paths.
    /// </summary>
    private double _timingPathPlanningOverall;
    /// <summary>
    /// The number of times path planning was called for a decision.
    /// </summary>
    private int _timingPathPlanningCount;
    /// <summary>
    /// The overall time spent in deciding the tasks for the bots.
    /// </summary>
    private double _timingTaskAllocationOverall;
    /// <summary>
    /// The number of times task allocation was called for a decision.
    /// </summary>
    private int _timingTaskAllocationCount;
    /// <summary>
    /// The overall time spent in deciding the pod for an item.
    /// </summary>
    private double _timingItemStorageOverall;
    /// <summary>
    /// The number of times item storage assignment was called for a decision.
    /// </summary>
    private int _timingItemStorageCount;
    /// <summary>
    /// The overall time spent in deciding the position for a pod.
    /// </summary>
    private double _timingPodStorageOverall;
    /// <summary>
    /// The number of times pod storage assignment was called for a decision.
    /// </summary>
    private int _timingPodStorageCount;
    /// <summary>
    /// The overall time spent in deciding about relocations.
    /// </summary>
    private double _timingRepositioningOverall;
    /// <summary>
    /// The number of times the repositioning manager was called for a decision.
    /// </summary>
    private int _timingRepositioningCount;
    /// <summary>
    /// The overall time spent in deciding the station for an item-bundle.
    /// </summary>
    private double _timingReplenishmentBatchingOverall;
    /// <summary>
    /// The number of times replenishment batching was called for a decision.
    /// </summary>
    private int _timingReplenishmentBatchingCount;
    /// <summary>
    /// The overall time spent in deciding the station for an order.
    /// </summary>
    private double _timingOrderBatchingOverall;
    /// <summary>
    /// The number of times order batching was called for a decision.
    /// </summary>
    private int _timingOrderBatchingCount;

    /// <summary>
    /// The overall time spent deciding / planning.
    /// </summary>
    public double TimingDecisionsOverall => _timingDecisionsOverall;

    /// <summary>
    /// The overall time spent in path planning.
    /// </summary>
    public double TimingPathPlanningOverall => _timingPathPlanningOverall;

    /// <summary>
    /// The average time consumed for making a single decision.
    /// </summary>
    public double TimingPathPlanningAverage => _timingPathPlanningCount == 0 ? 0 : _timingPathPlanningOverall / _timingPathPlanningCount;

    /// <summary>
    /// The number of decisions done for path planning.
    /// </summary>
    public int TimingPathPlanningDecisionCount => _timingPathPlanningCount;

    /// <summary>
    /// The overall time spent in task allocation.
    /// </summary>
    public double TimingTaskAllocationOverall => _timingTaskAllocationOverall;

    /// <summary>
    /// The average time consumed for making a single decision.
    /// </summary>
    public double TimingTaskAllocationAverage => _timingTaskAllocationCount == 0 ? 0 : _timingTaskAllocationOverall / _timingTaskAllocationCount;

    /// <summary>
    /// The number of decisions done for task allocation.
    /// </summary>
    public int TimingTaskAllocationDecisionCount => _timingTaskAllocationCount;

    /// <summary>
    /// The overall time spent in item storage.
    /// </summary>
    public double TimingItemStorageOverall => _timingItemStorageOverall;

    /// <summary>
    /// The average time consumed for making a single decision.
    /// </summary>
    public double TimingItemStorageAverage => _timingItemStorageCount == 0 ? 0 : _timingItemStorageOverall / _timingItemStorageCount;

    /// <summary>
    /// The number of decisions done for item storage.
    /// </summary>
    public int TimingItemStorageDecisionCount => _timingItemStorageCount;

    /// <summary>
    /// The overall time spent in pod storage.
    /// </summary>
    public double TimingPodStorageOverall => _timingPodStorageOverall;

    /// <summary>
    /// The average time consumed for making a single decision.
    /// </summary>
    public double TimingPodStorageAverage => _timingPodStorageCount == 0 ? 0 : _timingPodStorageOverall / _timingPodStorageCount;

    /// <summary>
    /// The number of decisions done for pod storage.
    /// </summary>
    public int TimingPodStorageDecisionCount => _timingPodStorageCount;

    /// <summary>
    /// The overall time spent in repositioning.
    /// </summary>
    public double TimingRepositioningOverall => _timingRepositioningOverall;

    /// <summary>
    /// The average time consumed for making a single decision.
    /// </summary>
    public double TimingRepositioningAverage => _timingRepositioningCount == 0 ? 0 : _timingRepositioningOverall / _timingRepositioningCount;

    /// <summary>
    /// The number of decisions done for repositioning.
    /// </summary>
    public int TimingRepositioningDecisionCount => _timingRepositioningCount;

    /// <summary>
    /// The overall time spent in replenishment batching.
    /// </summary>
    public double TimingReplenishmentBatchingOverall => _timingReplenishmentBatchingOverall;

    /// <summary>
    /// The average time consumed for making a single decision.
    /// </summary>
    public double TimingReplenishmentBatchingAverage => _timingReplenishmentBatchingCount == 0 ? 0 : _timingReplenishmentBatchingOverall / _timingReplenishmentBatchingCount;

    /// <summary>
    /// The number of decisions done for replenishment batching.
    /// </summary>
    public int TimingReplenishmentBatchingDecisionCount => _timingReplenishmentBatchingCount;

    /// <summary>
    /// The overall time spent in order batching.
    /// </summary>
    public double TimingOrderBatchingOverall => _timingOrderBatchingOverall;

    /// <summary>
    /// The average time consumed for making a single decision.
    /// </summary>
    public double TimingOrderBatchingAverage => _timingOrderBatchingCount == 0 ? 0 : _timingOrderBatchingOverall / _timingOrderBatchingCount;

    /// <summary>
    /// The number of decisions done for order batching.
    /// </summary>
    public int TimingOrderBatchingDecisionCount => _timingOrderBatchingCount;
    ...
    public void Update(double lastTime, double currentTime)
    {
        ...
        if (currentTime >= _nextSnapshotDistanceTraveled)
        {
            ...
            var newOverallTraveledDistance = _instance.StatOverallDistanceTraveled;
        ...
        if (currentTime >= _nextSnapshotInventoryLevelPolling)
        {
            ...
            var level = _instance.Pods.Sum(b => b.CapacityInUse) / _maxInventoryCapacity;
            // Get inversions
        ...
        if (currentTime >= _nextSnapshotBundleOrderSituationPolling)
        {
            ...
            var activeBundles = _instance.InputStations.SelectMany(s => s.ItemBundles);
            var activeOrders = _instance.OutputStations.SelectMany(s => s.AssignedOrders);
            // Add current situation
        ...
        if (currentTime >= _nextSnapshotWellSortednessPolling)
        {
            ...
            _nextSnapshotWellSortednessPolling += STEP_LENGTH_WELL_SORTEDNESS_POLL;
            // Add current situation
            var currentTuples = new List<WellSortednessPathTimeTuple>();
            foreach (var pathTime in _instance.Waypoints
                         ...
                         .OrderBy(g => g.Key))
            {
                var podCount = pathTime.Count(wp => wp.Pod != null);
                var storageLocationCount = pathTime.Count();
                var skuFrequencies = pathTime.Select(wp =>
                ...
                var podSpeeds = pathTime.Select(wp =>
                    ...
                    wp.Pod != null ?
                        ...
                        // Use 0 as the default value
                        0.0);
                var podUtilities = pathTime.Select(wp =>
                    // Check whether there is a pod at the location
                    wp.Pod != null ?
                        // Get the speed score of the pod
                        ...
                        // Use 0 as the default value
                        0.0);
                var podScores = pathTime.Select(wp =>
                    // Check whether there is a pod at the location
                    wp.Pod != null ?
                        // Get the speed score of the pod
        ...
        if (currentTime >= _nextSnapshotPerformancePolling)
        {
            ...
            _logPerformancePolling.Add(new PerformanceDatapoint(
                ...
                [
                    new(FootprintDatapoint.FootPrintEntry.PP.ToString(), _timingPathPlanningOverall),
                    new(FootprintDatapoint.FootPrintEntry.TA.ToString(), _timingTaskAllocationOverall),
                    new(FootprintDatapoint.FootPrintEntry.IS.ToString(), _timingItemStorageOverall),
                    new(FootprintDatapoint.FootPrintEntry.PS.ToString(), _timingPodStorageOverall),
                    new(FootprintDatapoint.FootPrintEntry.RB.ToString(), _timingReplenishmentBatchingOverall),
                    new(FootprintDatapoint.FootPrintEntry.OB.ToString(), _timingOrderBatchingOverall),
                    new(FootprintDatapoint.FootPrintEntry.RP.ToString(), _timingRepositioningOverall)
                ]
            ));
            // Flush on getting too big
