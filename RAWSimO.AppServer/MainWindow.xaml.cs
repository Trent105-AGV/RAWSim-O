...
public partial class MainWindow : Window
{
    ...
    /// </summary>
    private readonly MultiKeyDictionary<ClientType, int, TreeViewItem> _clientItems = new();
    /// <summary>
    ...
    private void EmulatedControlTreeViewItem_KeyDown(object sender, KeyEventArgs e)
    {
        if (e.Key == Key.Return)
        {
            ...
            try
            {
                ...
                string robotCommand = messageResult.Type switch
                {
                    ControlMessageTypesClient.Path => _server.ConvertToRobCmd(messageResult.RobotID,
                        messageResult.WaitTime, messageResult.Path),
                    ControlMessageTypesClient.Pickup => RobotTranslator.EncodeToPickup(),
                    ControlMessageTypesClient.Setdown => RobotTranslator.EncodeToSetdown(),
                    ControlMessageTypesClient.Rest => RobotTranslator.EncodeToRest(),
                    ControlMessageTypesClient.GetItem => RobotTranslator.EncodeToGetItem(),
                    ControlMessageTypesClient.PutItem => RobotTranslator.EncodeToPutItem(),
                    _ => throw new ArgumentException("Unknown command type: " + messageResult.Type.ToString())
                };
