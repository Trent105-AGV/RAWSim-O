...
public class BiDictionary<TFirst, TSecond> : IEnumerable<KeyValuePair<TFirst, TSecond>>
{
    ...
    public TSecond this[TFirst first] { get => _firstToSecond[first];
        set { _firstToSecond[first] = value; _secondToFirst[value] = first; } }
    /// <summary>
    /// Gets the tuple partner or sets a new tuple (replacing a present one, if it exists).
    /// </summary>
    /// <param name="second">The second element of the tuple.</param>
    /// <returns>The first element of the tuple.</returns>
    public TFirst this[TSecond second] { get => _secondToFirst[second];
        set { _secondToFirst[second] = value; _firstToSecond[value] = second; } }
    ...
    public ICollection<TFirst> ValuesFirst => _firstToSecond.Keys;

    /// <summary>
    /// Get only the second elements of all tuples.
    /// </summary>
    /// <returns>Only the second elements of all tuples.</returns>
    public ICollection<TSecond> ValuesSecond => _secondToFirst.Keys;
