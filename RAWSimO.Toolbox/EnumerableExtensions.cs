...
public static class EnumerableExtensions
{
    ...
    public static int RankOf<T>(this IEnumerable<T> source, T element, Func<T, double> elementSortValue)
    {
        ...
        // Get index for the element
        var smaller = 0;
        var elementValue = elementSortValue(element);
        foreach (var e in source)
    ...
    public static T ArgMax<T>(this IEnumerable<T> source, Func<T, double> selector, Func<T, bool> filter)
    {
        ...
        var assigned = false;
        // Search for the maximum
        foreach (var item in source.Where(e => filter(e)))
        {
            // Obtain value for the current element
            var v = selector(item);
            // Check for new maximum
    ...
    public static T ArgMax<T>(this IEnumerable<T> source, params Func<T, double>[] scorers)
    {
        ...
        var max = Enumerable.Repeat(double.NegativeInfinity, scorers.Length).ToArray();
        var value = new double[scorers.Length];
        var assigned = false;
        // Search for the maximum
        foreach (var item in source)
        {
            var better = false;
            for (var scorerIndex = 0; scorerIndex < scorers.Length; scorerIndex++)
            {
                // Obtain value for the current element
                ...
                if (max[scorerIndex] < value[scorerIndex] || !assigned)
                {
