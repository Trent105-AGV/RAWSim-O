    ...
    where K : IExposeVolatileID
{
    ...
    /// </summary>
    private readonly List<VolatileKeyValuePair<K, V>> _entries;
    /// <summary>
    /// The storage that is used for the keys after the collection is sealed.
    /// </summary>
    private readonly K[] _keyStorage;
    /// <summary>
    /// The storage that is used for the values after the collection is sealed.
    /// </summary>
    private readonly VolatileKeyValuePair<K, V>[] _entryStorage;
    /// <summary>
    ...
    public V this[K key]
    {
        get =>
            ...
            _entryStorage[key.VolatileID].Value;
        set =>
            // Set the value
...
public class InflexibleIntDictionary<K> : IInflexibleDictionary<K, int>
{
    ...
    /// </summary>
    private readonly Dictionary<K, int> _storage;
    ...
    {
        get => _storage.ContainsKey(key) ? _storage[key] : 0;
