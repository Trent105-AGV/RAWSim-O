...
public class MDPLP
{
    ...
    public MDPLP(string file, Action<string> logger, SolverType solverType, string[] solverArgs, int roundToDecimals = -1)
    {
        ...
        _solverArgs =
            solverArgs == null ?
                _solverArgs = [] :
                solverArgs.Select(a => { var arg = a.Split('='); return new KeyValuePair<string, string>(arg[0], arg[1]); }).ToArray();
    ...
    /// </summary>
    public bool SolutionAvailable { get; private set; }
    /// <summary>
    /// Some optional solver args to pass to the running solver.
    /// </summary>
    private readonly KeyValuePair<string, string>[] _solverArgs;
    ...
    private void Read(string file, int roundToDecimals = -1)
    {
        ...
        var handleLine = () =>
        {
            ...
            if (line.StartsWith(Constants.IDENT_COEFF_MATRIX)) { blocks[Constants.IDENT_COEFF_MATRIX] = []; currentBlock = Constants.IDENT_COEFF_MATRIX; return; }

            if (line.StartsWith(Constants.IDENT_RHS_VECTOR)) { blocks[Constants.IDENT_RHS_VECTOR] = []; currentBlock = Constants.IDENT_RHS_VECTOR; return; }

            if (line.StartsWith(Constants.IDENT_OBJ_COEFF_VECTOR)) { blocks[Constants.IDENT_OBJ_COEFF_VECTOR] = []; currentBlock = Constants.IDENT_OBJ_COEFF_VECTOR; return; }
        ...
        var coeffElements = string.Join(Constants.DELIMITER_MAIN.ToString(), blocks[Constants.IDENT_COEFF_MATRIX]).Split(Constants.DELIMITER_MAIN);
        foreach (var stringEle in coeffElements)
        {
            var element = stringEle.Split(Constants.DELIMITER_2ND_ORDER);
            var index = element[0].Replace(Constants.TUPLE_LEFT_SIDE, string.Empty).Replace(Constants.TUPLE_RIGHT_SIDE, string.Empty).Split(Constants.DELIMITER_TUPLE);
            var coeffValue = double.Parse(element[1], Constants.Formatter);
        ...
        {
            var rhsValue = double.Parse(v, Constants.Formatter);
            if (roundToDecimals >= 0)
        ...
        {
            var objValue = double.Parse(v, Constants.Formatter);
            if (roundToDecimals >= 0)
    ...
    private void AppendModel(StringBuilder sb)
    {
        ...
        {
            var i = (int)indexPair[0];
            var j = (int)indexPair[1];
            return
    ...
    public void Write(string file)
    {
        ...
        if (Path.GetExtension(file) == ".gz")
        {
            ...
            using (var compressedFileStream = new GZipStream(outputFilestream, CompressionMode.Compress, false))
            using (var sw = new StreamWriter(compressedFileStream))
    ...
    public void DumpLog(string file)
    {
        ...
        {
            using (var outputFilestream = new FileStream(file, FileMode.Create))
            using (var compressedFileStream = new GZipStream(outputFilestream, CompressionMode.Compress))
            using (var sw = new StreamWriter(compressedFileStream))
    ...
    public void LogPerformance(DateTime start, string name, TimeSpan time, bool solAvail, double obj, IEnumerable<string> solverParams)
    {
        ...
        var exists = File.Exists(Constants.CONSOLIDATED_FOOTPRINTS_FILE);
        using (var sw = new StreamWriter(Constants.CONSOLIDATED_FOOTPRINTS_FILE, true))
        {
    ...
    public void Solve()
    {
        ...
        var model = new LinearModel(SolverType.Gurobi, (string msg) => { Log(msg); });
        var xVar = new VariableCollection<int>(model, VariableType.Continuous, 0, double.PositiveInfinity, (int j) => { return "x" + j; });
        ...
        var nnzs = _coefficientMatrix.Keys.GroupBy(k => (int)k[0]).ToDictionary(k => k.Key, v => v.Select(e => (int)e[1]).ToList());
        ...
        // Solve
        model.Optimize();
        var solutionTime = DateTime.Now - before;
        // Get solution
