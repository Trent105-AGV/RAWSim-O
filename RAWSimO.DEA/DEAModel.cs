...
public class DEAModel
{
    ...
    /// </summary>
    private readonly DEAConfiguration _config;
    /// <summary>
    /// All underlying datapoints.
    /// </summary>
    private readonly List<FootprintDatapoint> _datapoints = [];
    /// <summary>
    /// All service units to investigate.
    /// </summary>
    private readonly List<ServiceUnit> _serviceUnits = [];
    /// <summary>
    /// All output measures for all service units.
    /// </summary>
    private readonly MultiKeyDictionary<ServiceUnit, FootprintDatapoint.FootPrintEntry, double> _serviceUnitOutputMeasures = new();
    /// <summary>
    /// A dummy group that will be used, if no group is given at all.
    /// </summary>
    private List<List<Tuple<FootprintDatapoint.FootPrintEntry, string>>> _singleGroupDummy =
        [[]];
    /// <summary>
    /// The scores of the different service units per group.
    /// </summary>
    private readonly MultiKeyDictionary<List<Tuple<FootprintDatapoint.FootPrintEntry, string>>, ServiceUnit, double> _serviceUnitScores =
        new();
    ...
    private void ParseData()
    {
        ...
        var counter = 0;
        using (var sr = new StreamReader(_config.Datafile))
        {
        ...
        var groupIdents = _config.ServiceUnitIdents.ToDictionary(s => s, s => _datapoints.Select(d => d[s].ToString()).Distinct().ToList());
        ...
        foreach (var serviceUnitIdentSet in EnumerationHelpers.CrossProduct(groupIdents.Select(g => g.Value).ToList()))
        {
            ...
            var datapoints = _datapoints.Where(d => ident.All(i => d[i.Item1].ToString() == i.Item2));
    ...
    private Dictionary<string, int> WriteScoreBasedFile(string filename, string firstLinePrefix, string delimiter)
    {
        ...
        using (var sw = new StreamWriter(Path.Combine(_config.OutputDirectory, filename), false))
        {
            // Write header
            sw.WriteLine(
                // Service unit name
        ...
        var valueColumns = new Dictionary<string, int>();
        var index = 0;
    ...
    {
        // Generate dat file
        var datFilename = _config.BoxPlotBaseFilename + ".dat";
        ...
        var plotScript = _config.BoxPlotBaseFilename + ".gp";
        using (var sw = new StreamWriter(Path.Combine(_config.OutputDirectory, plotScript), false))
        {
            sw.WriteLine("reset");
            ...
            {
                var groupIdentifier = _config.GroupsIndividuals != null && _config.GroupsIndividuals.Any() && _config.GroupsIndividuals.First().Any() ?
                    _config.GetGroupName(group) :
                    _config.GetNoGroupName();
                foreach (var ident in _serviceUnits.First().Idents)
                {
                    sw.WriteLine("set title \"" + (string.IsNullOrWhiteSpace(groupIdentifier) ? "" : groupIdentifier + " / ") + ident.Item1 + "\"");
                    sw.WriteLine("set xlabel \"" + ident.Item1 + "-values\"");
        ...
        var commandScript = _config.BoxPlotBaseFilename + ".cmd";
        using (var sw = new StreamWriter(Path.Combine(_config.OutputDirectory, commandScript), false))
        {
    ...
    {
        // Keep track of count
        var overallCount = _serviceUnits.Count * _config.GroupsIndividuals.Count;
        var counter = 0;
        // --> Solve all
        ...
        foreach (var groupIdents in _config.GroupsIndividuals)
        {
            ...
            foreach (var serviceUnitInFocus in _serviceUnits)
            {
                ...
                // Init
                var model = new LinearModel(_config.SolverChoice, null /* Disable output flood for now, use following to enable: (string s) => { _config.Log(s); } */ );
                // --> Init variables
                var efficiencyRating = new Variable(model, VariableType.Continuous, double.NegativeInfinity, double.PositiveInfinity, "EfficiencyRating");
                var weights = new VariableCollection<ServiceUnit>(model, VariableType.Continuous, 0, double.PositiveInfinity, (ServiceUnit u) => { return u.Ident; });
                ...
                if (_config.Inputs.Any())
                {
                    foreach (var inputEntry in _config.Inputs)
                    {
                        ...
                        var inputValues = _serviceUnits.ToDictionary(
                            k => k,
                        ...
                        var minOutputValue = inputValues.Min(v => v.Value);
                        if (minOutputValue < 0)
                            foreach (var serviceUnit in inputValues.Keys.ToList())
                ...
                if (_config.Outputs.Any())
                {
                    ...
                    {
                        // Shift and flip (if required) all values
                        var outputValues = _serviceUnits.ToDictionary(
                            k => k,
                        ...
                        // If there is a negative value, shift all values to positive ones
                        var minOutputValue = outputValues.Min(v => v.Value);
                        if (minOutputValue < 0)
                            foreach (var serviceUnit in outputValues.Keys.ToList())
