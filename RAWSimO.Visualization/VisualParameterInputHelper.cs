...
#pragma warning restore 0067
    ...
    private static Func<object> GetContructorFunction(Type type)
    {
        ...
        if (constructor != null)
        {
            return () => { return constructor.Invoke(new DefaultConstructorIdentificationClass[] { new() }); };
        }
    ...
    private static void AddUIElement(Dispatcher uiDispatcher, IList rootCollection, object rootObject, FieldInfo fieldInfo, object fieldValue)
    {
        ...
        if (typeof(IList).IsAssignableFrom(fieldInfo.FieldType))
        {
            ...
            var listHandleCommand = new ListHandlerCommand(uiDispatcher, treeViewRoot, fieldInfo.FieldType, fieldValue as IList);
        ...
        if (!fieldInfo.FieldType.IsPrimitive && !fieldInfo.FieldType.Equals(typeof(string)))
        {
            ...
            if (helpString != null)
                treeViewRoot.ToolTip = helpString;
            var initClearCommand = new DefaultGeneratorCommand(uiDispatcher, treeViewRoot, rootObject, fieldInfo);
