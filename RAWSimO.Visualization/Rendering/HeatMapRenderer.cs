...
public class HeatMapRenderer
{
    ...
    /// </summary>
    private readonly HeatMapRendererConfiguration _config;
    /// <summary>
    /// The canvas to draw on.
    /// </summary>
    ...
    /// </summary>
    private readonly Transformation2D _transformer;
    /// <summary>
    ...
    {
        var dataType = HeatDataType.PolledLocation;
    ...
    public static string[] GetSubDataChoices(HeatDataType dataType)
    {
        ...
        {
            HeatDataType.PolledLocation => Enum.GetNames(typeof(LocationDatapoint.LocationDataType)).ToArray(),
            HeatDataType.TimeIndependentTripData => Enum.GetNames(typeof(TimeIndependentTripDataPoint.TripDataType))
                .ToArray(),
            HeatDataType.StorageLocationInfo => Enum
                .GetNames(typeof(StorageLocationInfoDatapoint.StorageLocationInfoType))
                .ToArray(),
            ...
            using (StreamReader sr = new StreamReader(file))
            {
                ...
                while ((line = sr.ReadLine()) != null)
                {
                    ...
                    switch (_dataType)
                    {
                        ...
                        {
                            var datapoint = LocationDatapoint.FromCSV(line);
                        ...
                        {
                            var datapoint = StorageLocationInfoDatapoint.FromCSV(line);
                            // Only add datapoint, if it belongs to the set time window (if a window is specified)
                            if (_config.DataTimeFilterLow == _config.DataTimeFilterHigh || _config.DataTimeFilterLow <= datapoint.TimeStamp && datapoint.TimeStamp < _config.DataTimeFilterHigh)
                                // The datapoint made it through all filters - add it
                                _dataPoints.Add(datapoint);
