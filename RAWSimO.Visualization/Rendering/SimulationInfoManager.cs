...
public class SimulationInfoManager
{
    ...
    private IInstanceInfo _instance;
    private readonly SimulationInfoInstance _instanceInfoObject;

    private Dictionary<IGeneralObjectInfo, SimulationInfoObject> _managedInfoObjects = new Dictionary<IGeneralObjectInfo, SimulationInfoObject>();
    private Dictionary<SimulationVisual2D, SimulationInfoObject> _managed2DVisuals = new Dictionary<SimulationVisual2D, SimulationInfoObject>();
    private Dictionary<SimulationVisual3D, SimulationInfoObject> _managed3DVisuals = new Dictionary<SimulationVisual3D, SimulationInfoObject>();

    private SimulationInfoObject _currentInfoObject;

    private TreeView _infoHost;
...
public class SimulationVisualOrderManager
{
    ...
    private readonly string _headerOpenOrders;
    private readonly string _headerCompletedOrders;
        ...
        foreach (var order in openOrders.Concat(completedOrders))
        {
            ...
            if (!_orderStatusOpen.ContainsKey(order))
            {
                ...
                foreach (var position in order.GetInfoPositions())
                {
                    ...
                    {
                        var itemDescription = position as IColoredLetterDescriptionInfo;
                    ...
                    else
                    {
                        ...
                        {
                            var itemDescription = position as ISimpleItemDescriptionInfo;
    ...
    public void UpdateContentInfo(IItemBundleInfo[] bundles)
    {
        ...
        foreach (var item in bundles)
        {
            ...
            if (!_blocksContent.ContainsKey(item.GetInfoItemDescription()))
            {
                ...
                {
                    var itemDescription = item.GetInfoItemDescription() as IColoredLetterDescriptionInfo;
                ...
                else
                {
                    ...
                    {
                        var itemDescription = item.GetInfoItemDescription() as ISimpleItemDescriptionInfo;
        ...
        foreach (var item in _pod.GetInfoInstance().GetInfoItemDescriptions())
        {
            ...
            if (_pod.GetInfoContent(item) > 0)
            {
                ...
                if (!_blocksContent.ContainsKey(item))
                {
                    ...
                    {
                        var itemDescription = item as IColoredLetterDescriptionInfo;
                        textBlock.Background = VisualizationConstants.LetterColorBackgroundBrushes[itemDescription.GetInfoColor()];
                        textBlock.Foreground = VisualizationConstants.LetterColorComplete;
                    ...
                    else
                    {
                        ...
                        {
                            var itemDescription = item as ISimpleItemDescriptionInfo;
                            textBlock.Background = ColorManager.GenerateHueBrush(itemDescription.GetInfoHue());
                            textBlock.Foreground = VisualizationConstants.SimpleItemColorComplete;
                            textBlock.Text =
