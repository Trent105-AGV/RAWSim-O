...
public partial class MainWindow : Window
{
    ...
    /// </summary>
    private ViewMode _viewMode = ViewMode.View2D;

    /// <summary>
    /// Denotes whether to draw additional information like a heatmap or not.
    /// </summary>
    private bool _heatMode = false;

    /// <summary>
    /// The current mode of bot coloring.
    /// </summary>
    private BotColorMode _botColorMode = BotColorMode.DefaultBotDefaultState;
    ...
    /// </summary>
    private SimulationVisualizer _renderer;
    ...
    /// </summary>
    private readonly List<Light> _lights = new List<Light> { new DirectionalLight(Colors.White, new Vector3D(-4, -5, -6)), new DirectionalLight(Colors.White, new Vector3D(5, 6, 4)), };

    /// <summary>
    /// The image to use for the enable image view button.
    /// </summary>
    private readonly BitmapImage _imageEnableInfoView;

    /// <summary>
    /// The image to use for the disable image view button.
    /// </summary>
    private readonly BitmapImage _imageDisableInfoView;

    /// <summary>
    /// A directory to save snapshots to.
    /// </summary>
    private readonly string _snapshotDir = "snapshots";

    /// <summary>
    /// The list of all tiers currently available for visualization.
    /// </summary>
    private List<ITierInfo> _tiers;

    /// <summary>
    /// The index of the tier currently visualized.
    /// </summary>
    private int _focusedTierIndex;
    ...
    /// </summary>
    private readonly OrderGenerator.SimpleItemGeneratorPreConfiguration _simpleItemGeneratorPreConfig = new();
    /// <summary>
    /// The layout configuration to use for generating instances.
    /// </summary>
    private readonly LayoutConfiguration _layoutConfig = new();
    /// <summary>
    /// The configuration to attach to the instance when executing.
    /// </summary>
    private readonly SettingConfiguration _baseConfiguration = new();
    /// <summary>
    /// The control configuration to attach to the instance when executing.
    /// </summary>
    private readonly ControlConfiguration _controlConfiguration = new();
    ...
    private void ExecuteInstance()
    {
        ...
        _instance.SettingConfig.InventoryConfiguration.ColoredWordConfiguration?.WordFile = IOHelper.FindResourceFile(_instance.SettingConfig.InventoryConfiguration.ColoredWordConfiguration.WordFile, ".");
        // Find the specified order-list file (just-in-case)
        _instance.SettingConfig.InventoryConfiguration.FixedInventoryConfiguration?.OrderFile = IOHelper.FindResourceFile(_instance.SettingConfig.InventoryConfiguration.FixedInventoryConfiguration.OrderFile, ".");
        // Attach log-action
    ...
    private void ButtonSaveSettingConfiguration_Click(object sender, RoutedEventArgs e)
    {
        ...
        _baseConfiguration.InventoryConfiguration.ColoredWordConfiguration?.WordFile = System.IO.Path.GetFileName(_baseConfiguration.InventoryConfiguration.ColoredWordConfiguration.WordFile);
        // Remove directory information of order-list file
        _baseConfiguration.InventoryConfiguration.FixedInventoryConfiguration?.OrderFile = System.IO.Path.GetFileName(_baseConfiguration.InventoryConfiguration.FixedInventoryConfiguration.OrderFile);
        // Remove directory information of simple-item file
        _baseConfiguration.InventoryConfiguration.SimpleItemConfiguration?.GeneratorConfigFile = System.IO.Path.GetFileName(_baseConfiguration.InventoryConfiguration.SimpleItemConfiguration.GeneratorConfigFile);
    ...
    private void ButtonSaveOrderList_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            // Generate list
            var list = OrderGenerator.GenerateOrders(wordFile, colors, colorProbs, seed, timeMin, timeMax, count, weightMin, weightMax, positionMin, positionMax, bundleSizeMin, bundleSizeMax, relativeBundleAmount);
    ...
    private void ButtonSaveSimpleItemGeneratorConfig_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            var config = OrderGenerator.GenerateSimpleItemConfiguration(_simpleItemGeneratorPreConfig);
    ...
    private void ButtonGenerateInstance_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            // Find the specified word-list file
            _baseConfiguration.InventoryConfiguration.ColoredWordConfiguration?.WordFile = IOHelper.FindResourceFile(_baseConfiguration.InventoryConfiguration.ColoredWordConfiguration.WordFile, ".");
            // Find the specified order-list file
            _baseConfiguration.InventoryConfiguration.FixedInventoryConfiguration?.OrderFile = IOHelper.FindResourceFile(_baseConfiguration.InventoryConfiguration.FixedInventoryConfiguration.OrderFile, ".");
            // Find the specified generator-config file
            _baseConfiguration.InventoryConfiguration.SimpleItemConfiguration?.GeneratorConfigFile = IOHelper.FindResourceFile(_baseConfiguration.InventoryConfiguration.SimpleItemConfiguration.GeneratorConfigFile, ".");
            // Mark the attached visualization
    ...
    private void RemoveWaypointsBySelectionRect()
    {
        ...
        var lengthTransformer = new Transformation2D(
    ...
    private void ButtonConnectControl_Click(object sender, RoutedEventArgs e)
    {
        if (_instance != null)
        {
            ...
            var remoteControlAdapter = _instance.ActivateRemoteController();
    ...
    /// </summary>
    private string _heatMapInstanceFile;
    /// <summary>
    /// The heat data that shall be visualized for the given instance.
    /// </summary>
    private string _heatMapDataFile;
    /// <summary>
    /// The index of the sub heat information.
    /// </summary>
    private int _heatMapSubDataIndex;
    /// <summary>
    ...
    private void ButtonLoadHeatInfo_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (heatClick == true)
        {
            ...
            var dataType = HeatMapRenderer.ParseHeatDataType(heatDataDialog.FileName);
            // Get sub data type choices
            string[] choices = HeatMapRenderer.GetSubDataChoices(dataType);
            // Let the user choose
            var choiceDialog = new ComboBoxDialog(choices, (int index) => { _heatMapSubDataIndex = index; });
    ...
    private void ButtonHeatRedraw_Click(object sender, RoutedEventArgs e)
    {
        if (File.Exists(_heatMapInstanceFile) && File.Exists(_heatMapDataFile))
        {
            ...
            ReadInstanceForHeatmapRendering();
            // Start rendering
            var config = ParseHeatmapConfiguration();
