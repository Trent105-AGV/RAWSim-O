...
public class RobotTranslator
{
    ...
    /// </summary>
    private static readonly Dictionary<RobotActions, string> _actionTranslations = new()
    {
    ...
    /// </summary>
    private static readonly Dictionary<string, RobotActions> _commandTranslations = new()
    {
    ...
    public static RobotMessageResultServer DecodeTo(string cmds)
    {
        ...
        var parts = cmds.Split(CommunicationConstants.MSG_SUB_DELIMITER);
        var type = (RobotMessageTypesServer)Enum.Parse(typeof(RobotMessageTypesServer), parts[0]);
        return type switch
        {
            RobotMessageTypesServer.Go => new RobotMessageResultServer
            {
                Type = type,
                WaitTime = double.Parse(parts[1], CommunicationConstants.FORMATTER),
                Actions = parts.Skip(2).Select(a => TranslateAction(a))
            },
            RobotMessageTypesServer.Rest or RobotMessageTypesServer.Pickup or RobotMessageTypesServer.Setdown
                or RobotMessageTypesServer.GetItem
                or RobotMessageTypesServer.PutItem => new RobotMessageResultServer { Type = type },
            _ => throw new FormatException("Invalid command message: " + cmds)
        };
    }
    /// <summary>
    ...
    {
        // TODO more robust checking for validness - use error type to mark messages not possible to parse
        var parts = msg.Split(CommunicationConstants.MSG_SUB_DELIMITER);
        var type = (RobotMessageTypesClient)Enum.Parse(typeof(RobotMessageTypesClient), parts[0]);
        return type switch
        {
            RobotMessageTypesClient.Pos => new RobotMessageResultClient { Type = type, WaypointTag = parts[1] },
            RobotMessageTypesClient.Orient => new RobotMessageResultClient
            {
                Type = type, Orientation = (RobotOrientation)Enum.Parse(typeof(RobotOrientation), parts[1])
            },
            RobotMessageTypesClient.Pickup => new RobotMessageResultClient
            {
                Type = type, PickupSuccess = bool.Parse(parts[1])
            },
            RobotMessageTypesClient.Setdown => new RobotMessageResultClient
            {
                Type = type, SetdownSuccess = bool.Parse(parts[1])
            },
            _ => throw new FormatException("Invalid message: " + msg)
        };
    }
