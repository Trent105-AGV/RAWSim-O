...
public class CommunicationServer
{
    ...
    /// </summary>
    private readonly Action<ClientType, int, string> _clientAdded;
    /// <summary>
    /// A method that is called everytime a client is disconnected.
    /// </summary>
    private readonly Action<ClientType, int> _clientRemoved;
    /// <summary>
    /// Action that is called when the location and orientation of a robot is updated. (only for visual sake)
    /// </summary>
    private readonly Action<int, int, RobotOrientation> _locationUpdater;
    /// <summary>
    /// Changes visual feedback about the current mode.
    /// </summary>
    private readonly Action<bool> _errorModeSetter;
    /// <summary>
    ...
    /// </summary>
    private readonly HashSet<Socket> _connectedClients = [];
    /// <summary>
    /// All connected bot-clients by the ID of the robot.
    /// </summary>
    private readonly Dictionary<int, Socket> _botClients = new();
    /// <summary>
    /// All connected input-station-clients by the ID of the station.
    /// </summary>
    private readonly Dictionary<int, Socket> _iStationClients = new();
    /// <summary>
    /// All connected output-station-clients by the ID of the station.
    /// </summary>
    private readonly Dictionary<int, Socket> _oStationClients = new();
    /// <summary>
    /// All connected control-clients by their ID.
    /// </summary>
    private readonly Dictionary<int, Socket> _controlClients = new();

    #region Helpers

    private string GetPaddedString(ClientType clientType, int clientID)
    {
        return clientType + clientID.ToString();
    }
    ...
    private void HandleMetaMessage(Socket socket, string receivedMessage, out bool terminated)
    {
        ...
        var words = receivedMessage.Split(CommunicationConstants.MSG_MAIN_DELIMITER);
    ...
    private void HandleRobotComm(int robotID, string message)
    {
        ...
        if (RobotTranslator.CheckMessageValidnessFrom(message))
        {
            ...
            var messageResult = RobotTranslator.DecodeFrom(message);
                    ...
                    if (_waypointManager != null && _waypointManager.IsKnownTag(messageResult.WaypointTag))
                    {
                        ...
                        if (_lastPositions[robotID] != null && _positions[robotID] != null)
                        {
                            if (_waypointManager.IsConnected(_lastPositions[robotID], _positions[robotID]))
                            {
                                ...
                                var xDelta = _positions[robotID].X - _lastPositions[robotID].X;
                                var yDelta = _positions[robotID].Y - _lastPositions[robotID].Y;
                            ...
                            !_lastPaths[robotID].Contains(_positions[robotID].ID)) // See whether the new position is on the robots path
                        {
                            ...
                            var resolvingPath = _waypointManager.PlanPath(_positions[robotID], _waypointManager.Translate(_lastPaths[robotID][0])).Route.Select(s => s.ID).ToList();
    ...
    private void HandleControlComm(int controlID, string message)
    {
        ...
        {
            // Translate the message
            var messageResult = ControlTranslator.DecodeFrom(message);
    ...
    public void SendBroadMsg(string msg)
    {
        ...
        var codedMsg = Encoding.UTF8.GetBytes(msg);
        foreach (var cl in _connectedClients)
        {
            _logger(cl.RemoteEndPoint + "::>>>>::" + msg + "::" + DateTime.Now.ToString("G"));
            cl.Send(codedMsg);
        }
    }
    ...
    {
        // Prepare
        var codedMsg = Encoding.UTF8.GetBytes(CommunicationConstants.MSG_PREFIX + msg + CommunicationConstants.MSG_TERMINAL);
        // Get the right client
        var client = clientType switch
        {
            ClientType.R => _botClients[clientID],
            ClientType.I => _iStationClients[clientID],
            ClientType.O => _oStationClients[clientID],
            ClientType.C => _controlClients[clientID],
    ...
    {
        var _tcpServerThreadDelegate = new ThreadStart(Listen);
    ...
    public void Listen()
    {
        ...
        var localEP = new IPEndPoint(IPAddress.Any, _serverPort);
        try
        {
    ...
    public void OnConnectRequest(IAsyncResult ar)
    {
        ...
        var newClientSocket = serverSocket.EndAccept(ar);
        var welcomeMessage = "Welcome! The Server is Connected!";
        var byteDateLine = Encoding.UTF8.GetBytes(welcomeMessage);
        //show message to client and show the connection infos
        var ip = newClientSocket.RemoteEndPoint.ToString();
        ...
        var buffer = new MessageBuffer((string message) => { HandleMetaMessage(newClientSocket, message, out terminated); });
        ...
        try
        {
            ...
            {
                // Read the message
                var recv = newClientSocket.Receive(byteDateLine);
                var stringdata = Encoding.UTF8.GetString(byteDateLine, 0, recv);
        ...
        catch (SocketException ex)
        {
            ...
            {
                // Log exception
                _logger(newClientSocket.RemoteEndPoint + "::exception:" + ex.Message + ":" + DateTime.Now.ToString("G"));
                // Try to remove the client
                var foundClient = false; var clientType = ClientType.R; var clientID = 0;
    ...
    /// </summary>
    private readonly Dictionary<int, RobotOrientation> _orientations = new();
    /// <summary>
    /// Keeps track of the positions of the bots. (At least the last known one)
    /// </summary>
    private readonly Dictionary<int, DTOWaypoint> _positions = new();
    /// <summary>
    /// Keeps track of the previous positions of the bot in order to reconstruct orientations.
    /// </summary>
    private readonly Dictionary<int, DTOWaypoint> _lastPositions = new();
    /// <summary>
    /// Stores all paths submitted to the robots.
    /// </summary>
    private readonly Dictionary<int, List<int>> _lastPaths = new();
    /// <summary>
    /// Indicates whether the respective robot is in error handling mode.
    /// </summary>
    private readonly Dictionary<int, bool> _botErrorCorrectionMode = new();
    /// <summary>
    /// The paths used to bring robots back on track.
    /// </summary>
    private readonly Dictionary<int, List<int>> _resolvingPaths = new();

    public string ConvertToRobCmd(int robot, double waitTime, List<int> path)
    {
        // Get current position and orientation of the bot
        var currentOrientation = _orientations[robot];
        var currentWaypoint = _waypointManager.Translate(path.First());
        ...
        // Init command list
        var commands = new List<RobotActions>();
        ...
        {
            // Get next waypoint
            var nextWaypoint = _waypointManager.Translate(wpID);
            ...
            var xDelta = nextWaypoint.X - currentWaypoint.X; var yDelta = nextWaypoint.Y - currentWaypoint.Y;
