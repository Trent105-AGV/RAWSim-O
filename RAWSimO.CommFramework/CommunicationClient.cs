...
public class CommunicationClient
{
    ...
    /// </summary>
    private readonly Action<string> _receiveMessageCallback;
    /// <summary>
    ...
    /// </summary>
    public readonly Action<string> _logger;
    /// <summary>
    ...
    /// </summary>
    private int _nullMessageCounter;
    ...
    public void Connect(string server_ip, string server_port)
    {
        ...
        var ipadd = server_ip;
        var port = Convert.ToInt32(server_port);
        var ie = new IPEndPoint(IPAddress.Parse(ipadd), port);
        // Try to connect
        try
        {
            _socket.Connect(ie);
    ...
    private void ReceiveMsg()
    {
        try
        {
            // Buffer messages
            var data = new byte[1024];
            var buffer = new MessageBuffer((string message) =>
            {
    ...
    public void SendMsg(string message)
    {
        try
        {
            if (IsConnected)
            {
                ...
                var data = new byte[m_length];
                data = Encoding.UTF8.GetBytes(message);
                var i = _socket.Send(data);
                _logger(">>>>::" + message);
            }
        }
        catch (Exception ex)
