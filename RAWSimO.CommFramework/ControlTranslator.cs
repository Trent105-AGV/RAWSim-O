...
public class ControlTranslator
{
    ...
    public static string EncodeTo(int robotID, int waypointID, RobotOrientation robotOrientation)
    {
        ...
        var orientation = robotOrientation switch
        {
            RobotOrientation.North => Math.PI * 1.5,
            RobotOrientation.West => Math.PI,
            RobotOrientation.South => Math.PI * 0.5,
            RobotOrientation.East => 0,
            _ => throw new ArgumentException("Unknown orientation type: " + robotOrientation)
        };
    ...
    {
        var elements = msg.Split(CommunicationConstants.MSG_SUB_DELIMITER);
        var messageType = (ControlMessageTypesServer)Enum.Parse(typeof(ControlMessageTypesServer), elements.First());
        return messageType switch
        {
            ControlMessageTypesServer.Pos => new ControlMessageResultServer
            {
                Type = messageType,
            ...
            ControlMessageTypesServer.Pickup => new ControlMessageResultServer
            {
                Type = messageType, RobotID = int.Parse(elements[1]), PickupSuccess = bool.Parse(elements[2])
            },
            ControlMessageTypesServer.Setdown => new ControlMessageResultServer
            {
                Type = messageType, RobotID = int.Parse(elements[1]), SetdownSuccess = bool.Parse(elements[2])
            },
    ...
    {
        var elements = msg.Split(CommunicationConstants.MSG_SUB_DELIMITER);
        var messageType = (ControlMessageTypesClient)Enum.Parse(typeof(ControlMessageTypesClient), elements.First());
        return messageType switch
        {
            ControlMessageTypesClient.Path => new ControlMessageResultClient
            {
                Type = messageType,
                RobotID = int.Parse(elements[1]),
            ...
            ControlMessageTypesClient.Pickup or ControlMessageTypesClient.Setdown or ControlMessageTypesClient.Rest
                or ControlMessageTypesClient.GetItem
                or ControlMessageTypesClient.PutItem => new ControlMessageResultClient
                {
                    Type = messageType, RobotID = int.Parse(elements[1])
                },
            _ => throw new ArgumentException("Unknown message type: " + messageType)
        };
    }
