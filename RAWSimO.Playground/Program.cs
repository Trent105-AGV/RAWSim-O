...
internal class Program
{
    ...
    /// </summary>
    private static void ExecuteInstance()
    {
        // Choose base directory
        Console.WriteLine("Choose directory:");
        var directories = Directory.EnumerateDirectories(Path.Combine("..", "..", "..", "..", "Material", "Instances")).ToArray();
        ...
        // Choose instance
        var instances = Directory.EnumerateFiles(directories[directoryID], "*.xinst").Concat(Directory.EnumerateFiles(directories[directoryID], "*.xlayo")).ToArray();
        ...
        var settings = Directory.EnumerateFiles(directories[directoryID], "*.xsett").ToArray();
        ...
        // Choose configuration
        var configurations = Directory.EnumerateFiles(directories[directoryID], "*.xconf").ToArray();
    ...
    /// </summary>
    private static void ExecuteInstances()
    {
        ...
        // Get all files
        var instances = Directory.EnumerateFiles(directory, "*.xinst").Concat(Directory.EnumerateFiles(directory, "*.xlayo")).ToList();
        var settings = Directory.EnumerateFiles(directory, "*.xsett").ToList();
        var configs = Directory.EnumerateFiles(directory, "*.xconf").ToList();
        var seeds = Enumerable.Range(1, (int)seedCount).Select(s => s.ToString()).ToList();
        ...
        if (paraDeg < 2)
        {
            foreach (var combination in combinations)
            {
                ...
                Console.WriteLine("######################################");
                Console.WriteLine("---> Executing combination " + (++counter) + " / " + combinations.Count());
                Console.WriteLine("######################################");
                Console.WriteLine("--> Calling wrapped RAWSimO.CLI ...");
        ...
        else
        {
            ...
            Console.WriteLine("######################################");
            var programName = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "RAWSimO.CLI.exe");
            var total = combinations.Count; var completed = 0; var started = 0;
            ...
            var logTimer = new Timer(new TimerCallback((object unused) => { log(); }), null, 2000, 5000);
            ...
            {
                // Prepare sub-process
                var arguments = combination[0] + " " + combination[1] + " " + combination[2] + " " + directory + " " + combination[3];
                var startInfo = new ProcessStartInfo(programName, arguments) { };
