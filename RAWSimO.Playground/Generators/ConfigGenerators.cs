...
internal class ConfigGenerators
{
    ...
    public static void GenerateRotterdamPhase2Settings()
    {
        ...
        var skuFiles = new Dictionary<string, string> { { "Mu-1000.xgenc", "1000" }, { "Mu-10000.xgenc", "10000" }, };
        var orderSizeScenarios = new List<int> { 1, 2, 3 };
        var returnOrderProbabilities = new List<double> { 0, 0.3 };

        //List<int> pickStationCapacities = new List<int> { 6, 12, 18 };
        var pickStationCounts = new List<double> { 1.0 / 6.0, 2.0 / 6.0, 3.0 / 6.0, 4.0 / 6.0, 5.0 / 6.0, 6.0 / 6.0, };
        //List<double> replStationCounts = new List<double> { 1.0 / 3.0, 2.0 / 3.0, 1.0 };
        var botsPerStations = new List<int> { 2, 3, 4, 5, 6 };
            ...
            foreach (var skuFile in skuFiles)
            {
                foreach (var pickStationCount in pickStationCounts)
                {
                    foreach (var orderSizeScenario in orderSizeScenarios)
                    {
                        ...
                        {
                            var setting = GetRotterdamBaseSetting();
                            ...
                            var name = "MaTi" +
                                       ...
                                       "";
                            setting.Name = name;
                            var fileName = setting.Name + ".xsett";
    ...
    public static void GenerateRotterdamControllers()
    {
        ...
        var pathplanners = new List<PathPlanningConfiguration>
        {
            // WHCA_v^*
        ...
        var taskAllocaters = new List<TaskAllocationConfiguration>
        {
        ...
        var itemStoragers = new List<ItemStorageConfiguration>
        {
            // Random
        ...
        var podStoragers = new List<PodStorageConfiguration>
        {
            // Random
        ...
        var replenishmentBatchers = new List<ReplenishmentBatchingConfiguration>
        {
            // Random
        ...
        var orderBatchers = new List<OrderBatchingConfiguration>
        {
            // Random
        ...
        var forbiddenCombinations = new SymmetricKeyDictionary<ControllerConfigurationBase, bool>();
        ...
        var configurations = new List<ControlConfiguration>();
                    ...
                    foreach (var podStorager in podStoragers)
                    {
                        foreach (var replenishmentBatcher in replenishmentBatchers)
                        {
                            foreach (var orderBatcher in orderBatchers)
                            {
                                ...
                                var configObjects = new List<ControllerConfigurationBase>
        ...
        foreach (var config in configurations)
        {
            ...
            // Save
            var fileName = config.Name + ".xconf";
            Console.WriteLine("Saving " + fileName + " ...");
    ...
    {
        var stationControllers = new Dictionary<bool, StationActivationConfiguration>
        {
        ...
        var speedUtilityWeights = new List<Tuple<double, double>>
        {
            new(0, 1),
            new(1, 0),
            new(1, 1),
        };
        var positioningControllers = new List<Tuple<PodStorageConfiguration, RepositioningConfiguration>>
        {
            new(
        ...
        var subExperiments = new List<RepositioningSubExperiment>
        {
            new() { NightDown = false, BotsPerOStation = 4, BotAllocation = new Tuple<bool, double, double, double>(false, 1, 3, 0) },
            new() { NightDown = false, BotsPerOStation = 4, BotAllocation = new Tuple<bool, double, double, double>(true, 1, 2, 1) },
            new() { NightDown = false, BotsPerOStation = 5, BotAllocation = new Tuple<bool, double, double, double>(true, 1, 3, 1) },
            new() { NightDown = true, BotsPerOStation = 4, BotAllocation = new Tuple<bool, double, double, double>(true, 1, 3, 0) },
            new() { NightDown = true, BotsPerOStation = 4, BotAllocation = new Tuple<bool, double, double, double>(false, 1, 3, 0) },
        };
        var counter = 0;
        foreach (var subExperiment in subExperiments)
        {
            var dir = "SubExp" + ++counter;
            ...
            foreach (var speedUtilityWeight in speedUtilityWeights)
            {
                foreach (var positioningController in positioningControllers)
                {
                    ...
                    {
                        // Prepare setting
                        var setting = GetRepositioningBaseSetting();
                        setting.OverrideConfig = new OverrideConfiguration
                        {
                            OverrideBotCountPerOStation = true,
                        ...
                        if (subExperiment.NightDown)
                        {
                            ...
                            setting.InventoryConfiguration.BatchInventoryConfiguration = new BatchInventoryConfiguration
                            {
                                ...
                                BundleBatches = [new() { Key = TimeSpan.FromHours(16).TotalSeconds, Value = 0.75 }],
                                OrderBatches = [new() { Key = TimeSpan.FromHours(22).TotalSeconds, Value = 1500 }],
                            };
                        }
                        // Prepare config
                        var config = new ControlConfiguration
                        {
                        ...
                        // Save it
                        var fileNameController = Path.Combine(dir, config.Name + ".xconf");
                        Console.WriteLine("Saving " + fileNameController + " ...");
                        InstanceIO.WriteConfiguration(fileNameController, config);
                        var fileNameSetting = Path.Combine(dir, setting.Name + ".xsett");
