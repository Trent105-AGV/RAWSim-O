...
public class Graph
{
    ...
    /// [nodeID][edgeID]
    /// </summary>
    public readonly Dictionary<int, ElevatorEdge[]> ElevatorEdges;
    ...
    public void GenerateBackwardEgdes()
    {
        ...
        var dynBackwardEdges = new Dictionary<int, List<Edge>>();
        for (var nodeId = 0; nodeId < NodeCount; nodeId++)
            dynBackwardEdges[nodeId] = [];

        //fill dynamic Data Structure
        for (var nodeId = 0; nodeId < NodeCount; nodeId++)
        for (var edgeId = 0; edgeId < Edges[nodeId].Length; edgeId++)
    ...
    public List<int> getIntermediateNodes(int node1, int node2)
    {
        ...
        var angle = RadToDegree(Math.Atan2(PositionY[node2] - PositionY[node1], PositionX[node2] - PositionX[node1]));
