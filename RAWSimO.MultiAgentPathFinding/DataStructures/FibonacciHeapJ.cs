...
public class FibonacciHeapJ<T>
{
    ...
    public Entry Dequeue()
    {
        ...
        if (z != null)
        {
            if (z.child != null)
            {
                ...
                foreach (var x in z.child.nodelist())
                {
                    min = concatenateNode(min, x);
                    x.parent = null;
                }
    ...
    private void consolidate()
    {
        ...
        var arraySize = (int)Math.Floor(Math.Log(n) * oneOverLogPhi) + 1;
        var array = new Entry[arraySize];

        if (min != null)
        {
            foreach (var w in min.nodelist())
            {
