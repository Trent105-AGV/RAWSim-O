...
public class IndependenceDetection
{
    ...
    {
        LowLevSolver = lowLevelSolver;
        _graph = graph;
        _deadlockHandler = new DeadlockHandler(_graph, seed);
    }
    ...
    public void FindPaths(double currentTime, List<Agent> agents, double runtimeLimit, PathPlanningCommunicator communicator)
    {
        ...
        var agentGroups = new Dictionary<int, List<Agent>>(agents.ToDictionary(a => a.ID, a => new List<Agent>([a])));
        ...
        while (agentGroups.Count > 1)
        {
            ...
            var conflictAgent1 = -1;
            var conflictGroup1 = -1;
            var conflictAgent2 = -1;
            var conflictGroup2 = -1;
            int conflictNode;
            ...
            if (foundConflict)
            {
                ...
                //reset Assignment
                for (var agentIndex = 0; agentIndex < agentGroups[conflictGroup1].Count; agentIndex++)
