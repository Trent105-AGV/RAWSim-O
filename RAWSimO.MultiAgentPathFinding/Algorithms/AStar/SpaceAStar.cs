...
public class SpaceAStar : AStarBase
{
    ...
    /// The physic object
    /// </summary>
    private readonly short _startAngle;
    ...
    /// </summary>
    private readonly Edge[] _successorEdges;
        ...
        : base(startNode, endNode)
    {
        ...
        _startAngle = Graph.RadToDegree(orientation);
        _successorEdges = new Edge[_graph.NodeCount];
    }
    ...
    public override double h(int node)
    {
        ...
        var distance = Math.Sqrt((_graph.PositionX[node] - _graph.PositionX[GoalNode]) * (_graph.PositionX[node] - _graph.PositionX[GoalNode]) + (_graph.PositionY[node] - _graph.PositionY[GoalNode]) * (_graph.PositionY[node] - _graph.PositionY[GoalNode])) + (_biasedCost == null || !_biasedCost.ContainsKey(node) ? 0 : _biasedCost[node]);
    ...
    {
        //normal Edges
        for (var i = 0; i < _graph.Edges[n].Length; i++)
        {
    ...
    public void getReservationsAndPath(double startTime, ref Path path, out List<ReservationTable.Interval> reservations)
    {
        ...
        for (var edgeNo = 0; edgeNo < edgesPath.Count;)
        {
            ...
            for (var i = 0; i < checkPointNodes.Count - 1; i++)
