...
public class SpaceTimeAStar : AStarBase
{
    ...
    /// </summary>
    public readonly List<int> NodeBackpointerId;

    /// <summary>
    /// backpointer of the node, where we turned last time
    /// </summary>
    public readonly List<int> NodeBackpointerLastStopId;

    /// <summary>
    /// The mapping of generated nodes to the 2D Edge
    /// null = Wait Edge
    /// </summary>
    public readonly List<Edge> NodeBackpointerEdge;
    ...
    /// </summary>
    public readonly List<double> NodeTime;

    /// <summary>
    /// temporary backpointer of the generated nodes
    /// </summary>
    public readonly List<int> NodeBackpointerIdTemp;

    /// <summary>
    /// temporary backpointer of the node, where we turned last time
    /// </summary>
    public readonly List<int> NodeBackpointerLastTurnIdTemp;

    /// <summary>
    /// The temporary mapping of generated nodes to the 2D Edge
    /// null = Wait Edge
    /// </summary>
    public readonly List<Edge> NodeBackpointerEdgeTemp;

    /// <summary>
    /// The temporary mapping of generated nodes to time
    /// NodeTime[100] = point in time of the 100th generated node
    /// </summary>
    public readonly List<double> NodeTimeTemp;
    ...
    /// </summary>
    public int WaitStepsBeforeStart;
    ...
    /// </summary>
    protected int _numNodeId;

    /// <summary>
    /// Length of a time step.
    /// </summary>
    protected readonly double _lengthOfAWaitStep;

    /// <summary>
    /// Length of a window.
    /// </summary>
    protected readonly double _lengthOfAWindow;
    ...
    /// The RRA* Algorithm
    /// </summary>
    protected readonly ReverseResumableAStar _RRAStar;

    /// <summary>
    /// The tie breaking is turned on
    /// </summary>
    protected readonly bool _tieBreaking;
    ...
    {
        _graph = graph;
        _lengthOfAWaitStep = lengthOfAWaitStep;
        _lengthOfAWindow = lengthOfAWindow;
        _reservationTable = reservationTable;
        _agent = agent;
        _RRAStar = rraStar;
        _tieBreaking = tieBreaking;

        //built mappings
        NodeTime = [];
        NodeBackpointerId = [];
        NodeBackpointerLastStopId = [];
        NodeBackpointerEdge = [];
        NodeTimeTemp = [];
        NodeBackpointerIdTemp = [];
        NodeBackpointerLastTurnIdTemp = [];
        NodeBackpointerEdgeTemp = [];
    ...
    public override double h(int node)
    {
        ...
        if (_RRAStar.Closed.Contains(node2d))
            return
                ...
                // Biased costs
                (BiasedCost == null || !BiasedCost.ContainsKey(node2d) ? 0 : BiasedCost[node2d]);
        //find RRA* solution
    ...
    protected override IEnumerable<int> Successors(int n)
    {
        ...
        //current angle
        var lastStopAngleAfterTurn = GetLastStopAngleAfterTurn(n);
        ...
        foreach (var direction in _graph.Edges[NodeTo2D(n)])
        {
            ...
            var lastStopId = NodeBackpointerLastStopId[n];
            ...
            var lastStopAngleBeforeTurn = GetLastStopAngleAfterTurn(lastStopId);
            var timeToTurn = _agent.Physics.getTimeNeededToTurn(Graph.DegreeToRad(lastStopAngleBeforeTurn), Graph.DegreeToRad(direction.Angle));
                ...
                foreach (var edge in _graph.Edges[currentNode])
                {
                    ...
                    if (Math.Abs(edge.Angle - agentAngle) < 2)
                    {
                        ...
                        if (pathFree)
                        {
                            ...
                            var succ = _numNodeId + NodeTimeTemp.Count - 1;
    ...
    public void AddCheckPointDistances(int nodeFrom, int nodeTo, List<int> checkPointNodes, List<double> checkPointDistances)
    {
        ...
        for (var i = 0; i < checkPointDistances.Count; i++)
    ...
    public void GetPathAndReservations(ref Path path, out List<ReservationTable.Interval> reservations, int specifiedNode, double startTime)
    {
        ...
        while (node >= 0)
        {
            ...
            for (var i = checkpointNodes.Count - 1; i >= 0; i--)
            {
                if (i == 0)
        ...
        for (var i = reservations.Count - 2; i >= 0; i--)
        {
