...
public class PASMethod : PathFinder
{
    ...
    /// The RRA* Searches
    /// </summary>
    public readonly Dictionary<int, ReverseResumableAStar> rraStars;
    ...
    public override void FindPaths(double currentTime, List<Agent> agents)
    {
        ...
        //loop
        var cancelAt = currentTime + LengthOfAWindow;
    ...
    private void setAgentBackToPast(int agentId, PASActionGenerator actionGenerator, ref int node, ref List<ReservationTable.Interval> reservations, double time)
    {
        ...
        while (currentTime > time)
        {
            ...
            var previousNode = node > actionGenerator.NodeBackpointerLastStopId[node] ? actionGenerator.NodeBackpointerLastStopId[node] : actionGenerator.NodeBackpointerId[node];
...
public class PASActionGenerator : SpaceTimeAStar
{
    ...
    public Tuple<int, List<ReservationTable.Interval>, List<Collision>> GenerateWaitNode(int parent, ReservationTable reservationTable)
    {
        ...
        //add successor
        var waitNode = Tuple.Create(_numNodeId, new List<ReservationTable.Interval>([new ReservationTable.Interval(NodeTo2D(parent), NodeTime[parent], NodeTime[parent] + _lengthOfAWaitStep)
        ]), collisions);
    ...
    public List<Tuple<int, List<ReservationTable.Interval>, List<Collision>>> GetActions(int n, int prio, ReservationTable reservationTable)
    {
        ...
        {
            //add successor
            successors.Add(Tuple.Create(_numNodeId, new List<ReservationTable.Interval>([new ReservationTable.Interval(NodeTo2D(n), NodeTime[n], NodeTime[n] + _lengthOfAWaitStep)
            ]), collisions));
