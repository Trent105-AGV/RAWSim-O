...
public class WHCAnStarMethod : PathFinder
{
    ...
    /// </summary>
    private readonly Dictionary<int, List<ReservationTable.Interval>> _calculatedReservations;
        ...
        : base(graph, seed, logger)
    {
        ...
        {
            _calculatedReservations.Add(agentIds[i],
                [..new ReservationTable.Interval[] { new(startIds[i], 0, double.PositiveInfinity) }]);
    ...
    public override void FindPaths(double currentTime, List<Agent> agents)
    {
        ...
        foreach (var agent in agents.Where(a => a.RequestReoptimization && !a.FixedPosition && a.NextNode != a.DestinationNode))
        {
            ...
            if (!found)
            {
                ...
                _calculatedReservations[agent.ID] = new List<ReservationTable.Interval>([new(agent.NextNode, 0, double.PositiveInfinity)
                ]);
