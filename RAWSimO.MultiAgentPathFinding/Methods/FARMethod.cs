...
public class FARMethod : PathFinder
{
    ...
    /// </summary>
    public readonly Dictionary<int, int> _waitFor;

    /// <summary>
    /// Last passed node of Agent i
    /// </summary>
    public readonly Dictionary<int, ReverseResumableAStar> _rraStar;
    ...
    /// </summary>
    public readonly Dictionary<int, double> _moveTime;

    /// <summary>
    /// point in time where the last wait action ended
    /// </summary>
    public readonly Dictionary<int, double> _waitTime;

    /// <summary>
    /// Reservation Table
    /// </summary>
    public readonly ReservationTable _reservationTable;
    ...
    /// </summary>
    public readonly double MaximumWaitTime = 30.0;
    ...
    /// </summary>
    public readonly EvadingStrategy _evadingStragety;
    ...
    /// </summary>
    public readonly Dictionary<int, HashSet<int>> _es2evadedFrom;
    ...
    public override void FindPaths(double currentTime, List<Agent> agents)
    {
        ...
        foreach (var agent in agents.Where(a => !a.FixedPosition && a.NextNode != a.DestinationNode && a.ArrivalTimeAtNextNode <= currentTime && a.RequestReoptimization))
        {
            ...
            {
                //new search
                rraStar = new ReverseResumableAStar(Graph, agent, agent.Physics, agent.DestinationNode, []);
            ...
            if (!deadlockBreakingManeuver)
            {
                ...
                var waitForSet = new HashSet<int>();
            ...
            switch (_evadingStragety)
            {
                case EvadingStrategy.EvadeByRerouting:
                    ...
                    //obstacle free
                    if (!found)
                        rraStar = _rraStar[agent.ID] = new ReverseResumableAStar(Graph, agent, agent.Physics, agent.DestinationNode,
                            []);


                    for (var tries = 1; tries <= Es1MaximumNumberOfBreakingManeuverTries; tries++)
                    {
    ...
    private void _setPath(List<int> nextHopNodes, Agent agent)
    {
        ...
        agent.Path.Clear();
        for (var i = 0; i < nextHopNodes.Count; i++)
        {
    ...
    private List<int> _getNextHopNodes(double currentTime, Agent agent, out int reservationOwnerAgentId, out int reservationOwnerNodeId)
    {
        ...
        {
            //create intervals
            var intervals = _reservationTable.CreateIntervals(currentTime, currentTime, 0, agent.Physics, nextHopNodes[0], nextHopNodes[nextHopNodes.Count - 1], true);
            ...
            for (var i = nextHopNodes.Count - 1; i >= deleteFrom; i--)
