...
public class CBSMethod : PathFinder
{
    ...
    /// </summary>
    private readonly ReservationTable _agentReservationTable;
    ...
    public override void FindPaths(double currentTime, List<Agent> agents)
    {
        ...
        foreach (var agent in agents.Where(a => !a.FixedPosition))
        {
            var agentSolved = Solve(conflictTree.Root, currentTime, agent);
            if (!agentSolved)
            {
            ...
            Communicator.LogDefault(
                ...
                string.Join(",",
                    unsolvableAgents.Select(a =>
                        "Agent" + a.ID + "(" + a.NextNode + "->" +
                        a.DestinationNode + ")")));
        var bestNode = conflictTree.Root;

        //search loop
        var p = conflictTree.Root;
        while (open.Count > 0)
        {
            //local variables
            int agentId1;
